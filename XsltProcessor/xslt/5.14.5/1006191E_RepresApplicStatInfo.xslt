...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:rasi="urn:customs.ru:Information:CustomsDocuments:RepresApplicStatInfo:5.14.3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.14.3">
	...
	<xsl:template match="rasi:IdentityCard">
		...
		<br/>
		<xsl:if test="rasi:IdentityCardName">
			<xsl:value-of select="rasi:IdentityCardName"/>
		</xsl:if>
		<xsl:if test="rasi:IdentityCardCode">
			(код вида документа <xsl:value-of select="rasi:IdentityCardCode"/>)
		</xsl:if>
		<xsl:if test="rasi:IdentityCardName or rasi:IdentityCardCode">
			<br/>
		</xsl:if>
		<xsl:if test="rasi:IdentityCardSeries or rasi:IdentityCardNumber">
			<span>
				Серия номер:
				<xsl:value-of select="rasi:IdentityCardSeries"/>
				<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
				<xsl:value-of select="rasi:IdentityCardNumber"/>
			</span>
			<br/>
		</xsl:if>
		<xsl:if test="rasi:IdentityCardDate or rasi:OrganizationName">
			Выдан
			<xsl:call-template name="russian_date">
				<xsl:with-param name="dateIn" select="rasi:IdentityCardDate"/>
			</xsl:call-template>
			<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
			<xsl:value-of select="rasi:OrganizationName"/>
			<br/>
