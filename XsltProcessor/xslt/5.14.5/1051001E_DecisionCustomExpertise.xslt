...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dce="urn:customs.ru:Information:CustomsDocuments:DecisionCustomExpertise:5.14.5" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.14.3" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:CategoryCust="urn:customs.ru:Categories:3.0.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
				...
				<div class="page graphMain">
						...
						<tbody>
							...
							<tr>
								<td align="center" class="graphLittle" colspan="2">
									<xsl:text>(фамилия, имя, отчество (при наличии), должность, наименование таможенного органа, выявшего необходимость применения меры по минимизации рисков: проведение таможенной экспертизы)</xsl:text>
								</td>
							</tr>
						</tbody>
					</table>
						...
						<tbody>
							...
							<tr>
								<td align="center" class="graphLittle" colspan="2">
									...
									от 3 августа 2018 г. № 289-ФЗ "О таможенном регулировании в Российской Федерации и о внесении изменений в отдельные законодательные акты Российской Федерации")</xsl:text>
								</td>
							</tr>
						</tbody>
					</table>
					...
					<table>
						<tbody>
							...
							<tr>
								<td align="center" class="graphLittle" colspan="2">
									<xsl:text>(номер профиля риска)</xsl:text>
									<sup>1</sup>
								</td>
							</tr>
						</tbody>
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="2" class="graphLittle">
									<xsl:text>Указывается в случае назначения таможенной экспертизы в рамках реализации системы управления рисками.</xsl:text>
								</td>
							</tr>
							<tr>
								<td class="graphLittle">
	...
	<xsl:template match="dce:PresentedDocuments | dce:OtherDoc">
		<xsl:for-each select="*">
			<xsl:choose>
	...
	<xsl:template match="dce:ReceivedDocuments">
		...
		<xsl:for-each select="*">
				...
				<xsl:variable name="addNum">
					<xsl:choose>
						<xsl:when test="dce:AddNumber">
							<xsl:value-of select="dce:AddNumber"/>
						</xsl:when>
						<xsl:otherwise><xsl:value-of select="''"/></xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
			...
			<xsl:if test="dce:MPO_Number">
				<xsl:value-of select="dce:MPO_Number"/>
			</xsl:if>
