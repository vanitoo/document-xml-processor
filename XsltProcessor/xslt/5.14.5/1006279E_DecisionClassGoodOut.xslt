						...
						<tbody>
							...
							<tr>
								...
								<br/>
									<br/>
									<i>
									РКТ-<xsl:value-of select="dco:RegistrationNumber/dco:CustomsCode"/>-<xsl:value-of select="substring(dco:RegistrationNumber/dco:Date,3,2)"/>/<xsl:value-of select="dco:RegistrationNumber/dco:SerialNumber"/>
										<xsl:if test="dco:DocKind">
											<xsl:text>Д</xsl:text>
										</xsl:if>
										...
										<xsl:when test="dco:Goods/dco:IsPartialGood='false' or dco:Goods/dco:IsPartialGood='0' or dco:Goods/dco:IsPartialGood='f'">
											<xsl:choose>
												<xsl:when test="count(dco:Goods/dco:DTList)>1">
													<xsl:for-each select="dco:Goods/dco:DTList/dco:Numeric_TNVEDCode/dco:ListNumeric">
												...
												<xsl:otherwise>
													...
													<span class="value">
														<xsl:text>&#160;&#160;</xsl:text>
														<xsl:value-of select="dco:Goods/dco:DTList/dco:Numeric_TNVEDCode/dco:ListNumeric"/>
														<xsl:text>&#160;&#160;</xsl:text>
													</span>
													<xsl:text>, № документа (таможенной декларации или другого документа, используемого в качестве таможенной декларации), в котором таможенным органом выявлен неверный классификационный код товара в соответствии с единой Товарной номенклатурой внешнеэкономической деятельности Евразийского экономического союза</xsl:text>
													<span class="value">
