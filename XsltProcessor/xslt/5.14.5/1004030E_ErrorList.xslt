...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:err="urn:customs.ru:Information:ExchangeDocuments:ErrorList:5.14.5" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   <xsl:template match="/">
      <html>
         <head>
   ...
   <xsl:template match="err:ErrorList">
	  ...
	  <xsl:if test="err:CheckVersion">
		  <br/><span style="font-size: 13pt;">Версия алгоритма контроля: </span>
   ...
   <xsl:template match="err:Error">
		...
		<xsl:if test="err:ErrField">
			<br/><span>Номер графа ДТ/ДТС/КДТ: </span><xsl:value-of select="err:ErrField"/>
		</xsl:if>
		<xsl:if test="err:GTDGoodsNumber">
         ...
         <xsl:if test="err:CriterionGroup">
            <br/><span>Группа критериев: </span><xsl:apply-templates select="err:CriterionGroup"/>
         </xsl:if>
         <xsl:if test="err:CriterionNum">
            <br/><span>Код критерия: </span><xsl:apply-templates select="err:CriterionNum"/>
         </xsl:if>
         <xsl:if test="err:TechID">
            <br/><span>Технические идентификаторы: </span>
