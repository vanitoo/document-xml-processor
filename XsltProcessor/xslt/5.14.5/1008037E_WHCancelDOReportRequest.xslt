...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:cdor="urn:customs.ru:Information:WarehouseDocuments:WHCancelDOReportRequest:5.14.3" xmlns:catWH_ru="urn:customs.ru:Information:WarehouseDocuments:WarehouseCommonAggregateTypesCust:5.14.3" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	<!-- Шаблон для типа WHCancelDOReportRequestType -->
	<xsl:template match="cdor:WHCancelDOReportRequest">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Запрос на отмену подачи отчетности ДО-1 / ДО-2 / ДО-3</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="cdor:DOReportLinkData">
						<div class="title marg-top">Сведения об отчете, в отношении которого составлен запрос</div>
						<div>
							<xsl:apply-templates select="cdor:DOReportLinkData">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="cdor:RequestDetails">
						<div class="title marg-top">Обоснование запроса</div>
						<div>
							<xsl:apply-templates select="cdor:RequestDetails">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="cdor:WarehouseOwner">
						<div class="title marg-top">Данные о владельце склада</div>
						<div>
							<xsl:apply-templates select="cdor:WarehouseOwner">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<xsl:template match="cdor:DOReportLinkData">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<!-- Шаблон для типа cdor:RequestDetailsType -->
	<xsl:template match="cdor:RequestDetails">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					<td class="annot graphMain" style="width:50%">Индикатор обоснования отмены</td>
					<td class="value graphMain" style="width:50%">
						<xsl:value-of select="cdor:CancelationSign"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					<td class="annot graphMain" style="width:50%">Текстовое обоснование отмены подачи отчетности</td>
					<td class="graphMain value" style="width:50%">
						<xsl:for-each select="cdor:CancelationReasons">
							<xsl:value-of select="."/>
	...
	<xsl:template match="cdor:WarehouseOwner">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
