...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:ntfin="urn:customs.ru:Information:CustomsDocuments:NotifInspectionReqGoods:5.14.5" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template name="gdt_number">
	   ...
	   <xsl:if test="$add_number != ''">
		   <xsl:value-of select="$add_number"/>
       </xsl:if>
   ...
   <xsl:template match="ntfin:NotifInspectionReqGoods">
		<xsl:variable name="form">
			<xsl:choose>
				<xsl:when test="ntfin:FormSign = 'true' or ntfin:FormSign = 'TRUE' or ntfin:FormSign=1">досмотр</xsl:when>
                           ...
                           <td align="left">
							   <xsl:choose>
									<xsl:when test="ntfin:DocumentKind='1' and $form='осмотр'">
										<xsl:text>  В связи с необходимостью проведения таможенного контроля в форме таможенного осмотра на основании статьи 328 Таможенного кодекса ЕАЭС требуется предъявление товаров, совершение таможенных операций  ДТ № </xsl:text>
									</xsl:when>
								</xsl:choose>
								<span class="bordered">
									<xsl:call-template name="DocID">
                           ...
                           <table align="left" style="width:210mm">
                              <tbody>
                                 <tr>
                                    ...
                                    <td class="bordered" style="width:40mm"><br/>
                                       <!--xsl:call-template name="russian_date">
