...
<xsl:stylesheet exclude-result-prefixes="cos catSert_ru cat_ru" version="1.0" xmlns:catSert_ru="urn:customs.ru:Information:SertifDocuments:SertifCommonAggregateTypesCust:5.16.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:cos="urn:customs.ru:Information:SertifDocuments:ConformanceSertif:5.16.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
										...
										<td class="graph" style="width:120mm" valign="bottom">
											...
											<xsl:choose>
												...
												<xsl:otherwise>
													<span class="status">
														<xsl:choose>
															<xsl:when test="cos:Status='2'"> (действие приостановлено)</xsl:when>
															<xsl:when test="cos:Status='3'"> (действие временно приостановлено)</xsl:when>
															<xsl:when test="cos:Status='5'"> (действие прекращено)</xsl:when>
															<xsl:when test="cos:Status='7'"> (действие аннулировано)</xsl:when>
															<xsl:when test="cos:Status='8'"> (в архиве)</xsl:when>
															<xsl:when test="cos:Status='9'"> (не опубликован)</xsl:when>
															<xsl:when test="cos:Status='10'"> (исключен из реестра)</xsl:when>
														</xsl:choose>
													</span>
												</xsl:otherwise>
											</xsl:choose>
								...
								<tr valign="top">
									...
									<td class="graph value" style="width:50%">
										<xsl:choose>
											...
											<xsl:when test="cos:Section=2">Сертификат соответствия (СС) продукции требованиям тех. рег. Таможенного союза</xsl:when>
											<xsl:when test="cos:Section=3">Сертификат соответствия продукции, включенной в Единый перечень продукции в рамках Таможенного союза</xsl:when>
