								...
								<tbody>
									...
									<tr>
										<td class="value" align="center">
											...
											<xsl:if test="prs:CBC">; КБК: <xsl:value-of select="prs:CBC"/> <!--prs:CBC удален, основание:  IMO-000754 Письмо ГУФТДиТР от 21.08.2019 № 05-15/51496 "Об Альбоме форматов электронных
форм документов"-->
											</xsl:if>
										</td>
									</tr>
	...
	<xsl:template name="gtd_date">		<xsl:param name="dateIn"/>		<xsl:choose>			<xsl:when test="substring($dateIn,5,1)='-' and substring($dateIn,8,1)='-'">				<xsl:value-of select="substring($dateIn,9,2)"/>				<xsl:value-of select="substring($dateIn,6,2)"/>				<xsl:value-of select="substring($dateIn,3,2)"/>			</xsl:when>			<xsl:otherwise>				<xsl:value-of select="$dateIn"/>			</xsl:otherwise>		</xsl:choose>	</xsl:template></xsl:stylesheet>
