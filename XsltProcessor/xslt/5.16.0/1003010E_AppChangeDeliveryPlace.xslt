...
<xsl:stylesheet version="1.0" xmlns:acdt="urn:customs.ru:Information:TransportDocuments:AppChangeDeliveryPlace:5.16.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
						...
						<tbody class="graphMain">
							...
							<tr>
								<td class="value">
									...
									<xsl:if test="acdt:TransportDoc/acdt:MDPNumber"> Книжка МДП: <xsl:value-of select="acdt:TransportDoc/acdt:MDPNumber/acdt:TIRSeries"/>
										<xsl:text> </xsl:text>
										<xsl:value-of select="acdt:TransportDoc/acdt:MDPNumber/acdt:TIRID"/>
										<xsl:if test="acdt:TransportDoc/acdt:MDPNumber/acdt:RegistrationDate"> от <xsl:call-template name="russian_date">
												<xsl:with-param name="dateIn" select="acdt:TransportDoc/acdt:MDPNumber/acdt:RegistrationDate"/>
											</xsl:call-template>
										</xsl:if>
									</xsl:if>
									<xsl:if test="acdt:TransportDoc/acdt:CarnetNumber">Карнет АТА:<xsl:value-of select="acdt:TransportDoc/acdt:CarnetNumber"/>
									</xsl:if>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<xsl:if test="acdt:DeliveryRW">
									<tr>
										<td class="value">
											<span class="normal">
												<xsl:value-of select="acdt:DeliveryRW/acdt:StationName"/>
											</span>
											<xsl:if test="acdt:DeliveryRW/acdt:StationName">
												<span class="normal"> (</span>
											</xsl:if>
											<span class="normal">
												<xsl:value-of select="acdt:DeliveryRW/acdt:StationCode"/>
											</span>
											<xsl:if test="acdt:DeliveryRW/acdt:StationName">
												<span class="normal">) </span>
											</xsl:if>
											<xsl:value-of select="acdt:DeliveryRW/acdt:RailwayCode"/>
										</td>
									</tr>
									<tr>
										<td align="center" class="graphLittle">
	...
	<xsl:template match="acdt:RegistrationNumber">
		<span class="value">№ <xsl:value-of select="cat_ru:CustomsCode"/>
		</span>
		<span class="normal">/</span>
		<span class="value">
			<xsl:call-template name="russian_date_gtd">
