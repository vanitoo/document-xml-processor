			...
			<body>
				<div class="page">
					...
					<xsl:call-template name="inlinetable">
						...
						<xsl:with-param name="description" select="'(наименование, идентификационный номер налогоплательщика, код причины постановки на учет, местонахождение организации или фамилия, имя, отчество (при наличии), идентификационный номер налогоплательщика (при наличии), адрес места жительства физического лица, передающего товары)'"/>
						<xsl:with-param name="width" select="'100%'"/>
						<xsl:with-param name="supIndexDescr">
							<xsl:if test="atti:DocKind=0">1</xsl:if>
						</xsl:with-param>
					</xsl:call-template>
					<xsl:if test="atti:DocKind=0">
						<table>
								...
								<tr>
									<td style="text-align: justify;">
										<xsl:call-template name="inlinetable">
											...
											<xsl:with-param name="data_align" select="'left'"/>
											<xsl:with-param name="description" select="'(наименование, идентификационный номер налогоплательщика, код причины постановки на учет, местонахождение организации, или фамилия, имя, отчество (при наличии), идентификационный номер налогоплательщика
          (при наличии), адрес места жительства физического лица,
                          передающего товары)
'"/>
											<xsl:with-param name="width" select="'100%'"/>
											<xsl:with-param name="supIndexDescr" select="'5'"/>
										</xsl:call-template>
									</td>
								</tr>
							</tbody>
