...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:drp="urn:customs.ru:Information:CommercialFinanceDocuments:DecisionRevokePayments:5.16.0">
	<!-- 
...
-->
					...
					<tr class="delim_3"><td> </td></tr-->
							<tr align="center">
								<td>
									<b>
										<xsl:choose>
											<xsl:when test="drp:DocType = 'РПО'">Решение таможенного органа о приостановлении операций по счетам<br/>плательщика (лица, несущего солидарную обязанность) в банке и переводов его электронных денежных средств</xsl:when>
					...
					<table>
						<tbody align="center" class="graphMain">
							...
							<xsl:if test="drp:DocType = 'РОПО'">
								...
								<tr>
									<td class="border_bottom">
										<span class="normal">
									номер(а) счет(а/ов):
									<xsl:for-each select="drp:BankAndAccountInfo/drp:BankAccount">
												<xsl:value-of select="."/>
												<xsl:if test="position()!=last()">, </xsl:if>
											</xsl:for-each>
									; тип счет(а/ов): <xsl:apply-templates select="drp:BankAndAccountInfo/drp:BankAccountDescription"/>
							...
							<tr>
								<td class="border_bottom">
									<span class="normal">
										<xsl:apply-templates select="drp:BankAndAccountInfo/drp:BankName"/>
									</span>
									<xsl:if test="drp:BankAndAccountInfo/drp:BICID">
										<span class="normal">, БИК </span>
									</xsl:if>
									<span class="normal">
										<xsl:value-of select="drp:BankAndAccountInfo/drp:BICID"/>
									</span>
								</td>
							</tr>
							<tr class="graphLittle">
							...
							<tr class="graphLittle">
								<td align="center">
									<xsl:choose>
										<xsl:when test="drp:DocType = 'РПО'">(полное наименование организации или фамилия, имя, отчество (при наличии) плательщика (лица, несущего солидарную обязанность) индивидуального предпринимателя, идентификационный номер налогоплательщика)</xsl:when>
					...
					<table>
						<tr class="graphMain" align="center">
							...
							<td width="38%" class="border_bottom">
								<span class="normal">
									<xsl:value-of select="drp:CustomsChief/cat_ru:PersonSurname"/>&#160;
									<xsl:value-of select="drp:CustomsChief/cat_ru:PersonName"/>&#160;
									<xsl:if test="drp:CustomsChief/cat_ru:PersonMiddleName">
										<xsl:value-of select="drp:CustomsChief/cat_ru:PersonMiddleName"/>&#160;
									</xsl:if>
									<xsl:if test="drp:CustomsChief/drp:LNP">
										(ЛНП&#160;
										<xsl:value-of select="drp:CustomsChief/drp:LNP"/>)
									</xsl:if>
									<br/>
								</span>
							</td>
	...
	<xsl:template match="drp:Executor">
		<span class="normal">
			...
			<xsl:value-of select="drp:PersonName"/>
			<xsl:if test="drp:PersonMiddleName">
				&#160;<xsl:value-of select="drp:PersonMiddleName"/>
			</xsl:if>
