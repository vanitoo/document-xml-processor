						...
						<tbody class="graphMain">
							...
							<tr>
								<td class="value">
									...
									<xsl:if test="tdl:TransportDoc/tdl:MDPNumber"> Книжка МДП: <xsl:value-of select="tdl:TransportDoc/tdl:MDPNumber/tdl:TIRSeries"/>  
										<xsl:text> </xsl:text>
										<xsl:value-of select="tdl:TransportDoc/tdl:MDPNumber/tdl:TIRID"/>
										<xsl:if test="tdl:TransportDoc/tdl:MDPNumber/tdl:RegistrationDate"> от <xsl:call-template name="russian_date">
												<xsl:with-param name="dateIn" select="tdl:TransportDoc/tdl:MDPNumber/tdl:RegistrationDate"/>
											</xsl:call-template>
										</xsl:if>
									</xsl:if>
									<xsl:if test="tdl:TransportDoc/tdl:CarnetNumber">Карнет АТА:<xsl:value-of select="tdl:TransportDoc/tdl:CarnetNumber"/>
									</xsl:if>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td class="value">
									<span>
									   ...
									   <xsl:value-of select="substring(tdl:ProlongDate/tdl:DateLimit, 1, 4)"/>г.
									</span>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<xsl:if test="tdl:ProvidedDocuments">
									...
									<tr>
										<td class="value">
											<xsl:if test="tdl:UIN">
												УИН описи: <xsl:apply-templates select="tdl:UIN"/><xsl:if test="tdl:ProvidedDocuments">; <br/></xsl:if>
											</xsl:if>
