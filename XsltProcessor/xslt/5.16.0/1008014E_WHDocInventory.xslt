...
<xsl:stylesheet exclude-result-prefixes="whdi cat_ru catWH_ru" version="1.0" xmlns:catWH_ru="urn:customs.ru:Information:WarehouseDocuments:WarehouseCommonAggregateTypesCust:5.16.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:whdi="urn:customs.ru:Information:WarehouseDocuments:WHDocInventory:5.16.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0">
	...
	<xsl:template match="whdi:Carrier | whdi:Participant">
		<xsl:value-of select="cat_ru:OrganizationName"/>
		<xsl:if test="cat_ru:ShortName">
		...
		<xsl:if test="whdi:Ambassador">
			<br/>
		Представитель лица: 
		<xsl:value-of select="whdi:Ambassador/cat_ru:PersonSurname"/>
			<xsl:text> </xsl:text>
			<xsl:value-of select="whdi:Ambassador/cat_ru:PersonName"/>
			<xsl:text> </xsl:text>
			<xsl:value-of select="whdi:Ambassador/cat_ru:PersonMiddleName"/>
			<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
			<xsl:value-of select="whdi:Ambassador/cat_ru:PersonPost"/>
		</xsl:if>
