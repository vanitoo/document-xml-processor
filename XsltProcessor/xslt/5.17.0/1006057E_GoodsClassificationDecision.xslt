...
-->
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.17.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.17.0" xmlns:gcd="urn:customs.ru:Information:CustomsDocuments:GoodsClassificationDecision:5.17.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
						...
						13. Приложение к решению о внесении изменений в приложение к решению о классификации товара (перечень компонентов товара):</span>
								<span class="graphValue">
									<xsl:for-each select="gcd:Application">
										...
										<xsl:choose>
											<xsl:when test="gcd:ChangeAction='1'">
												<xsl:text> Дополнение компонентами товара</xsl:text>
											</xsl:when>
											<xsl:when test="gcd:ChangeAction='2'">
												<xsl:text> Изменение компонентов товара </xsl:text>
											</xsl:when>
											<xsl:when test="gcd:ChangeAction='3'">
												<xsl:text> Исключение компонентов товара</xsl:text>
											</xsl:when>
										</xsl:choose>
										<xsl:if test="gcd:Number|gcd:ComponentListPages|gcd:ComponentListCopies">
											<xsl:text>: приложение </xsl:text>
										</xsl:if>
										<xsl:if test="gcd:Number">
											<xsl:text> №</xsl:text>
											<xsl:apply-templates select="gcd:Number"/>
										</xsl:if>
										<xsl:if test="gcd:ComponentListPages">
								На <xsl:value-of select="gcd:ComponentListPages"/> л.
							</xsl:if>
										<xsl:if test="gcd:ComponentListCopies">
											<xsl:text> в </xsl:text>
											<xsl:value-of select="gcd:ComponentListCopies"/> экз.</xsl:if>
									</xsl:for-each>
									<br/>
