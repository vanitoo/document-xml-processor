...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exs="urn:customs.ru:Information:CustomsDocuments:ExpertSummary:5.17.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:CategoryCust="urn:customs.ru:Categories:3.0.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:sig="http://www.w3.org/2000/09/xmldsig#">
	...
	<xsl:template match="exs:ExpertSummary">
		...
		<html>
			<head>
				<xsl:if test="(string-length($jsPath) &gt;= 1)">
					<xsl:element name="script">
						<xsl:attribute name="type"><xsl:text>text/javascript</xsl:text></xsl:attribute>
						<xsl:attribute name="src"><xsl:value-of select="$jsPath"/><xsl:text>signature.js</xsl:text></xsl:attribute>
					</xsl:element>
				</xsl:if>
			...
			<body>
				...
				<xsl:if test="exs:CustomsControl">
					...
					<div class="page graphMain">
						...
						<xsl:choose>
							<xsl:when test="string-length($signatureData) &gt;= 1">
								<xsl:call-template name="work_with_signatureData">
									<xsl:with-param name="fullstr" select="$signatureData"/>
									<xsl:with-param name="str" select="''"/>
									<xsl:with-param name="i" select="0"/>
								</xsl:call-template>
							</xsl:when>
							<xsl:when test="(string-length($signatureHtml) &gt;= 1) and (string-length($signatureData) &lt; 1)">
								<div id="result" style="border: solid 0px"/>
								<script type="text/javascript">
									<xsl:text>document.getElementById('result').innerHTML = '</xsl:text>
									<xsl:value-of select="$signatureHtml"/>
									<xsl:text>';</xsl:text>
								</script>
							</xsl:when>
							<xsl:when test="(string-length($jsPath) &gt;= 1) and (string-length($signatureData) &lt; 1) and (string-length($signatureHtml) &lt; 1) and (../../sig:KeyInfo/sig:X509Data/sig:X509Certificate or sig:Signature/sig:KeyInfo/sig:X509Data/sig:X509Certificate)">
								<div style="height:130px;">
									<xsl:for-each select="../../sig:KeyInfo/sig:X509Data/sig:X509Certificate[generate-id(key('rows_X509Certificate',.)[1])=generate-id()]|sig:Signature/sig:KeyInfo/sig:X509Data/sig:X509Certificate[generate-id(key('rows_X509Certificate',.)[1])=generate-id()]">
										<div style="margin-left:3px;margin-bottom:3px;float: right; width:400px; height:130px; color: rgb(0,102,255); border: 3px rgb(0,102,255) solid; -moz-border-radius: 15px; -webkit-border-radius: 15px; -khtml-border-radius:15px; border-radius: 15px;">
											<table style="border: 0; table-layout: fixed; cellpadding: 3; cellspacing: 0; width: 100%; border-collapse: collapse; font-family: Arial; font-size: 7pt; color: rgb(0,102,255); font-weight: bold;">
												<xsl:attribute name="id"><xsl:text>result</xsl:text><xsl:value-of select="position()"/></xsl:attribute>
												<tbody>
													<tr>
														<td width="5%"/>
														<td width="18%"/>
													...
													<tr>
														<td/>
														<td style="padding-top: 6px;font-family: Arial;font-size: 10pt;font-weight: bold;color: rgb(0,102,255);text-align:center;vertical-align:middle;">
															<img width="50" height="50" src="signature.jpg"/>
														</td>
														<td style="padding-top: 6px;font-family: Arial;font-size: 10pt;font-weight: bold;color: rgb(0,102,255);text-align:center;vertical-align:middle;">
															<xsl:text>ДОКУМЕНТ ПОДПИСАН ЭЛЕКТРОННОЙ ПОДПИСЬЮ</xsl:text>
														</td>
														<td/>
													</tr>
													<tr>
														<td/>
														<td colspan="2" style="padding-top: 3px;padding-bottom: 3px; font-family: Arial; font-size: 7pt; background:rgb(0,102,255); color:#ffffff;font-weight: bold;text-align:center;vertical-align:middle;">
															<xsl:text>СВЕДЕНИЯ О СЕРТИФИКАТЕ ЭП</xsl:text>
														</td>
														<td/>
													</tr>
													<tr>
														<td/>
														<td colspan="2" style="padding-top: 6px;font-family: Arial; font-size: 7pt; color: rgb(0,102,255); font-weight: bold;"/>
														<td/>
													</tr>
													<tr>
														<td/>
													...
													<tr>
														<td/>
														<td colspan="2" style="padding-bottom: 8px;font-family: Arial; font-size: 7pt; color: rgb(0,102,255); font-weight: bold;"/>
														<td/>
													</tr>
												</tbody>
											</table>
										...
										<script type="text/javascript">
											<xsl:text>parseCert('</xsl:text>
											<xsl:value-of select="."/>
											<xsl:text>', document.getElementById('result</xsl:text>
							...
							<tbody>
								<tr>
									<td style="text-align: justify;">
										...
										<xsl:if test="count(exs:CustomsPerson)&gt;1">ы</xsl:if>
										<xsl:text> отдела </xsl:text>
										<xsl:apply-templates select="exs:CEKTUDivision/exs:Name"/>
										<xsl:text/>
										<xsl:apply-templates select="exs:EKSName"/>
										<xsl:text/>
										<xsl:for-each select="exs:CustomsPerson">
										 ...
										 на основании письма <xsl:apply-templates select="exs:OperationalSearchActivities/exs:Initiator"/> от 
										<xsl:apply-templates select="exs:OperationalSearchActivities/exs:DecisionDate" mode="russian_date"/>
										<!--<xsl:call-template name="russian_date">
											<xsl:with-param name="dateIn" select="exs:OperationalSearchActivities/exs:DecisionDate"/>
										</xsl:call-template>-->
										№ <xsl:apply-templates select="exs:OperationalSearchActivities/exs:DecisionNumber"/> , на основании письма № 
										<xsl:apply-templates select="exs:OperationalSearchActivities/exs:NumberCheck"/>, 
										произвел <xsl:apply-templates select="exs:OperationalSearchActivities/exs:CustomExpertiseKind"/> исследование.<br/>
									</td>
								</tr>
								<tr>
									<td align="center">
							...
							<tbody>
								...
								<tr>
									<td>
										...
										<xsl:apply-templates select="exs:ExpertSummaryDate" mode="monthword_date"/>
										<!--<xsl:call-template name="monthword_date">
	...
	<xsl:template name="work_with_signatureData">
		<xsl:param name="fullstr"/>
		<xsl:param name="str" select="''"/>
		<xsl:param name="i" select="0"/>
		<xsl:if test="string-length($fullstr) &gt; 0">
			<xsl:choose>
				<xsl:when test="substring($fullstr,1,1)='{'">
					<xsl:call-template name="work_with_signatureData">
						<xsl:with-param name="fullstr" select="substring($fullstr,2)"/>
						<xsl:with-param name="str" select="''"/>
						<xsl:with-param name="i" select="1"/>
					</xsl:call-template>
				</xsl:when>
				<xsl:when test="substring($fullstr,1,1)='}'">
					<xsl:call-template name="ecp">
						<xsl:with-param name="str_ecp" select="$str"/>
					</xsl:call-template>
					<xsl:call-template name="work_with_signatureData">
						<xsl:with-param name="fullstr" select="substring($fullstr,2)"/>
						<xsl:with-param name="str" select="''"/>
				...
				<xsl:otherwise>
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:call-template name="work_with_signatureData">
								<xsl:with-param name="fullstr" select="substring($fullstr,2)"/>
								<xsl:with-param name="str" select="concat($str,substring($fullstr,1,1))"/>
								<xsl:with-param name="i" select="1"/>
							</xsl:call-template>
						</xsl:otherwise>
					</xsl:choose>
	...
	<xsl:template name="ecp">
		<xsl:param name="str_ecp"/>
		<xsl:variable name="clPicture">
			<xsl:choose>
				<xsl:when test="substring($Picture,string-length($Picture)-1,1) = '&quot;'">
					<xsl:value-of select="substring($Picture,1,string-length($Picture)-1)"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="vPicture">
			<xsl:choose>
				<xsl:when test="$Picture">
					<xsl:variable name="lower">pngjef</xsl:variable>
					<xsl:variable name="upper">PNGJEF</xsl:variable>
					<xsl:variable name="f_ex4">
						<xsl:value-of select="translate(substring($clPicture,string-length($clPicture)-3,4),$upper,$lower)"/>
					</xsl:variable>
					<xsl:variable name="f_ex5">
						<xsl:value-of select="translate(substring($clPicture,string-length($clPicture)-4,5),$upper,$lower)"/>
					</xsl:variable>
					<xsl:choose>
						<xsl:when test="$f_ex4='.jpg' or $f_ex5='.jpeg' or $f_ex4='.png' or $f_ex4='.gif'">
							<xsl:value-of select="$Picture"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat('data:image/jpg;base64,',$Picture)"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
			</xsl:choose>
		...
		<xsl:variable name="CertID">
			<xsl:choose>
				<xsl:when test="string-length(substring-after($str_ecp,'&quot;CertID&quot;:&quot;')) &gt; 0">
					<xsl:value-of select="substring-before(substring-after($str_ecp,'&quot;CertID&quot;:&quot;'),'&quot;')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<xsl:variable name="Name">
			<xsl:choose>
				<xsl:when test="string-length(substring-after($str_ecp,'&quot;Name&quot;:&quot;')) &gt; 0">
					<xsl:value-of select="substring-before(substring-after($str_ecp,'&quot;Name&quot;:&quot;'),'&quot;')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<xsl:variable name="StartDate">
			<xsl:choose>
				<xsl:when test="string-length(substring-after($str_ecp,'&quot;StartDate&quot;:&quot;')) &gt; 0">
					<xsl:value-of select="substring-before(substring-after($str_ecp,'&quot;StartDate&quot;:&quot;'),'&quot;')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<xsl:variable name="EndDate">
			<xsl:choose>
				<xsl:when test="string-length(substring-after($str_ecp,'&quot;EndDate&quot;:&quot;')) &gt; 0">
					<xsl:value-of select="substring-before(substring-after($str_ecp,'&quot;EndDate&quot;:&quot;'),'&quot;')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
							...
							<table>
								<tbody>
									<tr>
										<td width="4%"/>
										<xsl:choose>
											<xsl:when test="string-length($vPicture) &gt; 1">
												<td width="18%" style="padding-top: 3px;font-family: Arial;font-size: 10pt;font-weight: bold;color: rgb(0,102,255);text-align:center;vertical-align:middle;">
													<img width="50" height="50" src="{$vPicture}"/>
												</td>
												<td width="74%" style="padding-top: 3px;font-family: Arial;font-size: 10pt;font-weight: bold;color: rgb(0,102,255);text-align:center;vertical-align:middle;word-break: break-word;word-wrap: break-word;">
													<xsl:text>ДОКУМЕНТ ПОДПИСАН ЭЛЕКТРОННОЙ ПОДПИСЬЮ</xsl:text>
												</td>
											</xsl:when>
											<xsl:otherwise>
												<td width="92%" colspan="2" style="padding-top: 3px;font-family: Arial;font-size: 10pt;font-weight: bold;color: rgb(0,102,255);text-align:center;vertical-align:middle;word-break: normal; overflow-wrap: break-word; word-wrap: break-word;">
													<xsl:text>ДОКУМЕНТ ПОДПИСАН ЭЛЕКТРОННОЙ ПОДПИСЬЮ</xsl:text>
												</td>
											</xsl:otherwise>
										</xsl:choose>
									...
									<tr>
										...
										<td colspan="2" style="padding-top: 3px;font-family: Arial; font-size: 7pt; color: rgb(0,102,255); font-weight: bold;">
											<xsl:if test="string-length($CertID) &gt; 0">
