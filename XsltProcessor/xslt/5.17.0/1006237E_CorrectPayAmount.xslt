...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cpa="urn:customs.ru:Information:CustomsDocuments:CorrectPayAmount:5.16.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
					...
					<table class="w190">
						<tr>
							...
							<td class="value graphMain">
								<xsl:apply-templates select="cpa:RegNumberDT"/>
							</td>
						</tr>
						<xsl:if test="cpa:ProcedureID">
							<tr>
								...
								<td class="value graphMain">
									<xsl:apply-templates select="cpa:ProcedureID"/>
								</td>
							</tr>
						</xsl:if>
						<tr>
							...
							<td class="value graphMain">
								<xsl:apply-templates select="cpa:RegistrationNumber"/>
							</td>
						</tr>
						<xsl:if test="cpa:Comments">
							<tr>
								...
								<td class="value graphMain">
									<xsl:apply-templates select="cpa:Comments"/>
								</td>
							</tr>
						</xsl:if>
						<tr>
							...
							<td class="value graphMain">
								<xsl:apply-templates select="cpa:Customs"/>
							</td>
						</tr>
					</table>
						...
						<xsl:for-each select="cpa:Goods">
							<tr>
								<td class="annot graphMain" style="border-bottom:1px solid black" colspan="9">
									<div class="title marg-top">Товар № <xsl:value-of select="position()"/>
									</div>
								</td>
							</tr>
	...
	<xsl:template match="RUScat_ru:IdentityCard">
		...
		<xsl:if test="RUScat_ru:IdentityCardSeries"> серия: <xsl:apply-templates select="RUScat_ru:IdentityCardSeries"/>
		</xsl:if>
	...
	<xsl:template match="cpa:Goods">
		...
		<xsl:if test="cpa:GoodsDescription">
			<tr>
				<td class="annot graphMain" colspan="3">
					<xsl:apply-templates select="cpa:GoodsDescription"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:if test="cpa:TNVEDCode or cpa:GoodsNumeric or cpa:GoodsAddTNVEDCode">
			<tr>
				<td class="annot graphMain" colspan="3">
					<i>
						<div class="marg-top">Сведения из ДТ</div>
					</i>
				</td>
			</tr>
			<xsl:if test="cpa:GoodsNumeric">
				<tr>
					...
					<td class="value graphMain">
						<xsl:apply-templates select="cpa:GoodsNumeric"/>
					</td>
				</tr>
			</xsl:if>
			<xsl:if test="cpa:TNVEDCode">
				<tr>
					...
					<td class="value graphMain">
						<xsl:apply-templates select="cpa:TNVEDCode"/>
					</td>
				</tr>
			</xsl:if>
			<xsl:if test="cpa:GoodsAddTNVEDCode">
				<tr>
					...
					<td class="value graphMain">
						<xsl:apply-templates select="cpa:GoodsAddTNVEDCode"/>
					</td>
				</tr>
			</xsl:if>
		</xsl:if>
		<xsl:if test="cpa:TNVEDCodeKDT or cpa:KDTGoodsNumeric or cpa:GoodsAddTNVEDCodeKDT">
			<tr>
				<td class="annot graphMain" colspan="3">
					<i>
						<div class="marg-top">Сведения из КДТ</div>
					</i>
				</td>
			</tr>
			<xsl:if test="cpa:KDTGoodsNumeric">
				<tr>
					...
					<td class="annot graphMain" style="width:38%">Номер товара</td>
					<td class="value graphMain">
						<xsl:apply-templates select="cpa:KDTGoodsNumeric"/>
					</td>
				</tr>
			</xsl:if>
			<xsl:if test="cpa:TNVEDCodeKDT">
				<tr>
					...
					<td class="annot graphMain" style="width:38%">Код товара по ТН ВЭД ЕАЭС</td>
					<td class="value graphMain">
						<xsl:apply-templates select="cpa:TNVEDCodeKDT"/>
					</td>
				</tr>
			</xsl:if>
			<xsl:if test="cpa:GoodsAddTNVEDCodeKDT">
				<tr>
					...
					<td class="annot graphMain" style="width:38%">Код товара по классификатору доп.тамож.информации</td>
					<td class="value graphMain">
						<xsl:apply-templates select="cpa:GoodsAddTNVEDCodeKDT"/>
					</td>
				</tr>
			</xsl:if>
		</xsl:if>
		<xsl:if test="cpa:IsPartialGood or cpa:EffectCode or cpa:NoTariffCode">
			<tr>
				<td class="annot graphMain" colspan="3">
					<i>
						<div class="marg-top">Решение по товарам</div>
					</i>
				</td>
			</tr>
				...
				<tr>
					...
					<td class="value graphMain">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:apply-templates select="cpa:EffectCode"/>
							</xsl:otherwise>
						</xsl:choose>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="value graphMain">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:apply-templates select="cpa:NoTariffCode"/>
							</xsl:otherwise>
						</xsl:choose>
					</td>
				</tr>
		...
		<xsl:if test="cpa:Declarant">
			<tr>
				<td class="annot graphMain" colspan="3">
					<i>
						<div class="marg-top">Декларант</div>
					</i>
				</td>
			</tr>
			<tr>
				...
				<td class="annot graphMain" colspan="2">
					<xsl:apply-templates select="cpa:Declarant"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:if test="$kol_str &gt; 0">
			<tr>
				<td class="annot graphMain" colspan="3">
					<i>
						<div class="marg-top">Таможенные платежи</div>
					</i>
				</td>
			</tr>
			<tr>
				<td class="annot graphMain" colspan="3">
					<table class="w190">
						...
						<xsl:if test="cpa:TotalPayCustCost">
							<tr>
								...
								<td class="graphMain bordered" style="font-size:11pt">
									<xsl:value-of select="translate(cpa:TotalPayCustCost, '.', ',')"/>
								</td>
							</tr>
						</xsl:if>
						<xsl:if test="cpa:TotalClassGood">
							<tr>
								<td class="graphMain bordered" align="center">
									<xsl:value-of select="$kol1+$kol2"/>.</td>
								...
								<td class="graphMain bordered" style="font-size:11pt">
									<xsl:value-of select="translate(cpa:TotalClassGood, '.', ',')"/>
								</td>
							</tr>
						</xsl:if>
	...
	<xsl:template match="cpa:RegistrationNumber">
		...
		<xsl:apply-templates select="cpa:CustomsCode"/>
		<xsl:text>-</xsl:text>
		<xsl:value-of select="substring(cpa:Date,3,2)"/>
		<xsl:text>/</xsl:text>
		<xsl:apply-templates select="cpa:SerialNumber"/>
		<xsl:text>И</xsl:text>
	</xsl:template>
	<xsl:template match="//*[local-name()='CorrectPayAmount']//*" priority="-1">
