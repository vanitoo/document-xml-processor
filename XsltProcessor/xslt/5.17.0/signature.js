...
function parseDate(str)
{
	...
	var m = parseInt(str.substring(5, 2));
	var d = parseInt(str.substring(8, 2));
	/*var h = parseInt(str.substring(11, 2));
	var mm = parseInt(str.substring(14, 2));
	var ss = parseInt(str.substring(17, 2));*/
	return new Date(y, m, d/*, h, m, s*/);
	//0123456789012345678
	//2019-06-25 11:12:31
}

function parseASN1(asn1, passId)
{
	if (asn1.sub == null)
	{
		return;
  ...
  if (asn1.sub.length == 2 && asn1.sub[0].tag.tagNumber == 23 && asn1.sub[1].tag.tagNumber == 23)
  {
    certData.dateFrom = parseDate(asn1.sub[0].content());
    certData.dateTo = parseDate(asn1.sub[1].content());
  }

	if (asn1.sub.length == 2 && asn1.sub[0].tag.tagNumber == 6 && asn1.sub[1].sub == null)
	{
		if (passId == null)
		{
			var name = asn1.sub[0].content().split('\n');
			if (name.length == 3)
			{
				var val = asn1.sub[1].content();
				if (name[0].substr(0, 6) == "2.5.4.")
				{
					if (isIssuer && certData.issuer[name[1]] != undefined)
					{
						isIssuer = false;
					}
					if (isIssuer)
					{
						certData.issuer[name[1]] = val;
					}
					else
					{
						certData.subject[name[1]] = val;
					}
				}
				else
				{
		...
		/*else
		{
			if (certData[passId] == undefined)
			{
				certData[passId] = {};
			}
			certData[passId][asn1.sub[0].content()] = asn1.sub[1].content();
		}*/
		return;
	}

	var objId = passId;
	if (asn1.sub.length > 0 && asn1.sub[0].tag.tagNumber == 6)
	{
		objId = asn1.sub[0].content();
	}
	
	for (var i = 0; i < asn1.sub.length; i++)
	{
		parseASN1(asn1.sub[i], objId);
	}
}
...
window.parseCert = function (certArr, el)
{
  var b = Base64.unarmor(certArr);
  var asn1 = ASN1.decode(b, 0);
  certData = { issuer: {}, subject: {} };
	parseASN1(asn1);
	//el.tBodies[0].rows[3].cells[1].innerText = 'Сертификат: ' + '000000';
	el.tBodies[0].rows[4].cells[1].innerText = 'Владелец: ' + certData.subject.surname + ' ' + certData.subject.givenName;
	el.tBodies[0].rows[5].cells[1].innerText = 'Действителен: с ' + dateToStr(certData.dateFrom) + ' по ' + dateToStr(certData.dateTo);
};
