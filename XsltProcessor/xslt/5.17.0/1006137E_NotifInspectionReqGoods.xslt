...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:ntfin="urn:customs.ru:Information:CustomsDocuments:NotifInspectionReqGoods:5.17.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.17.0">
			...
			<xsl:when test="$DocID/ntfin:DTMDocDetails">
				<xsl:for-each select="$DocID/ntfin:DTMDocDetails">
					<xsl:call-template name="gdt_number">
						...
						<xsl:with-param name="date" select="cat_ru:RegistrationDate"/>
						<xsl:with-param name="number" select="cat_ru:GTDNumber"/>
						<xsl:with-param name="add_number" select="catESAD_cu:TransportModeCode"/>
					</xsl:call-template>
				</xsl:for-each>
			</xsl:when>
						...
						<tbody>
							<xsl:if test="ntfin:DocumentKind='0' or ntfin:DocumentKind='7'">
								...
								<tr>
									<td align="left">
										<xsl:choose>
											<xsl:when test="ntfin:DocumentKind='0' and $form='осмотр'">
												<xsl:text>  В соответствии с пунктом 3 статьи 327 Таможенного кодекса ЕАЭС уведомляем, что в ходе таможенного контроля в отношении товаров, сведения о которых содержатся в таможенной декларации № </xsl:text>
											</xsl:when>
											<xsl:when test="ntfin:DocumentKind='0' and $form!='осмотр'">
												<xsl:text>  В соответствии с пунктом 3 статьи 328 Таможенного кодекса ЕАЭС уведомляем, что в ходе таможенного контроля в отношении товаров, сведения о которых содержатся в таможенной декларации № </xsl:text>
											</xsl:when>
											<xsl:when test="ntfin:DocumentKind='7'">
												<xsl:text>  В соответствии с пунктом 3 статьи 328 Таможенного кодекса ЕАЭС уведомляем, что в ходе таможенного контроля в отношении товаров, сведения о которых содержатся в заявлении о выпуске до подачи ДТ № </xsl:text>
											</xsl:when>
										</xsl:choose>
										<span class="bordered">
									...
									, иных случаях  <span class="bordered">
												<xsl:apply-templates select="ntfin:Otherwise"/>
											</span> 
								</xsl:if>
										<br/>принято решение о проведении таможенного <xsl:value-of select="$form"/>а.
							</td>
								</tr>
							</xsl:if>
							<xsl:if test="ntfin:DocumentKind='1' or ntfin:DocumentKind='8'">
								...
								<tr>
									<td align="left">
										<xsl:choose>
											<xsl:when test="ntfin:DocumentKind='1' and $form='осмотр'">
										<xsl:text>  В связи с необходимостью проведения таможенного контроля в форме таможенного осмотра на основании статьи 348 ТК ЕАЭС требуется предъявление товаров, совершение таможенных операций  ДТ&#160;№&#160;</xsl:text>
											</xsl:when>
										</xsl:choose>
										<span class="bordered">
											<xsl:call-template name="DocID">
										...
										, иных случаях  <span class="bordered">
											...
											<br/>с целью проведения таможенного <xsl:value-of select="$form"/>а.
								</xsl:if>
									</td>
								</tr>
							</xsl:if>
										...
										<tbody>
											...
											Взятие проб и образцов товаров и (или) транспортных средств состоится:
												...
												<td class="bordered" style="width:40mm">
													<xsl:apply-templates mode="russian_time" select="ntfin:InspTime"/>  <xsl:apply-templates mode="russian_date" select="ntfin:InspDate"/>
												</td>
											</tr>
										</tbody>
									</table>
									...
									<table align="left" style="width:210mm">
										<tbody>
											<tr>
												...
												<td class="bordered" style="width:40mm" align="right">
													<!--<xsl:apply-templates select="ntfin:CustomsPerson/cat_ru:LNP"/> (ЛНП)-->
												</td>
												<td style="width:0mm">
													<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text>
												</td>
											...
											<tr>
												...
												<td class="graphLittle" style="width:40mm">(подпись)</td>
												<td style="width:0mm">
													<!--xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text-->
												</td>
											</tr>
										...
										<tbody>
											<tr>
												<td class="bordered" style="width:100mm">
													...
													<xsl:if test="ntfin:DirectionNotifReq/ntfin:Person/cat_ru:PersonName"><xsl:text>. </xsl:text></xsl:if>
													<xsl:value-of select="substring(ntfin:DirectionNotifReq/ntfin:Person/cat_ru:PersonMiddleName,1,1)"/>
													<xsl:if test="ntfin:DirectionNotifReq/ntfin:Person/cat_ru:PersonMiddleName"><xsl:text>. </xsl:text></xsl:if>
												</td>
												<td style="width:10mm">
													<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text>
												</td>
