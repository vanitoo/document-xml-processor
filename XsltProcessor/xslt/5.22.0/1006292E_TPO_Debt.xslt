...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.17.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.22.0" xmlns:tpodbt="urn:customs.ru:Information:CustomsDocuments:TPO_Debt:5.22.0">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
					...
					<xsl:if test="tpodbt:FixRate">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Фиксированная ставка платежа в рублях</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="tpodbt:FixRate" mode="translate_number"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					...
					<xsl:if test="tpodbt:TPOTypeCode">
						<div class="title marg-top">Тип ордера</div>
						<div>
							<xsl:apply-templates select="tpodbt:TPOTypeCode">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="tpodbt:PaymentsForPeriod">
						<div class="title marg-top">Перечень платежей</div>
						<table>
							<tbody>
								<xsl:for-each select="tpodbt:PaymentsForPeriod">
									<tr>
										<td class="bordered">
											<xsl:apply-templates select="tpodbt:PaymentModeCode"/>
										</td>
										<td class="bordered">
											<xsl:apply-templates select="tpodbt:TaxBase"/>
										</td>
									</tr>
								</xsl:for-each>
							</tbody>
					...
					<xsl:if test="tpodbt:Periods">
						<div class="title marg-top">Период(ы) начисления платежей (пени, отсрочка или рассрочка платежей, временное хранение товаров и т.п.)</div>
						<table>
							<tbody>
								<xsl:for-each select="tpodbt:Periods">
									<tr>
										<td class="bordered">
											<xsl:apply-templates select="tpodbt:PeriodNumber"/>
										</td>
										<td class="bordered">
											<xsl:apply-templates select="tpodbt:CBRate"/>
										</td>
										<td class="bordered">
											<xsl:apply-templates select="tpodbt:Term/tpodbt:BeginDate" mode="russian_date"/>
										</td>
										<td class="bordered">
											<xsl:apply-templates select="tpodbt:Term/tpodbt:EndDate" mode="russian_date"/>
										</td>
									</tr>
								</xsl:for-each>
							</tbody>
					...
					<xsl:if test="tpodbt:PresentedDocument">
						<div class="title marg-top">Представленные документы</div>
						<table>
							<tbody>
								<xsl:for-each select="tpodbt:PresentedDocument">
									<tr>
										<td class="bordered">
	...
	<xsl:template match="tpodbt:AutomobileInfo">
		...
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="tpodbt:CRMoveDirectCode">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Код направления перемещения</td>
						<td class="value graphMain" style="width:50%">
							<xsl:choose>
								<xsl:when test="tpodbt:CRMoveDirectCode='1'">1 – ввоз</xsl:when>
								<xsl:when test="tpodbt:CRMoveDirectCode='2'">2 – вывоз</xsl:when>
								<xsl:when test="tpodbt:CRMoveDirectCode='3'">3 – временный ввоз</xsl:when>
								<xsl:when test="tpodbt:CRMoveDirectCode='4'">4 – обратный вывоз</xsl:when>
							</xsl:choose>
						</td>
					</tr>
				</table>
	...
	<xsl:template match="tpodbt:TPOTypeCode">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<xsl:if test="tpodbt:MovementPurpose">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Код цели перемещения товара</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="tpodbt:MovementPurpose"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="tpodbt:MovementWay">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Код способа перемещения товара</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="tpodbt:MovementWay"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="tpodbt:TransportModeCode"/>
					</td>
				</tr>
			</table>
		</div>
		...
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:call-template name="russian_date_time">
							<xsl:with-param name="dateIn" select="tpodbt:TPODateTime"/>
						</xsl:call-template>
					</td>
				</tr>
			</table>
