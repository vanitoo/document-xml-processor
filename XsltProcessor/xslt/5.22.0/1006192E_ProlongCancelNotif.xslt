...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:prcn="urn:customs.ru:Information:CustomsDocuments:ProlongCancelNotif:5.22.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.22.0">
   ...
   <xsl:template match="prcn:ProlongCancelNotif">
	   <xsl:variable name="title">
		   <xsl:choose>
				...
				<xsl:when test="prcn:DocSign='3'">Решение о продлении срока проверки</xsl:when>
				<xsl:when test="prcn:DocSign='4'">Решение о продлении срока проведения таможенного досмотра</xsl:when>
            ...
            <div class="page">
               <table>
				...
				<xsl:for-each select="prcn:Reasons">
                  ...
                  <xsl:if test="prcn:DocSign='3' or prcn:DocSign='4'">
                     <tr>
				...
				<xsl:value-of select="prcn:ApplicationRegNumber/prcn:CustomsCode"/>/<xsl:call-template name="russian_date_gtd">
                           <xsl:with-param name="dateIn" select="prcn:ApplicationRegNumber/prcn:RegistrationDate"/>
							...
							<xsl:if test="prcn:ApplicationRegNumber/prcn:AddNumber">
                        /<xsl:value-of select="prcn:ApplicationRegNumber/prcn:AddNumber"/>
                        </xsl:if>
                     </td>
                  </tr>
				...
				<xsl:call-template name="russian_date">
						   ...
						   <xsl:text> </xsl:text>
						   <xsl:value-of select="substring(prcn:ProlongDate, 12, 5)"/>
                        </td>
                     </tr>
                  </xsl:if>
