...
<xsl:stylesheet exclude-result-prefixes="math" version="1.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.22.0" xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.17.0" xmlns:math="http://exslt.org/math" xmlns:sez_rf4="urn:customs.ru:Information:CustomsDocuments:SEZReportForm4:5.22.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   <xsl:output encoding="utf-8" indent="yes" method="html" version="1.0"/>
   <!-- Шаблон для типа SEZReportForm4Type -->
   <xsl:template match="sez_rf4:SEZReportForm4">
	<html>
		<head>
            <meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/>
            <title>Формы отчетности о товарах, помещенных под таможенную процедуру свободной таможенной зоны, и товарах, изготовленных (полученных) из товаров, помещенных под таможенную процедуру свободной таможенной зоны. Форма №4</title>
            <style type="text/css">
				body
				{text-align: center;}
		...
		<body>
            <div>
					...
					<tbody>
						...
						<tr>
							<td align="center"><b><xsl:text>ОТЧЕТНОСТЬ</xsl:text><br/>
								...
								<xsl:text>таможенной зоны,</xsl:text><br/>
								<xsl:text>по состоянию на </xsl:text>
								<xsl:call-template name="DateStr">
									<xsl:with-param name="date" select="sez_rf4:ReportDate"/>
								</xsl:call-template>
								<xsl:text> на </xsl:text>
								<u><xsl:apply-templates select="sez_rf4:SheetsNumber"/></u>
						   ...
						   <td align="center">
								...
								<xsl:call-template name="inlinetable">
									<xsl:with-param name="data">
										<xsl:apply-templates select="sez_rf4:Organization/cat_ru:OrganizationName"/>
										<xsl:text>, ОГРН - </xsl:text>
										<xsl:apply-templates select="sez_rf4:Organization/cat_ru:RFOrganizationFeatures/cat_ru:OGRN"/>
										<xsl:text>,</xsl:text> 
									</xsl:with-param>
						   ...
						   <td align="center">
								<xsl:call-template name="inlinetable">
									<xsl:with-param name="data">
										...
										<xsl:for-each select="sez_rf4:Organization/sez_rf4:Address">
						...
						<tr>
							<td align="center">
								...
								<xsl:call-template name="inlinetable">
									<xsl:with-param name="data">
										<xsl:apply-templates select="sez_rf4:ReportPerson" mode="fio"/>
										<xsl:text>, дата - </xsl:text>
										<xsl:apply-templates select="sez_rf4:ReportPerson/cat_ru:IssueDate" mode="russian_date"/>
									</xsl:with-param>
									<xsl:with-param name="data_align" select="'center'"/>
				...
				<br/>
				<table border="1" style="width:180mm">
					<tbody>
						<tr>
							<td>№ п/п</td>
						...
						<xsl:for-each select="sez_rf4:Goods">
							<tr>
								<td>
									<xsl:value-of select="sez_rf4:Number"/>
								</td>
								<td>
									<xsl:value-of select="sez_rf4:Name"/>
								</td>
								<td>
									<xsl:value-of select="sez_rf4:TNVEDCode"/>
								</td>
								<td>
									<xsl:value-of select="sez_rf4:Quantity"/>
								</td>
								<td>
									<xsl:value-of select="sez_rf4:MeasureUnitQualifierName"/>
									<xsl:text> (</xsl:text>
									<xsl:value-of select="sez_rf4:MeasureUnitQualifierCode"/>
									<xsl:text>)</xsl:text>
								</td>
							</tr>
						</xsl:for-each>
	...
	<xsl:template match="//*[local-name()='SEZReportForm4']//*" priority="-1">
		<xsl:variable name="xpath">
			<xsl:call-template name="get_xpath">
   ...
   <xsl:template name="russian_date_gtd">
	...
	<xsl:template match="sez_rf4:SEZGoodsDisposal/sez_rf4:GTDID">
