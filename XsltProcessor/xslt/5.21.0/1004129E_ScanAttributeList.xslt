...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.21.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:sal="urn:customs.ru:Information:CustomsDocuments:ScanAttributeList:5.21.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:template match="sal:DocList">
		...
		<xsl:if test="sal:CheckList">
			<table class="w190">
				<tr>
					<td class="annot graphMain" style="width:50%">Лист проверки документа</td>
					<td class="value graphMain" style="width:50%">
	...
	<xsl:template match="sal:CheckList">
		...
		<xsl:text>, обязательность провеки: </xsl:text>
		<xsl:choose>
			<xsl:when test="sal:Obligation=1">да</xsl:when>
			<xsl:when test="sal:Obligation=2">нет</xsl:when>
			<xsl:when test="sal:Obligation=3">обязательна для определенных видов документов</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="sal:Obligation"/>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:text>, результат проверки: </xsl:text>
		<xsl:choose>
			<xsl:when test="sal:Result = 'true' or sal:Result = 1">пройдена</xsl:when>
			<xsl:when test="sal:Result = 'false' or sal:Result = 0">не пройдена</xsl:when>
			<xsl:otherwise>
