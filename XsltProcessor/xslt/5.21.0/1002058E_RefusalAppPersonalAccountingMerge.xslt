...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.21.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.21.0" xmlns:rapam="urn:customs.ru:Information:CommercialFinanceDocuments:RefusalAppPersonalAccountingMerge:5.21.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Уведомление об отказе в создании (объединении) лицевых счетов по обращению</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="rapam:RegistrationReqDoc">
					...
					<xsl:if test="rapam:ApplicantInfo">
						<div class="title marg-top">Сведения о заявителе</div>
						<div>
							<xsl:apply-templates select="rapam:ApplicantInfo">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Причины невозможности произвести создание (объединение) лицевых счетов</td>
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="rapam:ImpossibilityReasons" />
							</td>
						</tr>
					</table>
					<xsl:if test="rapam:CustomsPerson">
						<div class="title marg-top">Дожностное лицо таможенного органа</div>
						<table class="bordered">
							<tbody>
								<tr class="title">
									...
									<td class="graphMain bordered">Дата подписания</td>
								</tr>
								<xsl:for-each select="rapam:CustomsPerson">
  ...
  <xsl:template match="rapam:ApplicantInfo">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="RUScat_ru:OrganizationName">
  ...
  <xsl:template match="rapam:CustomsPerson">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <tr>
