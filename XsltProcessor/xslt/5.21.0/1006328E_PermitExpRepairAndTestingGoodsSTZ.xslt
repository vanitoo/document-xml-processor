...
<xsl:stylesheet version="1.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.21.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.21.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.21.0" xmlns:pertg_stz="urn:customs.ru:Information:CustomsDocuments:PermitExpRepairAndTestingGoodsSTZ:5.21.0" xmlns:catProc_ru="urn:customs.ru:Information:ProcessingDocuments:ProcessingCommonAggregateTypes:5.21.0">
	...
	<xsl:template match="pertg_stz:PermitExpRepairAndTestingGoodsSTZ">
		<html>
			<head>
				<style>
                  body {
						...
						<tbody>
							...
							<tr>
								<td align="center">
									...
									<xsl:value-of select="pertg_stz:PermissionRegNumber/pertg_stz:CustomsCode"/>
									<xsl:text>/</xsl:text>
									<xsl:apply-templates mode="gtd_date" select="pertg_stz:PermissionRegNumber/pertg_stz:RegistrationDate"/>
									<xsl:text>/</xsl:text>
									<xsl:value-of select="pertg_stz:PermissionRegNumber/pertg_stz:Number"/>
									<xsl:text>/РВР</xsl:text>
								</td>
							</tr>
							<tr>
								<td>
							...
							<tr>
								<td style="text-align: justify">
									<xsl:text>Настоящее разрешение свидетельствует о возможности вывоза с территории участка резидента Арктической зоны Российской Федерации </xsl:text>
									<xsl:call-template name="inlinetable">
										<xsl:with-param name="data">
											<xsl:value-of select="pertg_stz:ResidentFromPlace/cat_ru:OrganizationName"/>
										</xsl:with-param>
										<xsl:with-param name="data_align" select="'center'"/>
										<xsl:with-param name="description" select="'(наименование резидента Арктической зоны Российской Федерации)'"/>
										<xsl:with-param name="width" select="'70mm'"/>
									</xsl:call-template>
									<xsl:text> на остальную часть таможенной территории Евразийского экономического союза товаров по заявлению № </xsl:text>
									<xsl:call-template name="inlinetable">
										<xsl:with-param name="data">
											<xsl:value-of select="pertg_stz:AppRegNumber/pertg_stz:CustomsCode"/>
											<xsl:text>/</xsl:text>
											<xsl:apply-templates mode="gtd_date" select="pertg_stz:AppRegNumber/pertg_stz:RegistrationDate"/>
											<xsl:text>/</xsl:text>
											<xsl:value-of select="pertg_stz:AppRegNumber/pertg_stz:Number"/>
									...
									<xsl:text> на основании части 3 статьи 20 Федерального закона от 13
июля 2020 г. N 193-ФЗ "О государственной поддержке предпринимательской деятельности в Арктической зоне Российской Федерации" для целей:</xsl:text>
								</td>
							</tr>
							<tr>
								<td class="delim_3"/>
							...
							<tr>
								<td>
									<xsl:call-template name="inlinetable">
										<xsl:with-param name="data">
											<xsl:apply-templates select="pertg_stz:RepairAndTestingOperations"/>
										</xsl:with-param>
									...
									<xsl:call-template name="inlinetable">
										<xsl:with-param name="data">
											<xsl:for-each select="pertg_stz:RepairAndTestingPerson">
									...
									<xsl:call-template name="inlinetable">
										<xsl:with-param name="data">
											<xsl:apply-templates select="pertg_stz:RepairAndTestingAddress" mode="ru_address"/>
										</xsl:with-param>
										<xsl:with-param name="data_align" select="'left'"/>
										<xsl:with-param name="description" select="'(указывается операция, для проведения которой товары временно вывозятся, лицо, которое будет осуществлять такие операции, и место их проведения)'"/>
										<xsl:with-param name="width" select="'100%'"/>
									</xsl:call-template>
								</td>
							</tr>
								...
								<td>
									<table>
										<tbody>
											...
											<tr>
												<td class="graphMain">
													<xsl:text>Срок обратного ввоза товаров:</xsl:text>
												</td>
												<td class="value">
													<xsl:call-template name="russian_date">
														<xsl:with-param name="dateIn" select="pertg_stz:EntryGoodsTerm"/>
													</xsl:call-template>
												</td>
											</tr>
											<tr>
												<td class="graphMain">
													<xsl:text>Срок обратного ввоза товаров продлен до:</xsl:text>
												</td>
												<td class="value">
													<xsl:call-template name="russian_date">
														<xsl:with-param name="dateIn" select="pertg_stz:ProlongTerm"/>
													</xsl:call-template>
													<xsl:text> (</xsl:text>
													<xsl:value-of select="pertg_stz:DecisionProlongTerm/pertg_stz:CustomsCode"/>
													<xsl:text>/</xsl:text>
													<xsl:apply-templates mode="gtd_date" select="pertg_stz:DecisionProlongTerm/pertg_stz:RegistrationDate"/>
													<xsl:text>/</xsl:text>
													<xsl:value-of select="pertg_stz:DecisionProlongTerm/pertg_stz:Number"/>
													<xsl:text>/РВРПС)</xsl:text>
												</td>
											</tr>
										</tbody>
									</table>
										...
										<tbody>
											<tr valign="bottom">
												...
												<td align="center" class="value" width="30%">
													<xsl:apply-templates mode="PersonBase" select="pertg_stz:CustomsSignature"/>
												</td>
											</tr>
											<tr>
	...
	<xsl:template mode="organization" match="*">
		...
		<xsl:if test="pertg_stz:ResidentCertificate">
			<xsl:text> свидетельство  </xsl:text>
			<xsl:for-each select="pertg_stz:ResidentCertificate">
