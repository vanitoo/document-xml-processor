...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.21.0" xmlns:nwi="urn:customs.ru:Information:CommercialFinanceDocuments:NoticeWayInteraction:5.21.0">
              ...
              <tr>
                ...
                <td class="value graphMain" style="width:50%">
                  <xsl:value-of select="cat_ru:RefDocumentID" />
					...
					<div class="title marg-top">Реквизиты банка</div>
					<div>
						<xsl:apply-templates select="nwi:BankInfo">
							...
							<td class="value graphMain" style="width:50%">
								<xsl:choose>
									...
									<xsl:when test="nwi:WayKind=ЛК">
										<xsl:text>обмен через Личный кабинет</xsl:text>
									</xsl:when>
									<xsl:otherwise>
	...
	<xsl:template match="nwi:BankInfo">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
