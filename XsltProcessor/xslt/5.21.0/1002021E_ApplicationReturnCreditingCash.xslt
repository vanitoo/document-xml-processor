...
<xsl:stylesheet exclude-result-prefixes="cat_ru arcc" version="1.0" xmlns:arcc="urn:customs.ru:Information:CommercialFinanceDocuments:ApplicationReturnCreditingCash:5.21.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.21.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.21.0">
	...
	<xsl:template match="*" mode="Address">
		...
		<xsl:if test="*[local-name() = 'CounryName']">
			<xsl:value-of select="*[local-name() = 'CounryName']"/>, 
		</xsl:if>
		<xsl:if test="*[local-name() = 'Region']">
			<xsl:value-of select="*[local-name() = 'Region']"/>, 
		</xsl:if>
		<xsl:if test="*[local-name() = 'City']">
			<xsl:value-of select="*[local-name() = 'City']"/>, 
		</xsl:if>
		<xsl:if test="*[local-name() = 'StreetHouse']">
			<xsl:value-of select="*[local-name() = 'StreetHouse']"/>
		</xsl:if-->
		<xsl:choose>
