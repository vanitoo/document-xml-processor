...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:apam="urn:customs.ru:Information:CommercialFinanceDocuments:ApplicationPersonalAccountingMerge:5.21.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.21.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.21.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.21.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <html>
      ...
      <body>
        <div class="page" style="width: {$w}mm;">
            ...
            <tbody>
              <tr>
                <td align="center" class="title">
                  <font size="4">
                    <b>Обращение правопреемника о создании (объединении) лицевого счета</b>
                  </font>
                </td>
              </tr>
            </tbody>
          ...
          <xsl:if test="apam:RegistrationReqDoc">
            <div class="title marg-top">Регистрационный номер и дата обращения</div>
            <div>
              <xsl:apply-templates select="apam:RegistrationReqDoc">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          <xsl:if test="apam:Customs">
            <div class="title marg-top">Таможенный орган, в который подается обращение</div>
            <div>
          ...
          <xsl:if test="apam:ApplicantInfo">
            <div class="title marg-top">Сведения о заявителе</div>
            <div>
              <xsl:apply-templates select="apam:ApplicantInfo">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          <xsl:if test="apam:ReorgInfo">
            <div class="title marg-top">Сведения о реорганизованном юридическом лице</div>
            <div>
              <xsl:apply-templates select="apam:ReorgInfo">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          <table>
            <tr>
              <td class="annot graphMain" style="width:50%">Форма реорганизации</td>
              <td class="value graphMain" style="width:50%">
                <xsl:choose>
                  <xsl:when test="apam:ReorgForm='0'">
                    <xsl:text>слияние</xsl:text>
                  </xsl:when>
                  <xsl:when test="apam:ReorgForm='1'">
                    <xsl:text>присоединение</xsl:text>
                  </xsl:when>
                  <xsl:when test="apam:ReorgForm='2'">
                    <xsl:text>преобразование</xsl:text>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="apam:ReorgForm" />
                  </xsl:otherwise>
                </xsl:choose>
              </td>
            </tr>
          </table>
          <xsl:if test="apam:AttachedDocuments">
            ...
            <table class="bordered">
              <tbody>
                ...
                <xsl:for-each select="apam:AttachedDocuments">
          ...
          <xsl:if test="apam:ApplicantSignature">
            <div class="title marg-top">Подпись заявителя</div>
            <div>
              <xsl:apply-templates select="apam:ApplicantSignature">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
  ...
  <xsl:template match="apam:ApplicantInfo">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="RUScat_ru:OrganizationName">
  ...
  <xsl:template match="apam:ApplicantSignature">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
  ...
  <!-- Шаблон для типа apam:AttachedDocumentsType -->
  <xsl:template match="apam:AttachedDocuments">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <tr>
        ...
        <td class="graphMain bordered">
          <xsl:apply-templates select="apam:DocCode" />
        </td>
        <td class="graphMain bordered">
          <xsl:if test="apam:DocArchIdDetails">
            <xsl:apply-templates select="apam:DocArchIdDetails">
              <xsl:with-param name="wdth" select="$w" />
            </xsl:apply-templates>
          </xsl:if>
        </td>
  ...
  <xsl:template match="apam:DocArchIdDetails">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <xsl:template match="apam:RegistrationReqDoc">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <!-- Шаблон для типа RUScat_ru:RUOrganizationType -->
  <xsl:template match="apam:ReorgInfo">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="RUScat_ru:OrganizationName">
