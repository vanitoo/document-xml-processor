...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:lpid_req="urn:customs.ru:Information:CustomsDocuments:LongProcRequirementAppIdentify:5.24.0">
	<!-- Шаблон для типа LongProcRequirementAppIdentifyType -->
	<xsl:template match="lpid_req:LongProcRequirementAppIdentify">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Требование о представлении Заявления о проведении идентификации товаров</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="lpid_req:Customs">
						<div class="title marg-top">Таможенный орган</div>
						<div>
							<xsl:apply-templates select="lpid_req:Customs">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="lpid_req:DTNumber">
						...
						<div>
							<xsl:apply-templates select="lpid_req:DTNumber">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="lpid_req:GoodsNumeric">
						<table>
							<tr>
								...
								<td class="value graphMain" style="width:50%">
									<xsl:for-each select="lpid_req:GoodsNumeric">
					...
					<xsl:if test="lpid_req:CustomsPerson">
						<div class="title marg-top">Подпись должностного лица</div>
						<div>
							<xsl:apply-templates select="lpid_req:CustomsPerson">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<xsl:template match="lpid_req:Customs">
	...
	<!-- Шаблон для типа lpid_req:CustomsPersonSignatureType -->
	<xsl:template match="lpid_req:CustomsPerson">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
			...
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="lpid_req:IssueDate" mode="russian_date" />
					</td>
				</tr>
			</table>
		</div>
	...
	<xsl:template match="lpid_req:DTNumber">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
