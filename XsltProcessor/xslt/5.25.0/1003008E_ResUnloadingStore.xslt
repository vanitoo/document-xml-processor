...
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:res_us="urn:customs.ru:Information:TransportDocuments:ResUnloadingStore:5.24.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	<!-- Шаблон для типа ResUnloadingStoreType -->
	<xsl:variable name="part1" select="res_us:ResUnloadingStore[res_us:StoresInfo/res_us:DecisionKind = 1]"/>
	<xsl:variable name="part2" select="res_us:ResUnloadingStore[res_us:StoresInfo/res_us:DecisionKind = 2]"/>
...
законодательные акты Российской Федерации» принято решение в отношении припасов, находящихся на (в) </xsl:text>
					...
					<xsl:call-template name="inlinetable">
						<xsl:with-param name="data">
							<xsl:for-each select="$part2/res_us:StoresInfo/res_us:Reason">
								<xsl:choose>
						...
						<tbody>
							<tr>
								<td width="20%" valign="bottom">
									<xsl:choose>
										<xsl:when test="res_us:CustomsChief/res_us:CustomsPersonKind='1'">Начальник</xsl:when>
										<xsl:when test="res_us:CustomsChief/res_us:CustomsPersonKind='2'">Заместитель начальника</xsl:when>
										<xsl:otherwise>Начальник<br/>(заместитель начальника)</xsl:otherwise>
									</xsl:choose>
								</td>
							...
							<tr>
								<td colspan="6" align="left">
									<xsl:call-template name="russian_date_month"><xsl:with-param name="dateIn" select="res_us:CustomsChief/res_us:IssueDate"/></xsl:call-template>								
								</td>
							</tr>
						</tbody>
					</table>
...
<xsl:template match="res_us:AircraftInformation">
	...
	<xsl:template match="*" mode="organization">
		<xsl:choose>
			<xsl:when test="res_us:SubjectKind='1'">
				<xsl:text>российское юридическое лицо: </xsl:text>
			</xsl:when>
			<xsl:when test="res_us:SubjectKind='2'">
				<xsl:text>иностранное юридическое лицо: </xsl:text>
			</xsl:when>
			<xsl:when test="res_us:SubjectKind='3'">
				<xsl:text>физическое лицо: </xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="res_us:SubjectKind"/>
			</xsl:otherwise>
		</xsl:choose>
