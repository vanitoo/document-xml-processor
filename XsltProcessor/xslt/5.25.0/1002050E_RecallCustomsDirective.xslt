...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:rcd="urn:customs.ru:Information:CommercialFinanceDocuments:RecallCustomsDirective:5.24.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
		...
		<html>
			...
			<body>
                ...
                <xsl:choose>
												...
												<xsl:when test="rcd:OrderType='8'">
													<xsl:text>поручения на продажу драгоценных металлов и перечисление денежных средств от продажи драгоценных металлов на расчетный (текущий) счет плательщика (лица, несущего солидарную обязанность)</xsl:text>
												</xsl:when>
												<xsl:when test="rcd:OrderType='9'">
													<xsl:text>уведомления об отказе в замене ареста имущества на денежный залог</xsl:text>
												</xsl:when>
												<xsl:when test="rcd:OrderType='10'">
													<xsl:text>решения о замене ареста имущества на денежный залог</xsl:text>
												</xsl:when>
												<xsl:when test="rcd:OrderType='11'">
													<xsl:text>решения таможенного органа об обращении взыскания на товары</xsl:text>
												</xsl:when>
												<xsl:otherwise>
	...
	<xsl:template mode="organization" match="*">
		...
		<xsl:if test="rcd:OKPOID">,&#032;ОКПО:&#032;<xsl:value-of select="rcd:OKPOID"/>
		</xsl:if>
		<xsl:if test="rcd:ITN">,&#032;ИТН:&#032;<xsl:value-of select="rcd:ITN"/>
		</xsl:if>
		<xsl:if test="rcd:Address">,&#032;Адрес:&#032;<xsl:for-each select="rcd:Address">
				<xsl:for-each select="*">
					<xsl:value-of select="."/>
		...
		<xsl:if test="rcd:OKATOCode">,&#032;ОКАТО:&#032;<xsl:value-of select="rcd:OKATOCode"/>
		</xsl:if>
		<xsl:if test="rcd:Contact">,&#032;<xsl:for-each select="rcd:Contact">
