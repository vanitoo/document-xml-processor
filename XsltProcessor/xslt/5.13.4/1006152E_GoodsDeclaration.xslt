...
<xsl:stylesheet version="1.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.12.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:gode="urn:customs.ru:Information:CustomsDocuments:GoodsDeclaration:5.12.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                  ...
                  <tbody class="goods">
                     <tr>
                        ...
                        <xsl:if test="gode:GoodsList/gode:GoodsPackaging">
                           ...
                           <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:RBCargoKind">
                              <td width="3%">Вид грузовых мест</td>
                           </xsl:if>
                           <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PackageCode">
                              <td width="3%">Код вида груза</td>
                           </xsl:if>
                           <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PackingInformation">
                              <td width="3%">Код вида упаковки товаров</td>
                              <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PackingInformation/catESAD_cu:PakingQuantity">
                                 <td width="3%">Количество упаковок</td>
                              </xsl:if>
                           </xsl:if>
                           <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PalleteInformation">
                              <td width="3%">Количество поддонов</td>
                              <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PalleteInformation/catESAD_cu:PalleteDescription">
                                 <td width="10%">Описание поддонов</td>
                              </xsl:if>
                              <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PalleteInformation/catESAD_cu:PalleteCode">
                                 <td width="3%">Код вида поддона</td>
                              </xsl:if>
                           </xsl:if>
                           <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:UnitPackInfo">
                              <td width="3%">Код вида индивидуальной упаковки товаров</td>
                              <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:UnitPackInfo/catESAD_cu:PakingQuantity">
                                 <td width="5%">Количество индивидуальных упаковок</td>
                              </xsl:if>
                           </xsl:if>
                           <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:CargoInfo">
                              <td width="3%">Код вида упаковки товаров (при перевозе без упаковки)</td>
                              <xsl:if test="gode:GoodsList/gode:GoodsPackaging/catESAD_cu:CargoInfo/catESAD_cu:PakingQuantity">
                     ...
                     <xsl:apply-templates select="gode:GoodsList"/>
                  </tbody>
               </table>
            </div>
         </body>
   ...
   <xsl:template match="gode:GoodsList">
      <tr>
         ...
         <xsl:if test="../gode:GoodsList/gode:GoodsPackaging">
            ...
            <xsl:if test="../gode:GoodsList/gode:GoodsPackaging/catESAD_cu:RBCargoKind">
               <td>
                  <xsl:value-of select="gode:GoodsPackaging/catESAD_cu:RBCargoKind"/>
               </td>
            </xsl:if>
            <xsl:if test="../gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PackageCode">
               <td>
                  <xsl:value-of select="gode:GoodsPackaging/catESAD_cu:PackageCode"/>
               </td>
            </xsl:if>
            <xsl:if test="../gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PackingInformation">
               <xsl:choose>
                  <xsl:when test="gode:GoodsPackaging/catESAD_cu:PackingInformation">
                     <td>
                        <xsl:for-each select="gode:GoodsPackaging/catESAD_cu:PackingInformation">
            ...
            <xsl:if test="../gode:GoodsList/gode:GoodsPackaging/catESAD_cu:PalleteInformation">
               <td colspan="3">
                  <table border="1" style="border:0; border-collapse:collapse; margin: -5px;">
                     <tbody>
                        <xsl:for-each select="gode:GoodsPackaging/catESAD_cu:PalleteInformation">
                           <tr>
                              <xsl:choose>
            ...
            <xsl:if test="../gode:GoodsList/gode:GoodsPackaging/catESAD_cu:UnitPackInfo">
               <xsl:choose>
                  <xsl:when test="gode:GoodsPackaging/catESAD_cu:UnitPackInfo">
                     <td>
                        <xsl:for-each select="gode:GoodsPackaging/catESAD_cu:UnitPackInfo">
                           <xsl:value-of select="catESAD_cu:PackingCode"/>
                           <br/>
                           <br/>
                        </xsl:for-each>
            ...
            <xsl:if test="../gode:GoodsList/gode:GoodsPackaging/catESAD_cu:CargoInfo">
               <xsl:choose>
                  <xsl:when test="gode:GoodsPackaging/catESAD_cu:CargoInfo">
                     <td>
                        <xsl:for-each select="gode:GoodsPackaging/catESAD_cu:CargoInfo">
                           <xsl:value-of select="catESAD_cu:PackingCode"/>
                           <br/>
                           <br/>
                        </xsl:for-each>
