						...
						<tbody>
							...
							<tr>
								<td>
									...
									<xsl:if test="rrsd:ResolutionInfo/adt_cat:Date">
										<xsl:text> от </xsl:text>
										<u><xsl:call-template name="russian_date_month"><xsl:with-param name="dateIn" select="rrsd:ResolutionInfo/adt_cat:Date"/></xsl:call-template></u>
									</xsl:if>
							...
							<tr>
								<td>
									...
									<xsl:call-template name="inlinetable">
										<xsl:with-param name="data">
											<xsl:apply-templates mode="person_nopost" select="rrsd:SecondExempReceived"/>
											<xsl:text> </xsl:text>
											<xsl:apply-templates mode="identity" select="rrsd:SecondExempReceived/adt_cat:Passport"/>
											<xsl:text> </xsl:text>
											<xsl:apply-templates mode="document" select="rrsd:SecondExempReceived/adt_cat:AuthoritesDocument"/>
										</xsl:with-param>
										<xsl:with-param name="data_align" select="'left'"/>
							...
							<tr>
								<td>
									<xsl:apply-templates mode="signature2ndCopy" select="rrsd:CustomResolution/adt_cat:ApproverPerson"/>
								</td>
							</tr>
							<tr>
								<td>
