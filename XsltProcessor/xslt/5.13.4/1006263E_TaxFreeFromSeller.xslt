...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:tfr_sl="urn:customs.ru:Information:CustomsDocuments:TaxFreeFromSeller:5.13.4">
    ...
    <html>
      ...
      <body>
        <div class="page">
          ...
          <xsl:if test="tfr_sl:TaxFreeOperator">
            <div class="title marg-top" style="padding-left:{$level*10}px">Сведения об операторе TaxFree</div>
            <div style="padding-left:{$level*10}px">
              <xsl:apply-templates select="tfr_sl:TaxFreeOperator">
                <xsl:with-param name="level" select="$level+1" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          <xsl:if test="tfr_sl:CashVoucherNumber">
            <table>
              <tr>
                <td class="annot graphMain" style="width:50%">Номер кассового чека</td>
                <td class="value graphMain" style="width:50%">
          ...
          <xsl:if test="tfr_sl:CashVoucherDate">
            <table>
              <tr>
                <td class="annot graphMain" style="width:50%">Дата кассового чека</td>
                <td class="value graphMain" style="width:50%">
                  <xsl:call-template name="russian_date">
          ...
          <table class="bordered">
            <tbody>
              <tr class="title">
                ...
                <td class="graphMain bordered">Стоимость товара</td>
              </tr>
  ...
  <xsl:template match="tfr_sl:TaxFreeOperator">
    <xsl:param name="level" select="0" />
    <xsl:if test="cat_ru:OrganizationName">
      <table>
        <tr>
  ...
  <xsl:template match="tfr_sl:TaxFreeSeller">
    ...
    <xsl:if test="cat_ru:IdentityCard">
      <div class="title marg-top" style="padding-left:{$level*10}px">Документ, удостоверяющий личность</div>
      <div style="padding-left:{$level*10}px">
    ...
    <xsl:if test="cat_ru:Contact">
      <div class="title marg-top" style="padding-left:{$level*10}px">Контактная информация</div>
      <div style="padding-left:{$level*10}px">
    ...
    <xsl:if test="tfr_sl:BranchDescription">
      <div class="title marg-top" style="padding-left:{$level*10}px">Сведения об обособленном подразделении</div>
      <div style="padding-left:{$level*10}px">
        <xsl:apply-templates select="tfr_sl:BranchDescription">
          <xsl:with-param name="level" select="$level+1" />
        </xsl:apply-templates>
      </div>
    </xsl:if>
