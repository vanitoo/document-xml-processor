...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exs="urn:customs.ru:Information:CustomsDocuments:ExpertSummary:5.13.4" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:CategoryCust="urn:customs.ru:Categories:3.0.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:template match="exs:ExpertSummary">
		...
		<html>
			...
			<body>
<!-- ========================================================================= -->
				<xsl:if test="exs:CustomsControl"> <!-- Экспертиза в рамках ТК -->
					<!--ТК-->
					<div class="page">
						<table>
							<tbody>
							...
							<tbody>
								<tr>
									<td>
										...
										<xsl:text>, в связи с решением о назначении таможенной экспертизы № </xsl:text><xsl:value-of select="exs:CustomsControl/exs:DecisionNumber/cat_ru:CustomsCode"/>/<xsl:call-template name="number_date"><xsl:with-param name="dateIn" select="exs:CustomsControl/exs:DecisionNumber/cat_ru:RegistrationDate"/></xsl:call-template>/<xsl:value-of select="exs:CustomsControl/exs:DecisionNumber/exs:ExpertisePoint"/>/<xsl:value-of select="exs:CustomsControl/exs:DecisionNumber/cat_ru:SerialNumber"/><xsl:text> от </xsl:text>
								...
								по документам: <xsl:if test="exs:PresentedDocuments/exs:GTDNumber">ДТ № <xsl:apply-templates select="exs:PresentedDocuments/exs:GTDNumber"/>
								 ...
								 руководителем Центрального экспертно-криминалистического таможенного управления уведомлен<xsl:if test="count(exs:CustomsPerson)>1">ы</xsl:if> о правах и обязанностях таможенного эксперта, предусмотренных статьей <xsl:value-of select="exs:CustomsControl/exs:ArcticleNumberRigth"/>
								 <xsl:choose>
										<xsl:when test="number(translate(exs:ExpertSummaryDate,'-','')) &gt;= number('20180101')"> ТК ЕАЭС</xsl:when>
										<xsl:otherwise> Таможенного кодекса Таможенного союза</xsl:otherwise>
									</xsl:choose>.<br/>&#160;&#160;&#160;&#160;Об ответственности за дачу заведомо ложного заключения таможенного эксперта по статье <xsl:value-of select="exs:CustomsControl/exs:ArcticleNumber"/> Кодекса Российской Федерации об административных правонарушениях предупрежден<xsl:if test="count(exs:CustomsPerson)>1">ы</xsl:if>.</td>
								</tr>
							</tbody>
						</table>
						<table>
							<tbody>
								...
								<xsl:for-each select="exs:CustomsPerson">
									...
									<tr>
										...
										<td/>
										<td align="center">
											<br/><xsl:call-template name="monthword_date"><xsl:with-param name="dateIn" select="../exs:StudyStartDateTime"/></xsl:call-template></td>
										<td/>
									</tr>
								</xsl:for-each>
							</tbody>
							...
							<tbody>
										...
										<table>
											<tbody>
												<tr>
													<td>
														<xsl:text>№ </xsl:text>
														<xsl:value-of select="exs:ExpertSummaryNumber"/>
														<xsl:text> составлено </xsl:text>
														<xsl:call-template name="monthword_date">
													...
													<td align="right">
														<xsl:if test="substring(exs:ExpertSummaryCity,1,2) != 'г.' and substring(exs:ExpertSummaryCity,1,2) != 'Г.'">
															<xsl:text>г. </xsl:text>
														</xsl:if>
								...
								<tr>
									<td>&#160;&#160;&#160;&#160;
										...
										<xsl:text>на основании решения о назначении таможенной экспертизы  от </xsl:text>
										<xsl:call-template name="monthword_date">
											<xsl:with-param name="dateIn" select="exs:CustomsControl/exs:DecisionDate"/>
										</xsl:call-template>
										<xsl:text> г. </xsl:text>
										...
										<xsl:text> </xsl:text>
										<xsl:value-of select="exs:CustomsControl/exs:InitiatorPerson/cat_ru:PersonPost"/>
										<xsl:text>, произвел</xsl:text><xsl:if test="count(exs:CustomsPerson)>1">и </xsl:if><xsl:text> </xsl:text><xsl:value-of select="exs:CustomsControl/exs:CustomExpertiseKind"/><xsl:text> таможенную экспертизу  в период с </xsl:text>
								...
								<tr>
									<td align="center">
										<br/>
										<b>На таможенную экспертизу представлены:</b>
									</td>
								</tr>
								<tr>
									<td>
								...
								<tr>
									<td align="center">
										<br/>
										<b>Вопросы, поставленные на разрешение таможенного(ых) эксперта(ов):</b>
									</td>
								</tr>
								<tr>
									<td>
						...
						<table>
							<tbody>
								...
								<tr>
									<td>
										<xsl:for-each select="exs:CustomsPerson">
											<!--xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text--><xsl:if test="$hasCounter &gt; 1">1.<xsl:value-of select="position()"/><xsl:text>. </xsl:text></xsl:if>
											...
											<tbody>
												<xsl:for-each select="exs:CustomsPerson">
													<xsl:call-template name="CustomsPerson">
														...
														<xsl:with-param name="title_position" select="'Таможенный эксперт'"/>
								...
								<tr>
									<td>
										<xsl:for-each select="exs:CustomsPerson">
											<!--xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text--><xsl:if test="$hasCounter &gt; 1">2.<xsl:value-of select="position()"/><xsl:text>. </xsl:text></xsl:if>
								...
								<xsl:if test="exs:Synthesis">
									...
									<tr>
										<td>
											<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text><xsl:apply-templates select="exs:Synthesis"/>
										</td>
									</tr>
									<tr>
										<td>
...
<!-- ========================================================================= -->
				<xsl:if test="exs:AdministrativeOffence"> <!-- Экспертиза в рамках АП -->
					<div class="page">
						<table>
							<tbody>
								<tr>
						...
						<table>
							<tbody>
								<tr>
									<td><br/>
										<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;На основании определения о назначении криминалистической экспертизы от </xsl:text>
										<xsl:call-template name="monthword_date">
										...
										<xsl:text> </xsl:text>
										<xsl:value-of select="exs:AdministrativeOffence/exs:InitiatorPerson/cat_ru:PersonPost"/>
...
<!-- ========================================================================= -->
				<xsl:if test="exs:CriminalCase"> <!--Экспертиза в рамка УД-->
					<div class="page">
						<table>
							<tbody>
								<tr>
							...
							<tbody>
								<tr>
									<td>
										...
										<br/>&#160;&#160;&#160;&#160;В соответствии со ст. 14 Федерального закона от 31.05.2001 № 73-ФЗ “О государственной судебно-экспертной деятельности в Российской Федерации” (далее - ФЗ ГСЭД) руководителем <xsl:value-of select="exs:CriminalCase/exs:EKSName"/> разъяснены обязанности и права эксперта, предусмотренные ст. <xsl:value-of select="exs:CriminalCase/exs:ArcticleNumberRigth"/> УПК России.
				...
				<xsl:if test="exs:CustomsControlStudy"> <!--Данные к заключению специалиста по исследованию, назначенному при таможенном контроле-->
					<div class="page">
						<table>
							<tbody>
								<tr>
							...
							<tbody>
								<tr>
									<td>
										...
										№ <xsl:value-of select="exs:CustomsControlStudy/exs:DecisionNumber"/> (вх. № ЦЭКТУ <xsl:value-of select="exs:CustomsControlStudy/exs:LetterNumber"/> от 
										<xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="exs:CustomsControlStudy/exs:LetterDate"/></xsl:call-template>, по вопросу определения рыночной стоимости товара в 
										<xsl:value-of select="exs:CEKTUDivision/exs:Name"/>) представлены:
									</td>
								</tr>
								<tr>
									<td>
								...
								<tr>
									<td>
										<xsl:for-each select="exs:CustomsPerson">
											<!--xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text--><xsl:apply-templates select="exs:Study"/>
											<xsl:if test="position()!=last()"><br/></xsl:if>
										</xsl:for-each>
									</td>
								</tr>
...
<!-- ========================================================================= -->
				<xsl:if test="exs:OperationalSearchActivities"> <!-- Данные к заключению специалиста по исследованию в рамках оперативно-розыскных мероприятий -->
					<div class="page">
						<table>
							<tbody>
								<tr>
							...
							<tbody>
								<tr>
									<td>
										...
										<xsl:text> отдела криминалистических исследований Криминалистической службы Центрального экспертно-криминалистического таможенного управления </xsl:text>
										<xsl:for-each select="exs:CustomsPerson">
											<xsl:value-of select="concat(substring(cat_ru:PersonName,1,1),'.',substring(cat_ru:PersonMiddleName,1,1),'. ',cat_ru:PersonSurname)"/>
											<xsl:text>, </xsl:text>
											<xsl:value-of select="cat_ru:PersonPost"/>
										...
										№ <xsl:value-of select="exs:OperationalSearchActivities/exs:DecisionNumber"/> "О проведении криминалистического исследования", в рамках оперативно-розыскных мероприятий по материалу проверки № 
										<xsl:value-of select="exs:OperationalSearchActivities/exs:NumberCheck"/> (вх. ЦЭКТУ <xsl:value-of select="exs:OperationalSearchActivities/exs:LetterNumber"/> от <xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="exs:OperationalSearchActivities/exs:LetterDate"/></xsl:call-template>), 
...
<!-- ========================================================================= -->
				<xsl:if test="exs:ContinuousExpertSupport"> <!-- Данные к заключению специалиста по исследованию в рамках непрерывного экспертного обеспечения -->
					<div class="page">
						<table>
							<tbody>
								<tr>
	...
	<xsl:template match="exs:Conclusion">
		<tr>
			...
			<td>
				<xsl:for-each select="exs:ConclusionText">
	...
	<xsl:template match="exs:Qualification">
        ...
        специальность - <xsl:value-of select="exs:Specialization"/>, 
	...
	<xsl:template match="exs:Question">
		<tr valign="top">
			<td width="5%">
				<!--xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text--><xsl:value-of select="exs:QuestionNumber"/>.
			</td>
			<td>
