...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:mpco="urn:customs.ru:Information:CustomsDocuments:MPOCommon:5.24.0" xmlns:mprk="urn:customs.ru:Information:CustomsDocuments:MPOReestrMark:5.24.0">
	...
	<xsl:template match="mpco:CustomsPerson">
		<xsl:for-each select="*">
			<xsl:if test="local-name() = 'PersonPost'">, </xsl:if>
	...
	<xsl:template match="mpco:MPO_Registration">
		<xsl:apply-templates select="." mode="MPO_num"/>
	</xsl:template>
	<!-- Шаблон для типа mpco:MPO_RegistrationType -->
	...
	<xsl:template match="*" mode="MPO_num">
		...
		<tr>
			...
			<td class="graphMain bordered">
				<xsl:choose xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
					...
					<xsl:when test="mpco:DecisionCode=50">
						<xsl:text>МПО безвозвратно утеряна</xsl:text>
					</xsl:when>
					<xsl:when test="mpco:DecisionCode=51">
						<xsl:text>отказ в признании МПО безвозвратно утерянной</xsl:text>
					</xsl:when>
					<xsl:otherwise>
