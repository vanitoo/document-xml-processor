...
<xsl:stylesheet version="1.0" 
	...
	xmlns:exnot="urn:customs.ru:Information:CustomsDocuments:OEZ_ExportNotif:5.24.0" 
				...
				<div class="page">
					...
					<xsl:if test="exnot:DocSign='0' or exnot:DocSign='1' or exnot:DocSign='2'">
						<table style="width:297mm">
							<tbody>
								...
								<xsl:if test="exnot:CombinedCargo">
									<tr>
										<td align="left">
											<br/>
											<span class="incline">Сборный груз </span><u>
												<xsl:choose>
													<xsl:when test="exnot:CombinedCargo='1'">вывоз товаров в составе сборного груза</xsl:when>
					...
					<xsl:if test="exnot:DocSign='3' or exnot:DocSign='4'">
						...
						<table style="width:297mm">
							<tbody>
								<tr>
								   <td align="left"><br/>Прошу провести идентификацию в отношении следующих товаров при их вывозе с территории участка 
										<xsl:call-template name="inlinetable">
											<xsl:with-param name="data">
												<xsl:for-each select="exnot:GoodsInfo[exnot:IdentificationSign='1' or exnot:IdentificationSign='true' or exnot:IdentificationSign='t']">
													<xsl:number value="position()"/>
													<xsl:if test="position()!=last()">, </xsl:if>
												</xsl:for-each>
											</xsl:with-param>
											<xsl:with-param name="data_align" select="'center'"/>
											<xsl:with-param name="description" select="'(номер товара из графы № 1 Таблицы Сведения о вывозимых товарах)'"/>
