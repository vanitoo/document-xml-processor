...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:err="urn:customs.ru:Information:ExchangeDocuments:ErrorList:5.15.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="/">
		<html>
			<head>
	...
	<xsl:template match="err:ErrorList">
		...
		<xsl:if test="err:CheckVersion">
			...
			<xsl:value-of select="err:CheckVersion"/>
		</xsl:if>
		<br/>
		<br/>
	...
	<xsl:template match="err:BadDocument">
		...
		<tr>
			...
			<td style="width:95%; border-bottom: 1px solid black">
				<span>ID: </span>
				<xsl:value-of select="err:DocID"/>
				<br/>
				<xsl:if test="err:DocName">
					<span>Название: </span>
					<xsl:value-of select="err:DocName"/>
					<br/>
				</xsl:if>
				<xsl:if test="err:DocModeID">
					<span>Код вида документа: </span>
					<xsl:value-of select="err:DocModeID"/>
				</xsl:if>
				<xsl:if test="err:CRC">
					<br/>
					<span>Контрольная сумма: </span>
					<xsl:value-of select="err:CRC"/>
				</xsl:if>
