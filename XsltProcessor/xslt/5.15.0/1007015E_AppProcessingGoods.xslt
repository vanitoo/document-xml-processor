...
Проект приказа Минфина России "Порядок выдачи разрешения на переработку товаров вне таможенной территории, отзыва, 
аннулирования разрешения на переработку товаров вне таможенной территории и восстановления разрешения на переработку 
товаров вне таможенной территории."
Переработка - проекты приказов от  09-08-2019.rar, Приложение 2 к прик.вне (3).docx

 apg:ProcessingProcedureCode = 51 - переработка на ТТ
Проект приказа Минфина России "Порядок выдачи разрешения на переработку товаров на таможенной территории, передачи 
разрешения на переработку товаров на таможенной территории иному лицу, отзыва, аннулирования разрешения на переработку 
товаров на таможенной территории и восстановления действия разрешений на переработку товаров на таможенной территории"
Переработка - проекты приказов от  09-08-2019.rar, Приложение № 2.docx

 apg:ProcessingProcedureCode = 91 - переработка для внутреннего потребления
Проект приказа Минфина России "Порядок выдачи разрешения на переработку товаров для внутреннего потребления, 
отзыва и аннулирования разрешения на переработку товаров для внутреннего потребления, восстановления действия 
разрешения на переработку товаров для внутреннего потребления"
Переработка - проекты приказов от  09-08-2019.rar, Приложение 2 к для (3).docx
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.15.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:catProc_ru="urn:customs.ru:Information:ProcessingDocuments:ProcessingCommonAggregateTypes:5.15.0" xmlns:apg="urn:customs.ru:Information:ProcessingDocuments:AppProcessingGoods:5.15.0">
										...
										<xsl:for-each select="apg:ProcessingDescription/apg:WasteProducts">
											<xsl:choose>
												<xsl:when test="catProc_ru:IndicatorFurtherUse='1' or catProc_ru:IndicatorFurtherUse='true' or catProc_ru:IndicatorFurtherUse='TRUE' or catProc_ru:IndicatorFurtherUse='t'">
													<xsl:text>Дальнейшее коммерческого использования отходов возможно.</xsl:text>
												</xsl:when>
												<xsl:otherwise>
													<xsl:text>Дальнейшее коммерческого использования отходов невозможно.</xsl:text>
												</xsl:otherwise>
											</xsl:choose>
											<xsl:if test="catProc_ru:WasteOutProcedure">
												<br/>
												<xsl:text>Код таможенной процедуры для выпуска отходов: </xsl:text>
												<xsl:value-of select="catProc_ru:WasteOutProcedure"/>
											</xsl:if>
											<xsl:if test="catProc_ru:WasteOutDisposal">
												<br/>
												<xsl:apply-templates select="catProc_ru:WasteOutDisposal"/>
											</xsl:if>
										</xsl:for-each>
										<br/>
									</td>
		...
		<xsl:for-each select="apg:CustomsMark">
			<xsl:if test="apg:CancelDate">
				...
				<xsl:call-template name="russian_date">
					<xsl:with-param name="dateIn" select="apg:CancelDate"/>
				</xsl:call-template>
				<br/>
			</xsl:if>
			<xsl:if test="apg:RecoveryDate">
				...
				<xsl:call-template name="russian_date">
					<xsl:with-param name="dateIn" select="apg:RecoveryDate"/>
				</xsl:call-template>
				<br/>
			</xsl:if>
	...
	<xsl:template match="*" mode="doc">
		...
		<xsl:if test="apg:ArticleNumber">
			...
			<xsl:value-of select="apg:ArticleNumber"/>
		</xsl:if>
		<xsl:if test="apg:PartNumber">
			<xsl:text>, часть </xsl:text>
			<xsl:value-of select="apg:PartNumber"/>
		</xsl:if>
		<xsl:if test="apg:ClauseNumber">
			<xsl:text>, пункт </xsl:text>
			<xsl:value-of select="apg:ClauseNumber"/>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="DateLimit">
		<xsl:for-each select="apg:ProcessingDescription/apg:TermLimit">
			...
			<xsl:value-of select="apg:TermGeneral"/>
	...
	<xsl:template name="GoodsTblType1">
		...
		<table class="border">
			...
			<xsl:for-each select="$node">
				<tr class="annot_t">
					<td align="center">
						<xsl:value-of select="catProc_ru:GoodsNamber"/>
					</td>
					<td>
						<xsl:apply-templates select="catProc_ru:GoodsDescription"/>
					</td>
					<td>
						<xsl:value-of select="catProc_ru:GoodsTNVEDCode"/>
					</td>
					<td>
					...
					<td width="73%" class="aling">
						<b>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select="apg:DocumentStatus"/>
								</xsl:otherwise>
							</xsl:choose>
						</b>
					</td>
			...
			<table>
				...
				<tr>
					<xsl:choose>
						<xsl:when test="$isRecipient='0'">
							<td class="underline" align="center">
								<xsl:text>(полное и сокращенное наименование заявителя (декларанта), его идентификационный номер налогоплательщика (далее - ИНН), основной </xsl:text>
								<xsl:text>государственный регистрационный номер (далее - ОГРН), код причины постановки на учет (далее - КПП)</xsl:text>
							</td>
						</xsl:when>
						<xsl:otherwise>
							<td class="underline" align="center">
								<xsl:text>(полное и сокращенное наименование лица, его идентификационный номер налогоплательщика (далее - ИНН), основной </xsl:text>
								<xsl:text>государственный регистрационный номер (далее - ОГРН), код причины постановки на учет (далее - КПП)</xsl:text>
							</td>
						</xsl:otherwise>
					</xsl:choose>
	...
	<xsl:template match=" apg:ProcOrganization" mode="tbl_whith_underline">
		...
		<table>
			...
			<td class="underline" align="center">
				<xsl:choose>
					<xsl:when test="$ProcessingProcedureCode='21'">
						<xsl:text>(наименование(ния), (номер(а) и дата(ы) их подписания)</xsl:text>
					</xsl:when>
					<xsl:otherwise>
		...
		<xsl:if test="$ProcessingProcedureCode!='21'">
			...
			<xsl:variable name="OrgName">
				<xsl:choose>
					<xsl:when test="cat_ru:ShortName"><xsl:value-of select="cat_ru:ShortName"/></xsl:when>
					<xsl:otherwise><xsl:value-of select="cat_ru:OrganizationName"/></xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:call-template name="GoodsTblType1">
