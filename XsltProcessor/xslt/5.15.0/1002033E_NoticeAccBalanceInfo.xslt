        ...
        <div class="page" style="width: {$w}mm;">
          ...
          <table>
            <tr>
              ...
              <td class="value graphMain" style="width:50%">
				<xsl:value-of select="translate(translate(format-number(naccbi:Balance, '#,###.00'),',',' '),'.',',')"/>
              </td>
            </tr>
          </table>
          <table>
    ...
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
        <tr>
          ...
          <td class="graphMain value" style="width:50%">
            <xsl:for-each select="naccbi:BankAccount">
              <xsl:value-of select="." />
      ...
      <table>
        <tr>
          ...
          <td class="graphMain value" style="width:50%">
            <xsl:for-each select="naccbi:BankAccountDescription">
              <xsl:value-of select="." />
              <xsl:if test="position()!=last()">
                <xsl:text>, </xsl:text>
              </xsl:if>
      ...
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:value-of select="naccbi:BankName" />
          </td>
        </tr>
      </table>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:value-of select="naccbi:CodeBank" />
          </td>
        </tr>
      </table>
      <xsl:if test="naccbi:BankMFO">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">МФО банка</td>
            <td class="value graphMain" style="width:50%">
              <xsl:value-of select="naccbi:BankMFO" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="naccbi:OKPOID">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Код банка по ОКПО</td>
            <td class="value graphMain" style="width:50%">
              <xsl:value-of select="naccbi:OKPOID" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="naccbi:BICID">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">БИК банка</td>
            <td class="value graphMain" style="width:50%">
              <xsl:value-of select="naccbi:BICID" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="naccbi:SWIFTID">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">SWIFT код банка</td>
            <td class="value graphMain" style="width:50%">
              <xsl:value-of select="naccbi:SWIFTID" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="naccbi:CorrAccount">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Номер корреспондентского счета банка</td>
            <td class="value graphMain" style="width:50%">
              <xsl:value-of select="naccbi:CorrAccount" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="naccbi:TransitCurrencyAccount">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Валютный счет организации (транзитный)</td>
            <td class="graphMain value" style="width:50%">
              <xsl:for-each select="naccbi:TransitCurrencyAccount">
                <xsl:value-of select="." />
                <xsl:if test="position()!=last()">
                  <xsl:text>, </xsl:text>
                </xsl:if>
      ...
      <xsl:if test="naccbi:SpecialTransitCurrencyAccount">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Валютный счет организации (специальный транзитный)</td>
            <td class="graphMain value" style="width:50%">
              <xsl:for-each select="naccbi:SpecialTransitCurrencyAccount">
                <xsl:value-of select="." />
                <xsl:if test="position()!=last()">
                  <xsl:text>, </xsl:text>
                </xsl:if>
