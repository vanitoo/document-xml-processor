...
<!-- ************************************************************************************************************************************************************************************************************** -->	
				<xsl:if test="rd:UtilCollDetails/rd:Sign = 'К'"> <!-- реализация для (Форма расчета УС по ПП 1291_К) -->
					<div class="pageAlbum">
							<table>
								<tbody>
									<tr>
							...
							<table border="1" style="table-layout:fixed;word-wrap:break-word;width:297mm;">
								...
								<xsl:for-each select="rd:UtilCollDetails[rd:Sign = 'К']">
									<tr>
										<td align="center">
...
<!-- ************************************************************************************************************************************************************************************************************** -->							
				<xsl:if test="rd:UtilCollDetails/rd:Sign = 'С'"> <!-- реализация для (Форма расчета УС по ПП 81_С) -->
					<div class="pageAlbum">
							<table>
								<tbody>
									<tr>
							...
							<table border="1" style="table-layout:fixed;word-wrap:break-word;width:297mm;">
								...
								<xsl:for-each select="rd:UtilCollDetails[rd:Sign = 'С']">
									<tr>
	...
	<xsl:template name="Payer">
		...
		<table>
			...
			<tr>
				<td colspan="2" class="border-bottom">
					<xsl:apply-templates select="$payer/cat_ru:OrganizationName"/>
					<xsl:if test="$payer/cat_ru:OrganizationName and $payer/cat_ru:ShortName">
						<xsl:text> (</xsl:text>
					</xsl:if>
			...
			<tr>
				...
				<td width="70%" class="border-bottom">
					<xsl:apply-templates select="$payer/cat_ru:RFOrganizationFeatures/cat_ru:INN"/>
					<xsl:if test="$payer/cat_ru:RFOrganizationFeatures/cat_ru:INN">
						<xsl:text> / </xsl:text>
					</xsl:if>
					<xsl:apply-templates select="$payer/cat_ru:RFOrganizationFeatures/cat_ru:KPP"/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" class="border-bottom">
					<xsl:apply-templates select="$payer/cat_ru:RFOrganizationFeatures/cat_ru:OGRN"/>
					<xsl:if test="$payer/cat_ru:RFOrganizationFeatures/cat_ru:OGRN and $payer/rd:IdentityDoc">
