...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:ATDcat="urn:customs.ru:Information:CustomsDocuments:ATDCommon:5.17.0" xmlns:aai="urn:customs.ru:Information:CustomsDocuments:AddActInsp:5.17.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
			...
			<body>
				<div class="page">
						...
						<tbody>
							<tr>
								<td align="center" class="graphMain">
									<font size="4">
										<b>Дополнение к акту досмотра/осмотра</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="aai:Act_ID">
						<table class="w190">
							<tr>
								<td class="annot graphMain" style="width:30%">Уникальный идентификатор АТД</td>
					...
					<table class="w190">
						<tr>
							<td class="annot graphMain" style="width:30%">Номер АТД</td>
							<td class="value graphMain" style="width:70%">
					...
					<xsl:if test="aai:ATDGoods">
						<div class="title marg-top" style="background-color:gray;">ТОВАРЫ АТД</div>
						<xsl:apply-templates select="aai:ATDGoods"/>
					</xsl:if>
				</div>
			</body>
		</html>
	</xsl:template>
	<xsl:template match="aai:ATDGoods">
		<div class="title marg-top" style="background-color: lightgray;">
			<xsl:text>Номер товара АТД </xsl:text>
			<xsl:value-of select="aai:GoodsNumeric"/>
		</div>
		<xsl:if test="aai:IdentificationMeansDetails">
			<div class="title marg-top">Сведени о средствах идентификации</div>
			<table class="bordered w190">
				<tbody>
					<tr class="title">
						<td class="graphMain bordered">Порядковый номер записи</td>
						<td class="graphMain bordered">Код уровня маркировки</td>
						<td class="graphMain bordered">Цифровой идентификатор применения блока</td>
						<td class="graphMain bordered">Значение блока информации, содержащейся в коде идентификации</td>
						<td class="graphMain bordered">Строка, полученная со сканера кода идентификации</td>
					</tr>
					<xsl:for-each select="aai:IdentificationMeansDetails">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</tbody>
			</table>
		</xsl:if>
		<xsl:if test="aai:CIMList">
			<div class="title marg-top">Сведения о контрольно-идентификационных знаках (RFID)</div>
			<table class="bordered w190">
				<tbody>
					<tr class="title">
						<td class="graphMain bordered">Порядковый номер записи</td>
						<td class="graphMain bordered">Визуальный номер КИЗ</td>
						<td class="graphMain bordered">Идентификатор чипа<br/>радиочастотной метки (RFID) TID</td>
						<td class="graphMain bordered">Идентификатор<br/>Global Trade Item Number (GTIN)</td>
					</tr>
					<xsl:for-each select="aai:CIMList">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</tbody>
			</table>
	...
	<xsl:template match="aai:CIMList">
		<tr>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:Number"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:VisualIdentifierCIM"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:RFID_TID"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:GTINIdentifier"/>
			</td>
		</tr>
	</xsl:template>
	<xsl:template match="aai:IdentificationMeansDetails">
		<tr>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:Number"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:AggregationKindCode"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:AIId"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:IdentifacationMeansUnitCharacterValueId"/>
			</td>
			<td class="graphMain bordered" style="word-break: break-all">
				<xsl:value-of select="aai:ScanText"/>
			</td>
		</tr>
	</xsl:template>
