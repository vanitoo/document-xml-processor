...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.17.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:dcg="urn:customs.ru:Information:CustomsDocuments:DecisionClassGood:5.19.0">
						...
						<tr>
							<td colspan="2">
								...
								<div class="content">
									...
									<u>
										<xsl:for-each select="dcg:Goods/dcg:DTList">
											<xsl:apply-templates select="dcg:DTNum"/>
								...
								<tr>
									...
									<td class="bordered">
										<xsl:choose>
											...
											<xsl:when test="dcg:ReasonCode=3">в результате применения профиля риска (на основании полученного экспертного заключения)</xsl:when>
											<xsl:when test="dcg:ReasonCode=4">на основании поручения вышестоящих таможенных органов</xsl:when>
											<xsl:when test="dcg:ReasonCode=5">в связи с представлением декларантом после выпуска товаров документов, выданных федеральными органами исполнительной власти, либо обращением декларанта о смене кода товаров, декларированных в субпозициях TH ВЭД ЕАЭС, предназначенных для классификации товаров для промышленной сборки моторных транспортных средств товарных позиций 8701 - 8705 TH ВЭД ЕАЭС. их узлов и агрегатов</xsl:when>
											<xsl:when test="dcg:ReasonCode=6">на основании дополнительно запрошенных товаросопроводительных, коммерческих, технических документов (за исключением экспертного заключения и документов федеральных органов исполнительной власти)</xsl:when>
											<xsl:when test="dcg:ReasonCode=7">в результате применения профиля риска (за исключением экспертного заключения)</xsl:when>
											<xsl:when test="dcg:ReasonCode=9">прочие основания (в программном средстве обеспечена возможность указания конкретного основания принятия решения по классификации товара в комментарии)</xsl:when>
	...
	<xsl:template match="*[local-name() = 'GoodsGroupQuantity']">
		<xsl:value-of select="translate(*[local-name() = 'GoodsQuantity'], '.', ',')"/>
