...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:catComFin_ru="urn:customs.ru:Information:CommercialFinanceDocuments:CommercialFinanceCommonAgregateTypesCust:5.10.0" xmlns:cltTrans_ru="urn:customs.ru:Information:TransportDocuments:TransportCommonLeafTypesCust:5.14.3" xmlns:catTrans_ru="urn:customs.ru:Information:TransportDocuments:TransportCommonAgregateTypesCust:5.14.3" xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.17.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.17.0" xmlns:atb="urn:customs.ru:Information:TransportDocuments:AutoBill:5.18.0">
				...
				<div class="page">
							...
							<tr>
								<td align="center" class="graphMain">
									<font size="4">
										...
										<xsl:text>№ </xsl:text><xsl:apply-templates select="atb:NumberTN"/>
										<xsl:text> от </xsl:text><xsl:apply-templates select="atb:DateTN" mode="russian_date"/>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<table class="w190 bordered">
						<tbody>
							...
							<tr>
								<td class="bordered">Экземпляр № <xsl:apply-templates select="atb:NumberCopies"/></td>
								<td class="bordered">
									<table width="100%">
										<tbody>
											<tr>
												<td width="60%" style="border-right:1px solid">
													...
													<xsl:apply-templates select="atb:Order/atb:DateOrder" mode="russian_date"/>
												</td>
												<td>№ <xsl:apply-templates select="atb:Order/atb:NumberOrder"/></td>
											</tr>
										</tbody>
									</table>
								</td>
											...
											<xsl:for-each select="atb:Notes">
												<tr>
													...
													<td valign="top" style="border-top:1px solid">
														<xsl:apply-templates select="atb:PersonSign/cat_ru:PersonSurname"/>
														<xsl:text> </xsl:text>
														<xsl:variable name="xpath_PersonName">
															<xsl:call-template name="get_xpath">
																<xsl:with-param name="node" select="atb:PersonSign/cat_ru:PersonName"/>
															</xsl:call-template>
														</xsl:variable>
														...
														<br/>
														<xsl:apply-templates select="atb:PersonSign/cat_ru:IssueDate" mode="russian_date"/>
													</td>
												</tr>
											</xsl:for-each>
										</tbody>
	...
	<xsl:template match="atb:ConsignorInstruction">
		...
		<xsl:if test="atb:DeadlineTransport">
			Рекомендуемый предельный срок перевозки: 				<xsl:apply-templates select="atb:DeadlineTransport" mode="russian_date"/><br/>
		</xsl:if>
		<xsl:if test="atb:Temperature">
			Рекомендуемый температурный режим: <xsl:apply-templates select="atb:Temperature"/><br/>
		</xsl:if>
	...
	<xsl:template match="atb:LoadCargo|atb:UnloadCargo">
		<xsl:if test="local-name() = 'UnloadCargo'">
			<xsl:text>Код ТО: </xsl:text>
			<xsl:apply-templates select="../atb:DestinationOfficeIdentifier"/>
		</xsl:if>
		<xsl:if test="atb:Address">
			<xsl:if test="local-name() = 'UnloadCargo'"><br/></xsl:if>
		...
		<xsl:if test="atb:DateTime">
			...
			<xsl:apply-templates select="atb:DateTime" mode="russian_date"/>
			<xsl:text> </xsl:text>
		...
		<xsl:if test="atb:ActualDateTime">
			...
			<xsl:apply-templates select="atb:ActualDateTime" mode="russian_date"/>
			<xsl:text> </xsl:text>
		...
		<xsl:if test="atb:CargoWeight">
			...
			<xsl:apply-templates select="atb:CargoWeight"/>
		</xsl:if>
		<xsl:if test="atb:PlacesQuantity">
			<br/>
			<xsl:text>Количество грузовых мест: </xsl:text>
			<xsl:apply-templates select="atb:PlacesQuantity"/>
		</xsl:if>
	...
	<xsl:template match="atb:Conditions">
		<xsl:if test="atb:DeadlineDelivery">
			...
			<xsl:apply-templates select="atb:DeadlineDelivery" mode="russian_date"/>
		</xsl:if>
		<xsl:if test="atb:FormNotification">
			...
			<xsl:apply-templates select="atb:FormNotification"/>
		</xsl:if>
		...
		<xsl:if test="atb:PenaltyInfo">
			...
			<xsl:apply-templates select="atb:PenaltyInfo"/>
		</xsl:if>
	...
	<xsl:template match="atb:Loading|atb:Unloading|atb:Desinfection">
			<br/>
		<xsl:variable name="xpath_Loading">
			<xsl:call-template name="get_xpath">
				<xsl:with-param name="node" select="current()"/>
			</xsl:call-template>
		</xsl:variable>
		<element xml_node="{$xpath_Loading}">
			<xsl:value-of select="."/>
		</element>
	</xsl:template>
			...
			<tbody>
				<tr>
					...
					<td width="20%" style="border-bottom:1px solid;" align="center">
						<xsl:apply-templates select="atb:DateOrder" mode="russian_date"/>
					</td>
					<td width="1%"></td>
			...
			<tbody>
				<tr>
					<td width="50%">
						...
						<xsl:apply-templates select="atb:ChangeConditionsInMove"/>
					</td>
					<td width="50%">
						<xsl:apply-templates select="atb:ActualStateUnload"/>
						<xsl:apply-templates select="atb:ChangeConditionsInUnload"/>					
					</td>
				</tr>
			</tbody>
		</table>
	...
	<xsl:template match="atb:ActualStateLoad|atb:ActualStateUnload">
		...
		<xsl:if test="atb:PackingMark">
			<br/>
			Маркировка грузовых мест: <xsl:apply-templates select="atb:PackingMark"/>
		</xsl:if>
		...
		<xsl:if test="atb:CargoState">
			<br/>
			Фактическое состояние: <xsl:apply-templates select="atb:CargoState"/>
		</xsl:if>
		<xsl:if test="position()!=last()"><br/><br/></xsl:if>
	</xsl:template>
	<!-- 13. Прочие условия -->
	<xsl:template match="atb:OtherConditions">
		...
		<xsl:if test="atb:WorkingDay">
			...
			<xsl:for-each select="atb:WorkingDay">
				<xsl:value-of select="position()"/>
				<xsl:text>. c </xsl:text>
				<xsl:variable name="xpath_From">
					<xsl:call-template name="get_xpath">
						<xsl:with-param name="node" select="atb:From"/>
					</xsl:call-template>
				</xsl:variable>
				<element xml_node="{$xpath_From}">
					<xsl:value-of  select="substring(atb:From, 1, 8)"/>
				</element>
				<xsl:text> до </xsl:text>
				<xsl:variable name="xpath_To">
					<xsl:call-template name="get_xpath">
						<xsl:with-param name="node" select="atb:To"/>
					</xsl:call-template>
				</xsl:variable>
				<element xml_node="{$xpath_To}">
	...
	<xsl:template match="atb:Readdressing">
		<!--b><xsl:value-of select="position()"/>. </b-->
		<xsl:apply-templates select="atb:DateReaddressing" mode="russian_date"/>
	...
	<xsl:template match="atb:Calculation">
		<xsl:if test="atb:CostOfTheCarrierService">
			...
			<xsl:apply-templates select="atb:CostOfTheCarrierService"/>
		</xsl:if>
		<xsl:if test="atb:ProcedureCalculation">
			<br/>
			Порядок расчета провозной платы:
			<xsl:apply-templates select="atb:ProcedureCalculation"/>
		</xsl:if>
		<xsl:if test="atb:FeeOfTransporting">
			<br/>
			Размер провозной платы, руб:
			<xsl:apply-templates select="atb:FeeOfTransporting"/>
		</xsl:if>
		...
		<xsl:if test="atb:Payer">
			...
			<br/>
			<xsl:apply-templates select="atb:Payer/catComFin_ru:Name"/>
			<xsl:if test="atb:Payer/catComFin_ru:CompanyID">
				<br/>
				ИНН: <xsl:apply-templates select="atb:Payer/catComFin_ru:CompanyID"/>
			</xsl:if>
			<xsl:if test="atb:Payer/catComFin_ru:KPPCode">
				<br/>
				КПП: <xsl:apply-templates select="atb:Payer/catComFin_ru:KPPCode"/>
			</xsl:if>
            ...
            <xsl:if test="atb:Payer/catComFin_ru:Phone">
				<br/>
				Телефон: <xsl:apply-templates select="atb:Payer/catComFin_ru:Phone"/>
            </xsl:if>
			<br/>
		...
		<table>
			<tbody>
				<tr>
					<td width="1%"></td>
					<td width="50%">
						<xsl:apply-templates select="cat_ru:PersonPost"/><br/>
						<xsl:apply-templates select="cat_ru:PersonSurname"/>
						<xsl:text> </xsl:text>
