...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:catpi_ru="urn:customs.ru:Information:PriorInformation:PriorCommonAggregateTypes:5.17.0" xmlns:piaut="urn:customs.ru:Information:PriorInformation:PI_AutoArrive:5.17.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.16.0">
	...
	<xsl:template match="piaut:BorderTransport">
		<tr>
			<td style="border-top: 3px solid gray">
			...
			<td style="border-top: 3px solid gray">Код вида ТС по классификатору видов ТС <span class="text">
		...
		<tr>
			...
			<td colspan="2">
			Идентификатор/№ первого прицепного транспортного средства 
			<span class="text">
		...
		<xsl:variable name="st">
			<xsl:choose>
				<xsl:when test="position()=last()"><xsl:value-of select="'border-bottom: 3px solid gray'"/></xsl:when>
				<xsl:otherwise><xsl:value-of select="''"/></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<tr>
			<td colspan="2" style="{$st}">
			Код марки транспортного средства 
			<span class="text">
