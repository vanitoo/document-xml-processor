...
-->
											...
											<td class="bordered_only_bottom" valign="top">
												<div class="cellwidener">
													...
													<span class="normal">
														...
														<xsl:value-of select="argb:GoodsShipmentDetails/argb:ValueAmount/RUScat_ru:CurrencyCode"/>
													</span>
												</div>
											</td>
										</tr>
								...
								<tr>
									...
									<td align="right">
										<xsl:if test="argb:GoodsMeasureDetails/cat_ru:GoodsQuantity">
											<xsl:value-of select="argb:GoodsMeasureDetails/cat_ru:GoodsQuantity"/>
										</xsl:if>
									</td>
									<td align="center">
										<xsl:if test="argb:GoodsMeasureDetails/cat_ru:MeasureUnitQualifierName">
											<xsl:value-of select="argb:GoodsMeasureDetails/cat_ru:MeasureUnitQualifierName"/>
										</xsl:if>
										<xsl:if test="argb:GoodsMeasureDetails/cat_ru:MeasureUnitQualifierCode">
											...
											<xsl:value-of select="argb:GoodsMeasureDetails/cat_ru:MeasureUnitQualifierCode"/>
										</xsl:if>
									</td>
									<td align="right">
									...
									<td align="center">
										<xsl:choose>
											<xsl:when test="argb:OriginCountryDetails">
												<span class="normal">
													<xsl:value-of select="argb:OriginCountryDetails/RUScat_ru:CountryCode"/>
												</span>
												<xsl:if test="argb:OriginCountryDetails/RUScat_ru:CountryName">
													<br/>
													<span class="normal">
														<xsl:value-of select="argb:OriginCountryDetails/RUScat_ru:CountryName"/>
													</span>
												</xsl:if>
												<xsl:if test="argb:OriginCountryDetails/RUScat_ru:TerritoryCode">
													<br/>
													<span class="normal">
														<xsl:value-of select="argb:OriginCountryDetails/RUScat_ru:TerritoryCode"/>
													</span>
												</xsl:if>
											</xsl:when>
											<xsl:otherwise>
	...
	<xsl:template match="RUDECLcat:BrokerRegistryDocDetails">
		<span class="normal">1. <xsl:value-of select="RUDECLcat:DocKindCode"/>
		</span>
		<br/>
		<span class="normal">2. <xsl:value-of select="RUDECLcat:RegisterDocumentDetails/RUScat_ru:RegistrationNumberId"/>
		</span>
	...
	<xsl:template match="argb:GoodsLocationDetails">
		...
		<table>
			<tr>
				<td width="6%">
					<span class="normal">
						<xsl:value-of select="RUDECLcat:GoodsLocationCode"/>
					</span>
				</td>
				<td width="14%">
					<span class="normal">
						<xsl:value-of select="RUDECLcat:CustomsOfficeCode"/>
					</span>
				</td>
				<td width="5%">
					<span class="normal">
	...
	<xsl:template match="argb:PaymentGuaranteeDetails">
		...
		<xsl:value-of select="RUDECLcat:GuaranteeAmount/RUScat_ru:CurrencyCode"/>
			<xsl:if test="RUDECLcat:INN"> - ИНН: <xsl:value-of select="RUDECLcat:INN"/>
			</xsl:if>
			<xsl:if test="RUDECLcat:BankId"> - ID банка: <xsl:value-of select="RUDECLcat:BankId"/>
			</xsl:if>
	...
	<xsl:template match="argb:DepartureCountry|argb:DestinationCountry">
		...
		<xsl:if test="RUScat_ru:CountryName">
			<br/>
			<span class="normal">
				<xsl:value-of select="RUScat_ru:CountryName"/>
			</span>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="RUDECLcat:RepresentativeContractDetails">
		<span class="normal">3. <xsl:value-of select="RUScat_ru:DocKindCode"/>
		</span>
		<br/>
		<span class="normal">4. <xsl:value-of select="cat_ru:PrDocumentNumber"/>
		</span>
		<br/>
		<span class="normal">5. 
		<xsl:call-template name="russian_date">
