...
<xsl:stylesheet version="1.0"
	...
	xmlns:ATDcat="urn:customs.ru:Information:CustomsDocuments:ATDCommon:5.17.0" 
	xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.17.0" 
	xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.20.0" 
	xmlns:RUSclt_ru="urn:customs.ru:RUSCommonLeafTypes:5.14.3" 
	xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.20.0" 
	xmlns:tpo="urn:customs.ru:Information:CustomsDocuments:TPO:5.20.0"
					...
					<table width="100%" style="padding: 0;">
						<tbody>
											...
											<tr>
												<td class="small" style="border-right:1px solid black;border-bottom:1px solid black;" width="50%">
													...
													<span class="normal">
														...
														<xsl:if test="not(tpo:PaymentCurrency/tpo:CurrencyEuroRate)"/>
													</span>
												</td>
							...
							<tr valign="top">
								<xsl:choose>
									<xsl:when test="contains('ПЛ МА СХ РЕ', $MP)">
										<xsl:comment>graph18 data = tpo:CommonPaymentDetails</xsl:comment>
							...
							<tr valign="top">
								<td class="bordered small">
									...
									<br/>
									<span class="normal">
										<xsl:apply-templates select="tpo:CommonPaymentDetails/tpo:FactPaymentDetails[tpo:PaymentModeCode != '4XXX']/tpo:PaymentDocDetails[tpo:WriteOffAmount != 0]" mode="graph20"/>
										<xsl:if test="not(tpo:CommonPaymentDetails/tpo:FactPaymentDetails[tpo:PaymentModeCode != '4XXX']/tpo:PaymentDocDetails[tpo:WriteOffAmount != 0])">-</xsl:if>
									</span>
								</td>
								<td class="bordered small">
						...
						<td class="small">
							<table>
								<tbody>
									...
									<xsl:choose>
										<xsl:when test="contains('МА РЕ ПЛ', $MP)">
											<xsl:choose>
												...
												<xsl:when test="contains('МА РЕ', $MP) or ($MP = 'ПЛ' and ($TPOSign = '3' or $TPOSign = '4' or $TPOSign = '9'))">
	...
	<xsl:template match="*" mode="graph19data">
		...
		<tr valign="top" align="left">
			...
			<td class="small" style="{$borderBottom}" align="right">
				<xsl:apply-templates select="tpo:Amount|tpo:WriteOffAmount" mode="translate_number2decimals"/> <!-- второй вариант - ПЛ и Sign = 1 или 2 -->
			</td>
		</tr>
	</xsl:template>
					...
					<tbody>
						...
						<tr>
							<td width="15%" class="small" style="border-left:1px solid black; border-right:1px solid black; border-bottom: 1px solid black;">
								<xsl:choose>
									<xsl:when test="not($goods/tpo:GoodsNumber) or contains('МА ПЛ СХ', $MP)">-</xsl:when>
									<xsl:otherwise><xsl:apply-templates select="$goods/tpo:GoodsNumber"/></xsl:otherwise>
								</xsl:choose>
							</td>
							<td width="15%" class="small" style="border-bottom: 1px solid black;">
								<xsl:choose>
									<xsl:when test="not($goods/tpo:GoodsMovementWay) or string-length($MW) != 0 or contains('МА ПЛ СХ', $MP)">-</xsl:when>
									<xsl:otherwise><xsl:apply-templates select="$goods/tpo:GoodsMovementWay"/></xsl:otherwise>
								</xsl:choose>
							</td>
						</tr>
						<tr>
							...
							<td class="small" style="border-right: 1px solid black; border-bottom: 1px solid black;" width="50%">
								...
								<span class="normal">
									<xsl:choose>
										...
										<xsl:when test="$goods/tpo:GoodsTNVEDCode">
											<xsl:apply-templates select="$goods/tpo:GoodsTNVEDCode"/>
										</xsl:when>
										<xsl:otherwise>-</xsl:otherwise>
									</xsl:choose>
	...
	<xsl:template match="*" mode="currency">
		...
		<element xml_node="{$xpath_date}">
			...
			<xsl:choose>
				<xsl:when test="$BusinessProcessType"><xsl:value-of select="nsi:GetFromNsi('VALNAME', 'BUK', 'KOD', $val)"/></xsl:when>
				<xsl:otherwise>
					<xsl:choose>
		...
		<element xml_node="{$xpath_date}">
			...
			<xsl:choose>
				<xsl:when test="$BusinessProcessType">
					<xsl:value-of select="nsi:GetFromNsi('EDIZM', 'KRNAIM', 'KOD', $val)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="."/>
				</xsl:otherwise>
