...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.22.0" xmlns:reqins="urn:customs.ru:Information:CustomsDocuments:ReqInspection:5.22.0">
	<!-- Шаблон для типа ReqInspectionType -->
	<xsl:template match="reqins:ReqInspection">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Заявление о проведении осмотра, измерения товаров, находящихся под таможенным контролем, осуществления отбора проб и образцов, выполнения с ними грузовых операций/снятием средств таможенной идентификации с запирающих устройств грузовых отделений транспортных средств</b>
									</font>
								</td>
							</tr>
						</tbody>
						...
						<tr>
							...
							<td class="value graphMain" style="width:50%">
								<xsl:call-template name="date_time">
									<xsl:with-param name="dateTimeIn" select="reqins:AppDateTime"/>
								</xsl:call-template>
							</td>
						</tr>
					</table>
					<xsl:if test="reqins:Customs">
						...
						<div>
							<xsl:apply-templates select="reqins:Customs">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:CustomsPost">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Наименование пункта пропуска</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="reqins:CustomsPost" />
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="reqins:Declarant">
						<div class="title marg-top">Сведения об организации декларанта (физического лица)</div>
						<div>
							<xsl:apply-templates select="reqins:Declarant">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:GTDID">
						<div class="title marg-top">Номер ДТ</div>
						<div>
							<xsl:apply-templates select="reqins:GTDID">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:TransportDetails">
						<div class="title marg-top">Сведения о транспортных средствах</div>
						<div>
							<xsl:apply-templates select="reqins:TransportDetails">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:StorageHead">
						<div class="title marg-top">Владелец склада временного хранения (иного места временного хранения)</div>
						<div>
							<xsl:apply-templates select="reqins:StorageHead">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:StorageDocumentNumber">
						...
						<div>
							<xsl:apply-templates select="reqins:StorageDocumentNumber">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:LocationSign">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Место нахождения товаров</td>
								<td class="value graphMain" style="width:50%">
									<xsl:choose>
										<xsl:when test="reqins:LocationSign='true' or reqins:LocationSign='1'">
											<xsl:text>на временном хранении</xsl:text>
										</xsl:when>
										<xsl:when test="reqins:LocationSign='false' or reqins:LocationSign='0'">
											<xsl:text>на прилегающей к СВХ территории</xsl:text>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="reqins:LocationSign" />
										</xsl:otherwise>
									</xsl:choose>
								</td>
							</tr>
					...
					<xsl:if test="reqins:ZTKAdress">
						<div class="title marg-top">Адрес ЗТК, где находятся товары</div>
						<div>
							<xsl:apply-templates select="reqins:ZTKAdress">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:SubmittedAppPerson">
						<div class="title marg-top">Сведения о физическом лице, подавшем заявление </div>
						<div>
							<xsl:apply-templates select="reqins:SubmittedAppPerson">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="reqins:OtherInfo">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Иные сведения</td>
								<td class="graphMain value" style="width:50%">
									<xsl:for-each select="reqins:OtherInfo">
										<xsl:value-of select="." />
  ...
  <xsl:template match="reqins:Contact">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <xsl:template match="reqins:Customs">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
  ...
  <xsl:template match="reqins:Declarant">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <xsl:template match="reqins:GTDID">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
  ...
  <xsl:template match="reqins:IdentityCard">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <xsl:template match="reqins:StorageDocumentNumber">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <!-- Шаблон для типа cat_ru:OrganizationBaseType -->
  <xsl:template match="reqins:StorageHead">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="cat_ru:OrganizationName">
  ...
  <!-- Шаблон для типа reqins:SubmittedAppPersonType -->
  <xsl:template match="reqins:SubmittedAppPerson">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
      ...
      <xsl:if test="reqins:INN">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Индивидуальный номер налогоплательщика</td>
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="reqins:INN" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="reqins:IdentityCard">
        <div class="title marg-top">Сведения о документе, удостоверяющем личность</div>
        <div>
          <xsl:apply-templates select="reqins:IdentityCard">
            <xsl:with-param name="wdth" select="$w" />
          </xsl:apply-templates>
        </div>
      </xsl:if>
      <xsl:if test="reqins:Contact">
        <div class="title marg-top">Контактные реквизиты</div>
        <div>
          <xsl:apply-templates select="reqins:Contact">
            <xsl:with-param name="wdth" select="$w" />
          </xsl:apply-templates>
        </div>
      </xsl:if>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="reqins:DocKindCode" />
          </td>
        </tr>
      </table>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="reqins:DocValidityDate" mode="russian_date" />
          </td>
        </tr>
      </table>
    </div>
  </xsl:template>
  <!-- Шаблон для типа reqins:TransportDetailsType -->
  <xsl:template match="reqins:TransportDetails">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="reqins:TransportType" />
          </td>
        </tr>
      </table>
      <xsl:if test="reqins:TransportID">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Номер транспортного средства</td>
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="reqins:TransportID" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <xsl:if test="reqins:CarriageID">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Номер вагона</td>
            <td class="graphMain value" style="width:50%">
              <xsl:for-each select="reqins:CarriageID">
                <xsl:value-of select="." />
                <xsl:if test="position()!=last()">
                  <xsl:text>, </xsl:text>
                </xsl:if>
      ...
      <xsl:if test="reqins:BillSerialNumber">
        <table>
          <tr>
            ...
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="reqins:BillSerialNumber" />
            </td>
          </tr>
        </table>
      </xsl:if>
  ...
  <xsl:template match="reqins:ZTKAdress">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
  ...
  <xsl:template match="//*[local-name()='ReqInspection']//*" priority="-1">
