...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:npc="urn:customs.ru:Information:TreatmentDocuments:NotifProlongDocsConsid:5.22.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	<!-- Шаблон для типа NotifProlongDocsConsidType -->
	<xsl:template match="npc:NotifProlongDocsConsid">
		<html>
			<head>
				<style>
									body {
			...
			<body>
				<div class="page">
						...
						<tbody>
							<tr>
								<td align="center" class="graphMain">
									<font size="4">
										<b>Уведомление о продлении срока рассмотрения документов при рассмотрении вопроса о выдаче разрешения на переработку</b>
									</font>
								</td>
							</tr>
						</tbody>
						...
						<tr>
							...
							<td class="value graphMain" style="width:50%">
								<xsl:call-template name="russian_date">
									<xsl:with-param name="dateIn" select="npc:SendDate"/>
								</xsl:call-template>
							</td>
						</tr>
					</table>
					<xsl:if test="npc:Comments">
						<table class="w190">
							<tr>
								<td class="annot graphMain" style="width:50%">Комментарии</td>
								<td class="graphMain value" style="width:50%">
									<xsl:for-each select="npc:Comments">
										<xsl:value-of select="."/>
									</xsl:for-each>
								</td>
							</tr>
					...
					<xsl:if test="npc:Customs">
						<div class="title marg-top">Таможенный орган</div>
						<xsl:apply-templates select="npc:Customs"/>
					</xsl:if>
					<xsl:if test="npc:CustomsPerson">
						<div class="title marg-top">Должностное лицо таможенного органа</div>
						<xsl:apply-templates select="npc:CustomsPerson"/>
					</xsl:if>
				</div>
			</body>
		</html>
	...
	<xsl:template match="npc:Customs">
		<table class="w190">
			<tr>
	...
	<xsl:template match="npc:CustomsPerson">
		<table class="w190">
			<tr>
