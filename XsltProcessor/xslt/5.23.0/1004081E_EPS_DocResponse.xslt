...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:edrs="urn:customs.ru:Information:ExchangeDocuments:EPS_DocResponse:5.22.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
        ...
        <div class="page" style="width: {$w}mm;">
            ...
            <tbody>
              <tr>
                <td align="center" class="title">
                  <font size="4">
                    <b>Предоставление документа по запросу</b>
                  </font>
                </td>
              </tr>
            </tbody>
          ...
          <table>
            <tr>
              ...
              <td class="value graphMain" style="width:50%">
                <xsl:apply-templates select="edrs:RequestPositionID" />
              </td>
            </tr>
          </table>
          <xsl:if test="edrs:DocResponse">
            <div class="title marg-top">Представлен запрошенный документ</div>
            <div>
              <xsl:apply-templates select="edrs:DocResponse">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          <xsl:if test="edrs:ArchDocumentInfo">
            <div class="title marg-top">Представлена только информация из архива декларанта</div>
            <div>
          ...
          <xsl:if test="edrs:ArchDocumentInfoAdd">
            <div class="title marg-top">Информация архива декларанта</div>
            <div>
          ...
          <xsl:if test="edrs:DocResponseAdd">
            <div class="title marg-top">Запрошенный документ</div>
            <div>
              <xsl:apply-templates select="edrs:DocResponseAdd">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          <xsl:if test="edrs:ScanInfo">
            <div class="title marg-top">Идентификатор ранее сканированного документа в архиве ТО / Присвоенный идентификатор сканированного документа </div>
            <div>
              <xsl:apply-templates select="edrs:ScanInfo">
                <xsl:with-param name="wdth" select="$w" />
              </xsl:apply-templates>
            </div>
          </xsl:if>
          ...
          <table>
            <tr>
              ...
              <td class="value graphMain" style="width:50%">
                <xsl:apply-templates select="edrs:ReasonPaper" />
              </td>
            </tr>
          </table>
  ...
  <!-- Шаблон для типа edrs:ArchDocInfoType -->
  <xsl:template match="edrs:ArchDocumentInfo">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="edrs:ArchID">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Идентификатор архива</td>
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="edrs:ArchID" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:ArchDeclID" />
          </td>
        </tr>
      </table>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:ArchDocID" />
          </td>
        </tr>
      </table>
      <xsl:if test="edrs:ArchDocStatus">
        <table>
          <tr>
            <td class="annot graphMain" style="width:50%">Статус документа в архиве</td>
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="edrs:ArchDocStatus" />
            </td>
          </tr>
        </table>
      </xsl:if>
  ...
  <!-- Шаблон для типа edrs:ArchDocInfoType -->
  <xsl:template match="edrs:ArchDocumentInfoAdd">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="edrs:ArchID">
    ...
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:ArchDocAlbum" />
          </td>
        </tr>
      </table>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:ArchDocDate" mode="russian_date" />
          </td>
        </tr>
      </table>
      <xsl:if test="edrs:FirstDT">
        <div class="title marg-top">Регистрационный номер первой ДТ, при подаче который использован документ</div>
        <div>
  ...
  <!-- Шаблон для типа edrs:DocResponseType -->
  <xsl:template match="edrs:DocResponse">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="edrs:DocResponseBody">
        <div class="title marg-top">Тело запрошенного документа</div>
        <div>
          <xsl:apply-templates select="edrs:DocResponseBody">
            <xsl:with-param name="wdth" select="$w" />
          </xsl:apply-templates>
        </div>
      </xsl:if>
  ...
  <!-- Шаблон для типа edrs:DocResponseType -->
  <xsl:template match="edrs:DocResponseAdd">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <xsl:if test="edrs:DocResponseBody">
  ...
  <!-- Шаблон для типа edrs:DocResponseBodyType -->
  <xsl:template match="edrs:DocResponseBody">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;" />
  </xsl:template>
    ...
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:CustomsCode" />
          </td>
        </tr>
      </table>
      <xsl:if test="edrs:Comment">
        <table>
          <tr>
            ...
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="edrs:Comment" />
            </td>
          </tr>
        </table>
      </xsl:if>
      <table>
        <tr>
          ...
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:ScanDate" mode="russian_date" />
          </td>
        </tr>
      </table>
    </div>
  </xsl:template>
  <!-- Шаблон для типа edrs:ScanInfoType -->
  <xsl:template match="edrs:ScanInfo">
    <xsl:param name="wdth" select="0" />
    <xsl:variable name="w" select="$wdth - 5" />
    <div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
      <table>
        <tr>
          <td class="annot graphMain" style="width:50%">Идентификатор сканированного документа</td>
          <td class="value graphMain" style="width:50%">
            <xsl:apply-templates select="edrs:ScanID" />
          </td>
        </tr>
      </table>
      <xsl:if test="edrs:DocumentModeID">
        <table>
          <tr>
            ...
            <td class="value graphMain" style="width:50%">
              <xsl:apply-templates select="edrs:DocumentModeID" />
            </td>
          </tr>
        </table>
      </xsl:if>
  ...
  <xsl:template match="//*[local-name()='EPS_DocResponse']//*" priority="-1">
