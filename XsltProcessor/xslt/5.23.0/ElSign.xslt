...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ElS="urn:customs.ru:includes:ElSign:5.22.0">
	<xsl:template match="*" mode="Responsibility">
		<xsl:apply-templates select="." mode="Signature">
			<xsl:with-param name="header" select="'ПРЕДУПРЕЖДЕНИЕ ПОДПИСАНО ЭЛЕКТРОННОЙ ПОДПИСЬЮ'"/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="*" mode="Signature">
		<xsl:param name="header" select="'ДОКУМЕНТ ПОДПИСАН ЭЛЕКТРОННОЙ ПОДПИСЬЮ'"/>
						...
						<div style="border: 0px;">
							<table>
								<tbody>
									<tr>
										<td width="4%"/>
										<td width="92%" colspan="2" style="padding-top: 3px;font-family: Arial;font-size: 10pt;font-weight: bold;color: rgb(0,102,255);text-align:center;vertical-align:middle;word-break: normal; overflow-wrap: break-word; word-wrap: break-word;"><xsl:value-of select="$header"/></td>
										<td width="4%"/>
									</tr>
									<tr>
										<td/>
