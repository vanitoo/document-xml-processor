...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:od="urn:customs.ru:Information:ExchangeDocuments:STZ_ObligDecisionCCZ:5.26.0">
	<!-- Шаблон для типа STZ_ObligDecisionCCZType -->
	<xsl:template match="od:STZ_ObligDecisionCCZ">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Решение по обязательству о соблюдении условий использования иностранных товаров, помещенных под таможенную процедуру СТЗ</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<br/>
					<xsl:if test="od:Customs">
						<div class="title marg-top">Сведения о таможенном органе</div>
						<div>
							<xsl:apply-templates select="od:Customs">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="od:AppRegNum">
						<div class="title marg-top">Регистрационный номер Обязательства</div>
						<div>
							<xsl:apply-templates select="od:AppRegNum">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="od:Resident">
						<div class="title marg-top">Резидент особой  экономической зоны или участник свободной (особой) экономической зоны</div>
						<div>
							<xsl:apply-templates select="od:Resident">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Решение по Обязательству</td>
							<td class="value graphMain" style="width:50%">
								<xsl:choose>
									<xsl:when test="od:DecisionSign='1'">
										<xsl:text>Обязательство принято</xsl:text>
									</xsl:when>
									<xsl:when test="od:DecisionSign='2'">
										<xsl:text>Обязателство не принято</xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="od:DecisionSign"/>
									</xsl:otherwise>
								</xsl:choose>
							</td>
						</tr>
					</table>
					<xsl:if test="od:RefusalReasons">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Причины отказа в приеме Обязательства </td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="od:RefusalReasons"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="od:Actions">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Действия, которые необходимо совершить </td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="od:Actions"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="od:DecisionDocumentsBody">
						<div class="title marg-top">Документы, приложенные к решению</div>
						<table class="bordered">
							<tbody>
								<tr class="title">
									<td class="graphMain bordered">Имя файла</td>
								</tr>
								<xsl:for-each select="od:DecisionDocumentsBody">
					...
					<xsl:if test="od:Notes">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Примечания</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="od:Notes"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="od:CustomsPerson">
						<div class="title marg-top">Сведения об уполномоченном должностном лице таможенного органа</div>
						<div>
							<xsl:apply-templates select="od:CustomsPerson">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Дата Решения по Обязательству </td>
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="od:RefusalDate" mode="russian_date"/>
							</td>
						</tr>
					</table>
				</div>
	...
	<!-- Шаблон для типа od:RegNumType -->
	<xsl:template match="od:AppRegNum">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="od:CustomsCode"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="od:RegistrationDate" mode="russian_date"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="od:Number"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="od:AddNumber"/>
					</td>
				</tr>
			</table>
		</div>
	...
	<xsl:template match="od:CustomsPerson">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<!-- Шаблон для типа od:MessageDocBodyType -->
	<xsl:template match="od:DecisionDocumentsBody">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<tr>
				<td class="graphMain bordered">
					<xsl:apply-templates select="od:FileName"/>
				</td>
			</tr>
		</div>
	</xsl:template>
	<!-- Шаблон для типа od:ResidentType -->
	<xsl:template match="od:Resident">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="od:ResidentCertificate">
				<div class="title marg-top">Cвидетельство о включении в реестр резидентов</div>
				<div>
					<xsl:apply-templates select="od:ResidentCertificate">
						<xsl:with-param name="wdth" select="$w"/>
					</xsl:apply-templates>
				</div>
			</xsl:if>
	...
	<!-- Шаблон для типа od:ResidentCertificateType -->
	<xsl:template match="od:ResidentCertificate">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="od:CertificateNumber"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="od:CertificateDate" mode="russian_date"/>
					</td>
				</tr>
			</table>
			<xsl:if test="od:CertificateValidity">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Срок действия</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="od:CertificateValidity" mode="russian_date"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="od:CertificateAgency">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Наименование органа, выдавшего свидетельство</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="od:CertificateAgency"/>
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<xsl:template match="//*[local-name()='STZ_ObligDecisionCCZ']//*" priority="-1">
