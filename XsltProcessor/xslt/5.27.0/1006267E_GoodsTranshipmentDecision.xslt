...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.27.0" xmlns:RUSclt_ru="urn:customs.ru:RUSCommonLeafTypes:5.21.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:catpi_ru="urn:customs.ru:Information:PriorInformation:PriorCommonAggregateTypes:5.27.0" xmlns:gtshd="urn:customs.ru:Information:CustomsDocuments:GoodsTranshipmentDecision:5.27.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
								...
								<td align="center" class="title">
									<font size="4">
										...
										<xsl:choose>
											...
											<xsl:when test="gtshd:DocType=4">
												<xsl:text>Решение о выдаче разрешения на изменение маршрута перевозки (транспортировки) товаров</xsl:text>
											</xsl:when>
											<xsl:when test="gtshd:DocType=5">
												<xsl:text>Решение об отказе в выдаче разрешения на изменение маршрута перевозки (транспортировки) товаров</xsl:text>
											</xsl:when>
											<xsl:otherwise>
					...
					<xsl:if test="gtshd:TransitRoute">
						<br/>
						...
						<table class="graphMain border">
							<tbody>
								...
								<xsl:for-each select="gtshd:TransitRoute">
	...
	<!-- Шаблон для типа gtshd:CargoInformationType -->
	<xsl:template match="gtshd:CargoInformation">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="gtshd:TranspDoc">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Транспортный документ</td>
						<td class="value graphMain" style="width:50%">
							<xsl:for-each select="gtshd:TranspDoc">
	...
	<!-- Шаблон для типа RUScat_ru:SubjectDetailsType -->
	<xsl:template match="gtshd:Carrier | gtshd:NextCarrier">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<xsl:if test="cat_ru:OrganizationName">
			...
			<xsl:if test="gtshd:CarrierNumber">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Порядковый номер перевозчика</td>
						<td class="value graphMain" style="width:50%">
							<xsl:value-of select="gtshd:CarrierNumber"/>
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<xsl:template match="gtshd:IncidentInfo">
		...
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="gtshd:IncidentActNumber">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Реквизиты акта об аварии</td>
						<td class="value graphMain" style="width:50%">
							<xsl:for-each select="gtshd:IncidentActNumber">
								<xsl:value-of select="cat_ru:PrDocumentName"/>
								<xsl:if test="cat_ru:PrDocumentNumber">
									<xsl:text> № </xsl:text>
									<xsl:value-of select="cat_ru:PrDocumentNumber"/>
			...
			<xsl:if test="gtshd:CargoInformation">
				<div class="title marg-top">Сведения о грузе</div>
				<div>
					<xsl:apply-templates select="gtshd:CargoInformation">
						<xsl:with-param name="wdth" select="$w"/>
					</xsl:apply-templates>
				</div>
			</xsl:if>
			<xsl:if test="gtshd:Seal">
				<div class="title marg-top">Информация о нарушенных пломбах, печатях и иных средствах идентификации</div>
				<table class="bordered">
					<tbody>
						<tr class="title">
							<td class="graphMain bordered">Код способа идентификации</td>
			...
			<tr>
				...
				<td class="graphMain bordered">
					<xsl:choose>
						...
						<xsl:when test="gtshd:AbsenceKind=4">
							<xsl:text>Поврежден</xsl:text>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template match="catESAD_cu:PackagePalleteInformation">
		...
		<!--div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;"-->
			<xsl:if test="catESAD_cu:InfoKindCode">
				<xsl:choose>
					<xsl:when test="catESAD_cu:InfoKindCode='0'">
						<xsl:text>сведения об упаковке</xsl:text>
					</xsl:when>
					<xsl:when test="catESAD_cu:InfoKindCode='1'">
						<xsl:text>сведения о таре</xsl:text>
					</xsl:when>
					<xsl:when test="catESAD_cu:InfoKindCode='2'">
						<xsl:text>сведения о грузе</xsl:text>
					</xsl:when>
					<xsl:when test="catESAD_cu:InfoKindCode='3'">
						<xsl:text>сведения о поддонах</xsl:text>
					</xsl:when>
					<xsl:when test="catESAD_cu:InfoKindCode='4'">
						<xsl:text>сведения об индивидуальной упаковке</xsl:text>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="gtshd:Seal">
		...
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<tr>
				<td class="graphMain bordered">
					<xsl:value-of select="gtshd:CustomsIdentificationMethodCode"/>
				</td>
				<td class="graphMain bordered">
					<xsl:choose>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='01'">Пломбы</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='02'">Навигационные пломбы</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='03'">Печати</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='04'">Цифровая, буквенная или иная маркировка</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='05'">Идентификационные знаки</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='06'">Штампы</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='07'">Сейф-пакеты</xsl:when>
						<xsl:when test="gtshd:CustomsIdentificationMeansKindCode='99'">Иные средства, обеспечивающие идентификацию</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="gtshd:CustomsIdentificationMeansKindCode"/>
						</xsl:otherwise>
					</xsl:choose>
				</td>
				<td class="graphMain bordered">
					<xsl:value-of select="gtshd:SealQuantity"/>
				</td>
				<td class="graphMain bordered">
					<xsl:for-each select="gtshd:CustomsIdentificationMeansIdDetails">
						<xsl:if test="position()!=1">
							<br/><br/>
						</xsl:if>
						...
						<br/>
						<xsl:if test="gtshd:CustomsIdentificationMeansId">
							<xsl:text>Уникальный идентификатор (номер) средства идентификации - </xsl:text>
							<xsl:value-of select="gtshd:CustomsIdentificationMeansId"/>
							<xsl:text>; </xsl:text>
						</xsl:if>
						<xsl:if test="gtshd:DescriptionText">
							<xsl:text>Описание отличительных признаков - </xsl:text>
							<xsl:value-of select="gtshd:DescriptionText"/>
							<xsl:text>; </xsl:text>
						</xsl:if>
						<xsl:if test="gtshd:ForeignCustomsIdentificationMeansIndicator">
							<xsl:text>Признак признания таможенным органом средства идентификации - </xsl:text>
							<xsl:if test="gtshd:ForeignCustomsIdentificationMeansIndicator='0' or gtshd:ForeignCustomsIdentificationMeansIndicator='false' or gtshd:ForeignCustomsIdentificationMeansIndicator='f'">
								<xsl:text>Нет</xsl:text>
							</xsl:if>
							<xsl:if test="gtshd:ForeignCustomsIdentificationMeansIndicator='1' or gtshd:ForeignCustomsIdentificationMeansIndicator='true' or gtshd:ForeignCustomsIdentificationMeansIndicator='t'">
				...
				<td class="graphMain bordered">
					<xsl:for-each select="gtshd:DescriptionText">
						<xsl:if test="position()!=1">
							<xsl:text>; </xsl:text>
						</xsl:if>
						<xsl:value-of select="."/>
