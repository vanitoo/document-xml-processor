					...
					<table class="w270 border" cellpadding="5">
						...
						<thead>
							...
							<tr valign="middle" align="center" class="italic">
								...
								<td rowspan="2">Вес брутто/ вес нетто</td>
							...
							<tbody class="normal">
								<tr>
									...
									<td>
										...
										<!-- Добавлено 01.10.2025 из ESADout_CU -->
										<xsl:if test="argb:DTIdentificationMeansDetails/catESAD_cu:IdentificationMeansDetails">
											<br/>
											<xsl:text> Маркировка: </xsl:text>
											<xsl:for-each select="argb:DTIdentificationMeansDetails/catESAD_cu:IdentificationMeansDetails">
									...
									<td align="right">
										...
										<xsl:if test="argb:UnifiedNetMassMeasure and not(argb:UnifiedGrossMassMeasure)">
											<xsl:variable name="xpath_UnifiedNetMassMeasure1">
												<xsl:call-template name="get_xpath">
													<xsl:with-param name="node" select="argb:UnifiedNetMassMeasure"/>
												</xsl:call-template>
											</xsl:variable>
											<element xml_node="{$xpath_UnifiedNetMassMeasure1}">
												<xsl:choose>
													<xsl:when test="argb:UnifiedNetMassMeasure &gt; 0.001">
														<xsl:value-of select="translate(translate(format-number(argb:UnifiedNetMassMeasure,'0.000'), ',', ' '), '.', ',')"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="translate(translate(format-number(argb:UnifiedNetMassMeasure,'0.000000'), ',', ' '), '.', ',')"/>
													</xsl:otherwise>
												</xsl:choose>
											</element>
										</xsl:if>
	...
	<xsl:template name="argb:PresentedDocDetails_table">
		<xsl:copy>
			<xsl:for-each select="argb:GoodsShipmentDetails/argb:GoodsItemDetails/argb:PresentedDocDetails[generate-id(key('rows_Presented',.)[1])=generate-id()]">
				<xsl:sort select="RUDECLcat:DocKindCode"/>
