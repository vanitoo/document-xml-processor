			...
			<body>
				<div class="page">
					<table>
								...
								<font face="Times new roman" size="4">
									<b>
										<xsl:choose>
											...
											<xsl:when test="grn_cl:DocumentSign='12'">Освобождение от представления обеспечения</xsl:when>
											<xsl:when test="grn_cl:DocumentSign='13'">Информирование о завершении контроля по несостоявшемуся резервированию</xsl:when>
						...
						<xsl:for-each select="grn_cl:Reason">
							<tr>
								<td class="graph">Причина, по которой завершено использование обеспечения</td>
								<td class="graph">
									<xsl:value-of select="grn_cl:Name"/> <xsl:value-of select="grn_cl:Comment"/>
									<xsl:if test="grn_cl:DecisionCode">
										, код решения: <xsl:value-of select="grn_cl:DecisionCode"/>
									</xsl:if>
									<xsl:if test="grn_cl:DateClose">
										<br/>дата завершения использования обеспечения: 
										<xsl:call-template name="russian_date">
											<xsl:with-param name="dateIn" select="grn_cl:DateClose"/>
										</xsl:call-template>
									</xsl:if>
									<xsl:if test="grn_cl:ReasonClose">
										<br/>причина закрытия  сертификата: <xsl:choose>
											<xsl:when test="grn_cl:ReasonClose=1">Завершение транзита</xsl:when>
											<xsl:when test="grn_cl:ReasonClose=2">Погашение сертификата в связи с его неиспользованием</xsl:when>
											<xsl:when test="grn_cl:ReasonClose=3">Погашение сертификата в связи с недоставкой товара</xsl:when>
											<xsl:when test="grn_cl:ReasonClose=4">Прочее</xsl:when>
											<xsl:when test="grn_cl:ReasonClose=5">Аннулирование в связи с технической ошибкой</xsl:when>
											<xsl:otherwise>
												<xsl:value-of select="grn_cl:ReasonClose"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:if>
									<xsl:for-each select="grn_cl:ReasonCloseTempTrans">
										<br/>причина закрытия обеспечения: 
										<xsl:choose>
											<xsl:when test=". = 1">Вывоз с территории ЕАЭС</xsl:when>
											<xsl:when test=". = 2">Выпуск в свободное обращение</xsl:when>
											<xsl:otherwise>
												<xsl:value-of select="."/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:for-each>
									<xsl:for-each select="grn_cl:ReasonDocument">
										<br/>документ-основание: 
										...
										<xsl:text> </xsl:text>
										<xsl:choose>
											<xsl:when test="grn_cl:DocumentKind = 0">заявление о выдаче/приобретении акцизных марок</xsl:when>
											<xsl:when test="grn_cl:DocumentKind = 1">ДТ/ЗВ</xsl:when>
											<xsl:when test="grn_cl:DocumentKind = 2">ТД</xsl:when>
											<xsl:when test="grn_cl:DocumentKind = 5">ТПО</xsl:when>
											<xsl:when test="grn_cl:DocumentKind = 16">Пассажирская декларация</xsl:when>
										</xsl:choose>
									</xsl:for-each>
								</td>
							</tr>
