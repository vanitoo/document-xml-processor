					...
					<table width="100%">
						<tr>
							<td class="t">
                                ...
                                Всего т-ов
                                <br/>
								<xsl:apply-templates select="KDTout:KTDoutTotalGoodsNumber"/>
								<xsl:if test="KDTout:KTDoutTotalGoodsNumber and catESAD_cu:TotalGoodsNumber">/</xsl:if>
						...
						<tr>
							<td class="green" width="34.9%">
								...
								<!--xsl:for-each select="KDTout:KTDoutConsigment/KDTout:KDToutDepartureArrivalTransport/KDTout:TransportMeans[not(cat_ru:ActiveTransportIdentifier)]"-->
										<xsl:for-each select="KDTout:KTDoutConsigment/KDTout:KDToutDepartureArrivalTransport/KDTout:RUTransportMeans">
											<xsl:variable name="currentTransportCode" select="cat_ru:TransportKindCode"/>
											<xsl:variable name="nextTransportCode" select="following-sibling::KDTout:RUTransportMeans[1]/cat_ru:TransportKindCode"/>
							...
							<td class="green" width="44.5%">
								<xsl:choose>
									<xsl:when test="KDTout:KTDoutConsigment/KDTout:KDToutBorderTransport">
										...
										<xsl:for-each select="KDTout:KTDoutConsigment/KDTout:KDToutBorderTransport/KDTout:RUTransportMeans">
											<xsl:variable name="currentTransportCode" select="cat_ru:TransportKindCode"/>
											<xsl:variable name="nextTransportCode" select="following-sibling::KDTout:RUTransportMeans[1]/cat_ru:TransportKindCode"/>
											<xsl:variable name="codeList" select="'201 202 203 204 205 206 301 302 303 304 305 306 307'"/>
											<xsl:if test="$currentTransportCode and not(contains($codeList, $currentTransportCode)) and position() != 1">
			...
			<tr>
				<td colspan="2" rowspan="3" style="border-bottom:solid 0.8pt black;background:#FFFFFF;">
					...
					<xsl:if test="catESAD_cu:WoodCodeDetails">
						...
						<xsl:for-each select="catESAD_cu:WoodCodeDetails">
							...
							<xsl:if test="catESAD_cu:ItemQuantity">
								...
								<xsl:if test="catESAD_cu:LabelId">
									<xsl:text> Бирок: </xsl:text>
                  ...
                  <xsl:apply-templates select="$goods/KDTout:SupplementaryGoodsQuantity/cat_ru:MeasureUnitQualifierCode"/>
					...
					<xsl:if test="$goods/KDTout:DestinationCountryDetails[catESAD_cu:CountryName]">
						<br/>
						<span class="graph">14) </span>
						<xsl:for-each select="$goods/KDTout:DestinationCountryDetails/catESAD_cu:CountryName">
							<xsl:if test="position()!=1">, </xsl:if>
							<xsl:apply-templates select="."/>
						</xsl:for-each>
					</xsl:if>
		...
		<table>
			<tbody>
				<tr>
					<td style="width:45mm;">
						<xsl:choose>
							<xsl:when test="KDTout:RFOrganizationFeatures/cat_ru:INN">
								<xsl:apply-templates select="KDTout:RFOrganizationFeatures/cat_ru:INN"/>
							</xsl:when>
							<xsl:when test="KDTout:UITN/RUScat_ru:UITNCode">
								<xsl:apply-templates select="KDTout:UITN/RUScat_ru:UITNCode"/>
							</xsl:when>
							<xsl:when test="KDTout:IdentityCard">
						...
						<xsl:text> сумма уплаты/возврата: </xsl:text>
