...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:rd="urn:customs.ru:Information:ExchangeDocuments:STZ_RefusalDecisionCCZ:5.24.0">
	<!-- Шаблон для типа STZ_RefusalDecisionCCZType -->
	<xsl:template match="rd:STZ_RefusalDecisionCCZ">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Сообщения об отказе в выдаче Решения о создании постоянной зоны таможенного контроля на участке СТЗ/ОЭЗ</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="rd:AppRegNum">
						<div class="title marg-top">Регистрационный номер Заявления</div>
						<div>
							<xsl:apply-templates select="rd:AppRegNum">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Дата отказа</td>
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="rd:RefusalDate" mode="russian_date" />
							</td>
						</tr>
					</table>
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Обоснование отказа в выдаче Решения</td>
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="rd:RefusalReasons" />
							</td>
						</tr>
					</table>
					<xsl:if test="rd:CustomsPerson">
						<div class="title marg-top">Сведения об уполномоченном должностном лице таможенного органа</div>
						<div>
							<xsl:apply-templates select="rd:CustomsPerson">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<!-- Шаблон для типа rd:RegNumType -->
	<xsl:template match="rd:AppRegNum">
		...
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="rd:Number" />
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="rd:AddNumber" />
					</td>
				</tr>
			</table>
		</div>
	...
	<xsl:template match="rd:CustomsPerson">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<xsl:template match="//*[local-name()='STZ_RefusalDecisionCCZ']//*" priority="-1">
