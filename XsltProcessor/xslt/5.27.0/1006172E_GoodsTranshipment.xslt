...
<xsl:stylesheet version="1.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.27.0" xmlns:RUSclt_ru="urn:customs.ru:RUSCommonLeafTypes:5.21.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:catpi_ru="urn:customs.ru:Information:PriorInformation:PriorCommonAggregateTypes:5.27.0" xmlns:gtsh="urn:customs.ru:Information:CustomsDocuments:GoodsTranshipment:5.27.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<html>
			...
			<body>
				<div class="page">
					<h1 align="center" style="font: 15pt; font-weight:bold">
						...
						<element xml_node="{$xpath_DocType}">
							<xsl:choose>
								...
								<xsl:when test="gtsh:DocType='3'">Уведомление об изменении маршрута перевозки (транспортировки) товаров</xsl:when>
							</xsl:choose>
						</element>
					</h1>
					...
					<xsl:if test="gtsh:Carrier or (gtsh:CarrierEqualFlag = '1' or gtsh:CarrierEqualFlag = 'true' or gtsh:CarrierEqualFlag = 't' or gtsh:CarrierEqualFlag = 'TRUE')">
						<br/>
						<table>
							<tbody>
								<tr>
								...
								<tr>
									<td class="graphMargin value" align="left">
										<xsl:choose>
											<xsl:when test="gtsh:CarrierEqualFlag = '1' or gtsh:CarrierEqualFlag = 'true' or gtsh:CarrierEqualFlag = 't' or gtsh:CarrierEqualFlag = 'TRUE'">
					...
					<xsl:if test="gtsh:Seal">
						...
						<table class="graphMain border">
							<tbody>
								<tr>
									<th align="center">Код способа идентификации</th>
									<th align="center">Вид средств идентификации</th>
									<th align="center">Количество средств идентификации</th>
									<th align="center">Сведения о средстве идентификации</th>
									<th align="center">Описание идентификации</th>
								</tr>
								<xsl:apply-templates select="gtsh:Seal"/>
							</tbody>
						</table>
					</xsl:if>
					<xsl:if test="gtsh:TransitRoute">
						<br/>
						<b>Информация о новом маршруте: </b>
						<table class="graphMain border">
							<tbody>
								<tr>
									<th align="center">Порядковый номер точки транзита</th>
									<th align="center">Наименование точки маршрута</th>
								</tr>
								<xsl:for-each select="gtsh:TransitRoute">
									<xsl:sort select="catpi_ru:TransitPointNumber" case-order="lower-first" data-type="number" order="ascending"/>
									<tr>
										<td>
											<xsl:apply-templates select="catpi_ru:TransitPointNumber"/>
										</td>
										<td>
											<xsl:apply-templates select="catpi_ru:TransitRoutePlace"/>
										</td>
									</tr>
								</xsl:for-each>
							</tbody>
	...
	<xsl:template match="gtsh:IncidentInfo">
		...
		<table style="margin-top:0;">
			<tbody>
				...
				<xsl:if test="gtsh:CargoInformation">
					<table style="margin-top:0;">
						<tbody>
							<tr class="title marg-top">
								<td>Сведения о грузе</td>
							</tr>
						</tbody>
					</table>
					<table style="margin-top:0;">
						<tbody>
							<xsl:apply-templates select="gtsh:CargoInformation"/>
						</tbody>
					</table>
				</xsl:if>
				<xsl:if test="gtsh:Seal">
					<table style="margin-top:0;">
						<tbody>
							<tr class="title marg-top">
								<td>Информация о нарушенных пломбах, печатях и иных средствах идентификации</td>
							</tr>
						</tbody>
					</table>
					<table class="bordered">
						<tbody>
							<tr class="title">
								<td class="graphMain bordered">Код способа идентификации</td>
								<td class="graphMain bordered">Вид средств идентификации</td>
								<td class="graphMain bordered">Количество средств идентификации</td>
								<td class="graphMain bordered">Сведения о средстве идентификации</td>
								<td class="graphMain bordered">Описание идентификации</td>
							</tr>
							<xsl:apply-templates select="gtsh:Seal"/>
						</tbody>
					</table>
	...
	<!-- Шаблон для типа gtsh:CargoInformationType -->
	<xsl:template match="gtsh:CargoInformation">
		<xsl:if test="gtsh:DamageIndicators">
			<xsl:apply-templates select="gtsh:DamageIndicators"/>
		</xsl:if>
		<xsl:if test="gtsh:TranspDoc">
			<tr>
				<td class="graphMargin" valign="bottom" width="40%">Транспортный документ</td>
				<td align="center" class="graphMargin value" valign="bottom">
					<xsl:for-each select="gtsh:TranspDoc">
		...
		<xsl:if test="gtsh:LostGoodsInfo">
			<table style="margin-top:0;">
				<tbody>
					<tr class="title marg-top">
						<td>Информация об утрате (отсутствии/уничтожении) части товара</td>
					</tr>
				</tbody>
			</table>
			<table class="bordered">
				<tbody>
					<tr class="title">
						...
						<td class="graphMain bordered">Сведения об утраченных упаковках</td>
						<td class="graphMain bordered">Количество утраченной части товара</td>
					</tr>
	...
	<xsl:template match="gtsh:LostGoodsInfo">
		<tr>
				...
				<td class="graphMain bordered">
					...
					<element xml_node="{$xpath_AbsenceKind}">
						<xsl:choose>
							...
							<xsl:when test="gtsh:AbsenceKind = '4'">Поврежден</xsl:when>
							<xsl:otherwise>
								<xsl:apply-templates select="gtsh:AbsenceKind"/>
							</xsl:otherwise>
						</xsl:choose>
					</element>
				</td>
			...
			<xsl:if test="gtsh:LostGoodsQuantity">
				<td class="graphMain bordered">
					<xsl:apply-templates select="gtsh:LostGoodsQuantity/cat_ru:GoodsQuantity"/>
					<xsl:text> </xsl:text>
					<xsl:if test="gtsh:LostGoodsQuantity/cat_ru:MeasureUnitQualifierName">
						<xsl:apply-templates select="gtsh:LostGoodsQuantity/cat_ru:MeasureUnitQualifierName"/>
					</xsl:if>	
				</td>
			</xsl:if>
		</tr>
	...
	<xsl:template match="gtsh:Seal">
		<tr>
			<td class="graphMain bordered">
				<xsl:value-of select="gtsh:CustomsIdentificationMethodCode"/>
			</td>
			<td class="graphMain bordered">
				<xsl:choose>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='01'">Пломбы</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='02'">Навигационные пломбы</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='03'">Печати</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='04'">Цифровая, буквенная или иная маркировка</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='05'">Идентификационные знаки</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='06'">Штампы</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='07'">Сейф-пакеты</xsl:when>
					<xsl:when test="gtsh:CustomsIdentificationMeansKindCode='99'">Иные средства, обеспечивающие идентификацию</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="gtsh:CustomsIdentificationMeansKindCode"/>
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td class="graphMain bordered">
				<xsl:value-of select="gtsh:SealQuantity"/>
			</td>
			<td class="graphMain bordered">
				<xsl:for-each select="gtsh:CustomsIdentificationMeansIdDetails">
					<xsl:if test="position()!=1">
						<br/><br/>
					</xsl:if>
					<xsl:text>Сведения о средстве идентификации № </xsl:text>
					<xsl:value-of select="position()"/>
					<br/>
					<xsl:if test="gtsh:CustomsIdentificationMeansId">
						<xsl:text>Уникальный идентификатор (номер) средства идентификации - </xsl:text>
						<xsl:value-of select="gtsh:CustomsIdentificationMeansId"/>
						<xsl:text>; </xsl:text>
					</xsl:if>
					<xsl:if test="gtsh:DescriptionText">
						<xsl:text>Описание отличительных признаков - </xsl:text>
						<xsl:value-of select="gtsh:DescriptionText"/>
						<xsl:text>; </xsl:text>
					</xsl:if>
					<xsl:if test="gtsh:ForeignCustomsIdentificationMeansIndicator">
						<xsl:text>Признак признания таможенным органом средства идентификации - </xsl:text>
						<xsl:if test="gtsh:ForeignCustomsIdentificationMeansIndicator='0' or gtsh:ForeignCustomsIdentificationMeansIndicator='false' or gtsh:ForeignCustomsIdentificationMeansIndicator='f'">
							<xsl:text>Нет</xsl:text>
						</xsl:if>
						<xsl:if test="gtsh:ForeignCustomsIdentificationMeansIndicator='1' or gtsh:ForeignCustomsIdentificationMeansIndicator='true' or gtsh:ForeignCustomsIdentificationMeansIndicator='t'">
			...
			<td class="graphMain bordered">
				<xsl:for-each select="gtsh:DescriptionText">
					<xsl:if test="position()!=1">
						<xsl:text>; </xsl:text>
					</xsl:if>
					<xsl:value-of select="."/>
	...
	<xsl:template match="gtsh:LostPackages">
		<xsl:if test="catESAD_cu:PakageQuantity">
			<xsl:text>Общее количество грузовых мест, занятых товаром: </xsl:text>
			<xsl:value-of select="catESAD_cu:PakageQuantity"/>
		</xsl:if>
		<xsl:if test="catESAD_cu:PakageTypeCode">
			<br/>
			<xsl:text> Код наличия упаковки товара: </xsl:text>
			<xsl:choose>
		...
		<xsl:if test="catESAD_cu:PakagePartQuantity">
			<br/>
			<xsl:text>Количество грузовых мест, частично занятых товаром: </xsl:text>
		...
		<xsl:if test="catESAD_cu:PackagePalleteInformation">
			...
			<xsl:apply-templates select="catESAD_cu:PackagePalleteInformation"/>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="*" mode="org">
		...
		<xsl:if test="gtsh:CarrierNumber">
			<br/>
			<span class="italic">Порядковый номер перевозчика</span>
			<span class="normal">: </span>
			<xsl:apply-templates select="gtsh:CarrierNumber"/>
		</xsl:if>
	</xsl:template>
