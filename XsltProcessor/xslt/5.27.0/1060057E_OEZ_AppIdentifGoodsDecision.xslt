              ...
              <tr>
                ...
                <td class="value graphMain" style="width:50%">
                  <xsl:apply-templates select="cat_ru:MCD_ID" />
					...
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%; font-weight: bold;">Результат рассмотрения Заявления</td>
							<td class="value graphMain" style="width:50%; font-size: 10pt;">
								<!--xsl:choose>
								...
								<xsl:if test="oagd:DecisionAppIdentif[oagd:GoodsDecision = 1]">
									<p style="font-weight: bold;">
										<xsl:text>Идентификация проведена. Товар(ы) Евразийского экономического союза </xsl:text>
										<span style="display: inline-block; border-bottom: 1px solid black;">
											<xsl:for-each select="oagd:DecisionAppIdentif[oagd:GoodsDecision = 1]">
												<xsl:if test="position() != 1"><xsl:text>, </xsl:text></xsl:if>
												<xsl:apply-templates select="oagd:GoodsNumber"/>
											</xsl:for-each>
										</span>
										<xsl:text> ввезен(ы) в ОЭЗ, акт таможенного досмотра (осмотра) </xsl:text>
										<xsl:apply-templates select="oagd:ActNumber"/>
										
									</p>
								</xsl:if>
								<xsl:if test="oagd:DecisionAppIdentif[oagd:GoodsDecision = 2]">
									<p style="font-weight: bold;">
										<xsl:text>В идентификации товара(ов) отказано </xsl:text>
										<span style="display: inline-block; border-bottom: 1px solid black;">
											<xsl:for-each select="oagd:DecisionAppIdentif[oagd:GoodsDecision = 2]">
												<xsl:if test="position() != 1"><xsl:text>, </xsl:text></xsl:if>
												<xsl:apply-templates select="oagd:GoodsNumber"/>
											</xsl:for-each>
										</span>
										<xsl:text>, акт таможенного (досмотра) осмотра  </xsl:text>
										<xsl:apply-templates select="oagd:ActNumber"/>
