...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:stz_apcp="urn:customs.ru:Information:ExchangeDocuments:STZ_AppProlongOrCancelPermit:5.27.0">
	<!-- Шаблон для типа STZ_AppProlongOrCancelPermitType -->
	<xsl:template match="stz_apcp:STZ_AppProlongOrCancelPermit">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Заявление о продлении срока или аннулировании  разрешения таможенного органа на совершение операций, предусмотренных подпунктами 1, 2 и 5 пункта 4 статьи 205 Таможенного кодекса Евразийского экономического союза</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="stz_apcp:Customs">
						<div class="title marg-top">Сведения о таможенном органе</div>
						<div>
							<xsl:apply-templates select="stz_apcp:Customs">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="stz_apcp:Resident">
						...
						<div>
							<xsl:apply-templates select="stz_apcp:Resident">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<table>
						<tr>
							...
							<td class="value graphMain" style="width:50%">
								<xsl:choose>
									<xsl:when test="stz_apcp:Operation='1'">
										<xsl:text>аннулирование разрешения</xsl:text>
									</xsl:when>
									<xsl:when test="stz_apcp:Operation='2'">
										<xsl:text>продление срока действия разрешения</xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="stz_apcp:Operation"/>
									</xsl:otherwise>
								</xsl:choose>
							</td>
						</tr>
					</table>
					<xsl:if test="stz_apcp:AppRegNumber">
						<div class="title marg-top">Регистрационный номер Заявления</div>
						<div>
							<xsl:apply-templates select="stz_apcp:AppRegNumber">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="stz_apcp:PermitRegNumber">
						<div class="title marg-top">Регистрационный номер полученного Разрешения таможенного органа</div>
						<div>
							<xsl:apply-templates select="stz_apcp:PermitRegNumber">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="stz_apcp:CancelReason">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Причина аннулирования</td>
								<td class="graphMain value" style="width:50%">
									<xsl:for-each select="stz_apcp:CancelReason">
										<xsl:value-of select="."/>
					...
					<xsl:if test="stz_apcp:ProlongReason">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Причина продления</td>
								<td class="graphMain value" style="width:50%">
									<xsl:for-each select="stz_apcp:ProlongReason">
										<xsl:value-of select="."/>
										<xsl:if test="position()!=last()">
											<xsl:text>, </xsl:text>
										</xsl:if>
					...
					<xsl:if test="stz_apcp:ExtImpTerm">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Срок, до которого продляется срок вывоза товаров</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="stz_apcp:ExtImpTerm" mode="russian_date"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="stz_apcp:Notes">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Примечание</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="stz_apcp:Notes"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="stz_apcp:PresentedDoc">
						...
						<table class="bordered">
							<tbody>
								...
								<xsl:for-each select="stz_apcp:PresentedDoc">
					...
					<xsl:if test="stz_apcp:ResidentSignature">
						<div class="title marg-top">Лицо, подающее заявление</div>
						<div>
							<xsl:apply-templates select="stz_apcp:ResidentSignature">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<!-- Шаблон для типа stz_apcp:RegistrationNumberType -->
	<xsl:template match="stz_apcp:AppRegNumber">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_apcp:CustomsCode"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_apcp:RegistrationDate" mode="russian_date"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_apcp:Number"/>
					</td>
				</tr>
			</table>
			<xsl:if test="stz_apcp:DocKind">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Четвертая часть регистрационного номера документа</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_apcp:DocKind"/>
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<xsl:template match="stz_apcp:Customs">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<!-- Шаблон для типа stz_apcp:RegistrationNumberType -->
	<xsl:template match="stz_apcp:PermitRegNumber">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<!-- Шаблон для типа stz_apcp:PresentedDocType -->
	<xsl:template match="stz_apcp:PresentedDoc">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<tr>
				...
				<td class="graphMain bordered">
					<xsl:apply-templates select="stz_apcp:Position"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="stz_apcp:SheetsNumber"/>
				</td>
			</tr>
		</div>
	</xsl:template>
	<!-- Шаблон для типа stz_apcp:ResidentType -->
	<xsl:template match="stz_apcp:Resident">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="stz_apcp:ResidentCertificate">
				<div class="title marg-top">Cвидетельство о включении в реестр резидентов</div>
				<div>
					<xsl:apply-templates select="stz_apcp:ResidentCertificate">
						<xsl:with-param name="wdth" select="$w"/>
					</xsl:apply-templates>
				</div>
			</xsl:if>
	...
	<!-- Шаблон для типа stz_apcp:ResidentCertificateType -->
	<xsl:template match="stz_apcp:ResidentCertificate">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_apcp:CertificateNumber"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_apcp:CertificateDate" mode="russian_date"/>
					</td>
				</tr>
			</table>
			<xsl:if test="stz_apcp:CertificateValidity">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Срок действия</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_apcp:CertificateValidity" mode="russian_date"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="stz_apcp:CertificateAgency">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Наименование органа, выдавшего свидетельство</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_apcp:CertificateAgency"/>
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<xsl:template match="stz_apcp:ResidentSignature">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<xsl:template match="//*[local-name()='STZ_AppProlongOrCancelPermit']//*" priority="-1">
