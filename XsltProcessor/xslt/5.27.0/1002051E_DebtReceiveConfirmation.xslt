...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:drc="urn:customs.ru:Information:CommercialFinanceDocuments:DebtReceiveConfirmation:5.24.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:template match="drc:Address">
	...
	<xsl:template match="drc:Contact">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<xsl:template match="drc:RecieverInfo">
		...
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="drc:OKPOID">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Код организации по ОКПО</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="drc:OKPOID"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="drc:OKATOCode">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Код ОКАТО</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="drc:OKATOCode"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="drc:Address">
				<div class="title marg-top">Адрес </div>
				<div>
					<xsl:apply-templates select="drc:Address">
						<xsl:with-param name="wdth" select="$w"/>
					</xsl:apply-templates>
				</div>
			</xsl:if>
			<xsl:if test="drc:Contact">
				<div class="title marg-top">Контактная информация</div>
				<div>
					<xsl:apply-templates select="drc:Contact">
						<xsl:with-param name="wdth" select="$w"/>
					</xsl:apply-templates>
				</div>
			</xsl:if>
