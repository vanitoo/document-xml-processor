...
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:nad="urn:customs.ru:Information:CommercialFinanceDocuments:NotifAdvDep:5.27.0" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.27.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0">
	...
	<xsl:template mode="organization" match="*">
		...
		<xsl:if test="nad:OKPOID">,&#032;ОКПО:&#032;<xsl:value-of select="nad:OKPOID"/>
		</xsl:if>
		<xsl:if test="nad:ITN">,&#032;ИТН:&#032;<xsl:value-of select="nad:ITN"/>
		</xsl:if>
		<xsl:if test="nad:Address">,&#032;Адрес:&#032;<xsl:for-each select="nad:Address">
				<xsl:choose>
		...
		<xsl:if test="nad:OKATOCode">,&#032;ОКАТО:&#032;<xsl:value-of select="nad:OKATOCode"/>
		</xsl:if>
		<xsl:if test="nad:Contact">,&#032;<xsl:for-each select="nad:Contact">
