...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.27.0" xmlns:catESAD_ru="urn:customs.ru:RUCommonAggregateTypes:5.27.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:goom="urn:customs.ru:Information:CustomsDocuments:GTDoutCustomsMark:5.27.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:output encoding="UTF-8" indent="yes" method="html" version="1.0"/>
	<xsl:variable name="showFoundationGoods" select="count(goom:GTDoutCustomsMark/goom:GTDOutGoodsResolution/goom:GTDOutGoodsResult[catESAD_ru:DecisionCode != 90]) &gt; 0"/>
	<xsl:template match="/">
		<xsl:apply-templates/>
	</xsl:template>
					...
					<table border="0" width="100%">
						<tbody>
							<tr class="graphBody" valign="top">
								...
								<xsl:if test=" goom:GTDOutResolution/catESAD_ru:Foundation and goom:GTDOutResolution/catESAD_ru:DecisionCode != 90">
					...
					<table border="1" cellspacing="0" width="100%">
						<thead>
							<tr class="graphTable">
								...
								<td align="center">Время</td>
								<xsl:if test="$showFoundationGoods = 'true'">
									<td align="center">Обоснование</td>
								</xsl:if>
					...
					<table border="1" cellspacing="0" width="100%">
						<thead>
							<tr class="graphTable">
								...
								<xsl:if test="//goom:GTDOutGoodsResolution/goom:GTDOutGoodsMark/catESAD_ru:Deadline">
									<td align="center">Дата / Срок сдачи</td>
								</xsl:if>
	...
	<xsl:template match="goom:GTDOutGoodsResult">
		...
		<xsl:if test="$showFoundationGoods = 'true'">
			<td>
				<xsl:choose>
					<xsl:when test="catESAD_ru:Foundation and catESAD_ru:DecisionCode != 90">
	...
	<xsl:template match="goom:FoundationDes">
		...
		<xsl:if test="goom:Recommendation">
			<br/>
			<xsl:text> Рекомендации по устранению: </xsl:text>
			<xsl:apply-templates select="goom:Recommendation"/>
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>
