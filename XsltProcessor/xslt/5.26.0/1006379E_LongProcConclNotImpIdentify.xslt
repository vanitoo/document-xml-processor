...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:lpcn_id="urn:customs.ru:Information:CustomsDocuments:LongProcConclNotImpIdentify:5.24.0">
	<!-- Шаблон для типа LongProcConclNotImpIdentifyType -->
	<xsl:template match="lpcn_id:LongProcConclNotImpIdentify">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Уведомление о невозможности идентификации товаров в продуктах их переработки</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="lpcn_id:Customs">
						<div class="title marg-top">Таможенный орган</div>
						<div>
							<xsl:apply-templates select="lpcn_id:Customs">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="lpcn_id:CurrentDT">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Номер ДТ, в которой заявлены продукты переработки</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="lpcn_id:CurrentDT" />
								</td>
							</tr>
						</table>
					</xsl:if>
					...
					<table class="bordered">
						<tbody>
							<tr class="title">
								<td class="graphMain bordered">Порядковый номер товара в ДТ, в которой заявлены продукты переработки</td>
								<td class="graphMain bordered">Регистрационный номер разрешения на переработку товаров</td>
							...
							<xsl:for-each select="lpcn_id:DeclaredGood">
								<xsl:apply-templates select="." />
							</xsl:for-each>
						</tbody>
					</table>
					<xsl:if test="lpcn_id:CustomsPerson">
						<div class="title marg-top">Подпись должностного лица</div>
						<div>
							<xsl:apply-templates select="lpcn_id:CustomsPerson">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<!-- Шаблон для типа cat_ru:GTDIDType и lpcn_id:AppGoodType -->
	<xsl:template match="lpcn_id:PermIdentifNumber | lpcn_id:PreviousDT | lpcn_id:CurrentDT">
		...
		<br/>
		<xsl:if test="lpcn_id:GoodsNumeric">
			<xsl:text>№ товара - </xsl:text>
			<xsl:for-each select="lpcn_id:GoodsNumeric">
	...
	<xsl:template match="lpcn_id:Customs">
	...
	<!-- Шаблон для типа lpcn_id:CustomsPersonSignatureType -->
	<xsl:template match="lpcn_id:CustomsPerson">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
			...
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="lpcn_id:IssueDate" mode="russian_date" />
					</td>
				</tr>
			</table>
		</div>
	</xsl:template>
	<!-- Шаблон для типа lpcn_id:DeclaredGoodType -->
	<xsl:template match="lpcn_id:DeclaredGood">
		<tr valign="top">
			<td class="bordered">
				<xsl:value-of select="lpcn_id:GoodsNumeric"/>
			</td>
			<td class="bordered">
				<xsl:apply-templates select="lpcn_id:PermIdentifNumber"/>
			</td>
			<td class="bordered">
				<xsl:for-each select="lpcn_id:PreviousDT">
					<xsl:apply-templates select="."/>
				</xsl:for-each>
			</td>
			<td class="bordered">
				<xsl:choose>
					<xsl:when test="lpcn_id:DecisionText='1'">
						<xsl:text>товар идентифицирован</xsl:text>
					</xsl:when>
					<xsl:when test="lpcn_id:DecisionText='2'">
						<xsl:text>товар не идентифицирован</xsl:text>
					</xsl:when>
				</xsl:choose>
			</td>
			<td class="bordered">
				<xsl:value-of select="lpcn_id:Justification"/>
			</td>
		</tr>
	</xsl:template>
