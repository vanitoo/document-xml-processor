...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:notccz="urn:customs.ru:Information:CustomsDocuments:OEZ_GoodsNotifCCZ:5.26.0">
	<!-- Шаблон для типа OEZ_GoodsNotifCCZType -->
	<xsl:template match="notccz:OEZ_GoodsNotifCCZ">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<xsl:choose>
											<xsl:when test="notccz:DocKind='1'">
												<b>Уведомление о размещении товаров в ЗТК</b>
											</xsl:when>
											<xsl:when test="notccz:DocKind='2'">
												<b>Сообщение о необходимости предъявления товара в ЗТК</b>
											</xsl:when>
											<xsl:otherwise>
												<b>Уведомление о размещении товаров в ЗТК</b>
											</xsl:otherwise>
					...
					<br/>
					<xsl:if test="notccz:Customs">
						<div class="title marg-top">Наименование и код таможенного органа </div>
						<div>
							<xsl:apply-templates select="notccz:Customs">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="notccz:Applicant">
						<div class="title marg-top">Сведения об инициаторе вывоза товаров ЕАЭС (назначенном операторе почтовой связи)/ Сведения о заинтересованном лице</div>
						<div>
							<xsl:apply-templates select="notccz:Applicant">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="notccz:RegistrationNumber">
						<div class="title marg-top">Опись документов по которой принято решение/ Заявление о проведении идентификации</div>
						<div>
							<xsl:apply-templates select="notccz:RegistrationNumber">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="notccz:CCZAddress">
						<div class="title marg-top">Место нахождения ЗТК</div>
						<div>
							<xsl:apply-templates select="notccz:CCZAddress">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="notccz:PlacementTerm">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Срок размещения товаров в ЗТК </td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="notccz:PlacementTerm" mode="russian_date"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="notccz:CustomsPerson">
						<div class="title marg-top">Сведения о должностном лице таможенного органа</div>
						<div>
							<xsl:apply-templates select="notccz:CustomsPerson">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<xsl:template match="notccz:Applicant">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<!-- Шаблон для типа RUScat_ru:RUAddressType -->
	<xsl:template match="notccz:CCZAddress">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<xsl:template match="notccz:Customs">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<xsl:template match="notccz:CustomsPerson">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<!-- Шаблон для типа notccz:RegistrationNumberType -->
	<xsl:template match="notccz:RegistrationNumber">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="notccz:CustomsCode"/>
						<xsl:text>/</xsl:text>
						<xsl:apply-templates select="notccz:RegistrationDate" mode="num_date"/>
						<xsl:text>/</xsl:text>
						<xsl:apply-templates select="notccz:Number"/>
						<xsl:if test="notccz:DocKind">
							<xsl:text>/</xsl:text>
							<xsl:apply-templates select="notccz:DocKind"/>
						</xsl:if>
					</td>
				</tr>
			</table>
	...
	<xsl:template match="//*[local-name()='OEZ_GoodsNotifCCZ']//*" priority="-1">
