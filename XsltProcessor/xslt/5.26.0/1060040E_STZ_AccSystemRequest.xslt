...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:stz_asreq="urn:customs.ru:Information:CustomsDocuments:STZ_AccSystemRequest:5.25.0">
	<!-- Шаблон для типа STZ_AccSystemRequestType -->
	<xsl:template match="stz_asreq:STZ_AccSystemRequest">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Сообщение о запросе информации из АСУ</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<br/>
					<xsl:if test="stz_asreq:Customs">
						<div class="title marg-top">Сведения о таможенном органе</div>
						<div>
							<xsl:apply-templates select="stz_asreq:Customs">
					...
					<table>
						<tr>
							...
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="stz_asreq:Date" mode="russian_date"/>
							</td>
						</tr>
					</table>
					<xsl:if test="stz_asreq:Resident">
						<div class="title marg-top">Сведения о резиденте СТЗ</div>
						<div>
							<xsl:apply-templates select="stz_asreq:Resident">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="stz_asreq:Term">
						<div class="title marg-top">Период времени, за который запрашивается ЭУД</div>
						<div>
							<xsl:apply-templates select="stz_asreq:Term">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="stz_asreq:DTNumbers">
						<div class="title marg-top">Номера таможенных деклараций, по которым необходимо представить ЭУД</div>
						<table class="bordered">
							<tbody>
								...
								<xsl:for-each select="stz_asreq:DTNumbers">
					...
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Признак дополнительных ЭУД</td>
							<td class="value graphMain" style="width:50%">
								<xsl:choose>
									<xsl:when test="stz_asreq:AddSign='0'">
										<xsl:text>без предоставления дополнительных ЭУД</xsl:text>
									</xsl:when>
									<xsl:when test="stz_asreq:AddSign='1'">
										<xsl:text>включая дополнительные ЭУД</xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="stz_asreq:AddSign"/>
									</xsl:otherwise>
								</xsl:choose>
							</td>
						</tr>
					</table>
					<xsl:if test="stz_asreq:CustomsPerson">
						<div class="title marg-top">Должностное лицо уполномоченного таможенного органа</div>
						<div>
							<xsl:apply-templates select="stz_asreq:CustomsPerson">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<xsl:template match="stz_asreq:Customs">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<xsl:template match="stz_asreq:CustomsPerson">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<xsl:template match="stz_asreq:DTNumbers">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<tr>
	...
	<!-- Шаблон для типа stz_asreq:ResidentType -->
	<xsl:template match="stz_asreq:Resident">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="stz_asreq:ResidentCertificate">
				<div class="title marg-top">Cвидетельство о включении в реестр резидентов</div>
				<div>
					<xsl:apply-templates select="stz_asreq:ResidentCertificate">
						<xsl:with-param name="wdth" select="$w" />
					</xsl:apply-templates>
				</div>
			</xsl:if>
	...
	<!-- Шаблон для типа stz_asreq:ResidentCertificateType -->
	<xsl:template match="stz_asreq:ResidentCertificate">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asreq:CertificateNumber" />
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asreq:CertificateDate" mode="russian_date" />
					</td>
				</tr>
			</table>
			<xsl:if test="stz_asreq:CertificateValidity">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Срок действия</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_asreq:CertificateValidity" mode="russian_date" />
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="stz_asreq:CertificateAgency">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Наименование органа, выдавшего свидетельство</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_asreq:CertificateAgency" />
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<!-- Шаблон для типа stz_asreq:TermType -->
	<xsl:template match="stz_asreq:Term">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asreq:BeginDate" mode="russian_date"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asreq:EndDate" mode="russian_date"/>
					</td>
				</tr>
			</table>
		</div>
	...
	<xsl:template match="//*[local-name()='STZ_AccSystemRequest']//*" priority="-1">
