...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.24.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:qrdr="urn:customs.ru:Information:CustomsDocuments:QuotaRDReport:5.24.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:template mode="doc" match="*">
		...
		<td style="font-size: 8pt;">
			<xsl:choose>
				<xsl:when test="qrdr:CustDocType='ДТ'"><span>Декларация на товары: </span></xsl:when>
				<xsl:when test="qrdr:CustDocType='ПР'"><span>Сообщение о прибытии: </span></xsl:when>
				<xsl:when test="qrdr:CustDocType='УБ'"><span>Сообщение об убытии: </span></xsl:when>
				<xsl:when test="qrdr:CustDocType='ТД'"><span>Транзитная декларация: </span></xsl:when>
			</xsl:choose>
			<br/>
	...
	<xsl:template match="qrdr:GTDID">
		<span><xsl:value-of select="cat_ru:CustomsCode"/></span>
		<xsl:text>/</xsl:text>
		<span><xsl:call-template name="gtd_date"><xsl:with-param name="dateIn" select="cat_ru:RegistrationDate"/></xsl:call-template></span>
		<xsl:text>/</xsl:text>
		<span><xsl:value-of select="cat_ru:GTDNumber"/></span>
	</xsl:template>
	...
	<xsl:template name="gtd_date">
		...
		<xsl:choose>
			<xsl:when test="substring($dateIn,5,1)='-' and substring($dateIn,8,1)='-'">
				<span><xsl:value-of select="substring($dateIn,9,2)"/></span>
				<span></span>
				<span><xsl:value-of select="substring($dateIn,6,2)"/></span>
				<span></span>
				<span><xsl:value-of select="substring($dateIn,3,2)"/></span>
			</xsl:when>
			<xsl:otherwise>
				<span><xsl:value-of select="$dateIn"/></span>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
