...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:rgncwc="urn:customs.ru:Information:ReportingCollection:RepGoodsNoCustomsWarehouseCancel:5.25.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0">
	<!-- Шаблон для типа RepGoodsNoCustomsWarehouseCancelType -->
	<xsl:template match="rgncwc:RepGoodsNoCustomsWarehouseCancel">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
						...
						<tbody>
							<tr>
								<td align="center" class="title">
									<font size="4">
										<b>Уведомление об отмене Отчета лица, поместившего товары под таможенную процедуру таможенного склада и осуществляющего их хранение в местах, не являющихся таможенными складами</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Дата подачи уведомления</td>
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="rgncwc:NotifDate" mode="russian_date"/>
							</td>
						</tr>
					</table>
					<xsl:if test="rgncwc:ReportRegNumber">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Регистрационный номер отчета</td>
								<td class="value graphMain" style="width:50%">
									<xsl:apply-templates select="rgncwc:ReportRegNumber"/>
								</td>
							</tr>
						</table>
					</xsl:if>
					<xsl:if test="rgncwc:Organization">
						<div class="title marg-top">Сведения об организации / физическом лице</div>
						<div>
							<xsl:apply-templates select="rgncwc:Organization">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="rgncwc:Customs">
						<div class="title marg-top">Таможенный орган подачи уведомления</div>
						<div>
							<xsl:apply-templates select="rgncwc:Customs">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="rgncwc:ApplicantSignature">
						<div class="title marg-top">Сведения о заявителе</div>
						<div>
							<xsl:apply-templates select="rgncwc:ApplicantSignature">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Причина отмены</td>
							<td class="graphMain value" style="width:50%">
								<xsl:for-each select="rgncwc:CancelReason">
									<xsl:value-of select="."/>
					...
					<xsl:if test="rgncwc:AttachedDocuments">
						...
						<table class="bordered">
							<tbody>
								<tr class="title">
									...
									<td class="graphMain bordered">Код вида документа (заполняется в соответствии с классификатором видов документов, используемых при заявлении сведений в графе 44 ДТ)</td>
									<td class="graphMain bordered">Уникальный идентификатор декларанта в Электронном Архиве Декларанта</td>
									<td class="graphMain bordered">Уникальный идентификатор архива декларанта (используется при запросе документа из архива)</td>
									<td class="graphMain bordered">Уникальный идентификатор документа в архиве (используется при запросе документа из архива)</td>
									<td class="graphMain bordered">Уникальный идентификатор документа (используется при запросе документа из архива)</td>
									<td class="graphMain bordered">Статус документа в архиве</td>
									<td class="graphMain bordered">Дата/время помещения документа в архив (последнего изменения документа)</td>
								...
								<xsl:for-each select="rgncwc:AttachedDocuments">
	...
	<xsl:template match="rgncwc:ApplicantSignature">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<!-- Шаблон для типа rgncwc:DocumentInfoType -->
	<xsl:template match="rgncwc:AttachedDocuments">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<tr>
				...
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:DocCode"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:ArchDeclID"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:ArchID"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:ArchDocID"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:ArchDocumentID"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:ArchDocStatus"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:ArchDocDate"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:PrDocumentTerm" mode="russian_date"/>
				</td>
				<td class="graphMain bordered">
					<xsl:apply-templates select="rgncwc:DocumentModeID"/>
				</td>
			</tr>
		</div>
	</xsl:template>
	<!-- Шаблон для типа cat_ru:CustomsType -->
	<xsl:template match="rgncwc:Customs">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<!-- Шаблон для типа rgncwc:OrganizationInfoType -->
	<xsl:template match="rgncwc:Organization">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<xsl:if test="RUScat_ru:OrganizationName">
			...
			<table>
				<tr>
					<td class="annot graphMain" style="width:50%">Тип заявителя (1 - юридическое лицо, 2 - индивидуальный предприниматель, 3 – физическое лицо)</td>
					<td class="value graphMain" style="width:50%">
						<xsl:choose>
							<xsl:when test="rgncwc:ApplicantType='1'">
								<xsl:text>юридическое лицо</xsl:text>
							</xsl:when>
							<xsl:when test="rgncwc:ApplicantType='2'">
								<xsl:text>индивидуальный предприниматель</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="rgncwc:ApplicantType"/>
							</xsl:otherwise>
						</xsl:choose>
					</td>
				</tr>
	...
	<xsl:template match="//*[local-name()='RepGoodsNoCustomsWarehouseCancel']//*" priority="-1">
