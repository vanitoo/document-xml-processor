...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.24.0" xmlns:stz_asres="urn:customs.ru:Information:CustomsDocuments:STZ_AccSystemResponse:5.25.0">
	<!-- Шаблон для типа STZ_AccSystemResponseType -->
	<xsl:template match="stz_asres:STZ_AccSystemResponse">
		...
		<html>
			...
			<body>
				<div class="page" style="width: {$w}mm;">
					...
					<br/>
					<xsl:if test="stz_asres:Customs">
						<div class="title marg-top">Сведения о таможенном органе</div>
						<div>
							<xsl:apply-templates select="stz_asres:Customs">
					...
					<table>
						<tr>
							<td class="annot graphMain" style="width:50%">Дата сообщения</td>
							<td class="value graphMain" style="width:50%">
								<xsl:apply-templates select="stz_asres:Date" mode="russian_date"/>
							</td>
						</tr>
					</table>
					<xsl:if test="stz_asres:Resident">
						<div class="title marg-top">Сведения о резиденте СТЗ</div>
						<div>
							<xsl:apply-templates select="stz_asres:Resident">
								<xsl:with-param name="wdth" select="$w" />
							</xsl:apply-templates>
						</div>
					</xsl:if>
					<xsl:if test="stz_asres:EAD">
						<div class="title marg-top">Перечень ЭУД</div>
						<xsl:for-each select="stz_asres:EAD">
							<xsl:apply-templates select=".">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</xsl:for-each>
					</xsl:if>
					<xsl:if test="stz_asres:NotSendingReasons">
						<table>
							<tr>
								<td class="annot graphMain" style="width:50%">Причины не направления ЭУД</td>
								<td class="graphMain value" style="width:50%">
									<xsl:for-each select="stz_asres:NotSendingReasons">
										<xsl:value-of select="."/>
					...
					<xsl:if test="stz_asres:Person">
						<div class="title marg-top">Лицо, уполномоченное участником ОЭЗ</div>
						<div>
							<xsl:apply-templates select="stz_asres:Person">
								<xsl:with-param name="wdth" select="$w"/>
							</xsl:apply-templates>
						</div>
					</xsl:if>
	...
	<xsl:template match="stz_asres:Customs">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
	...
	<!-- Шаблон для типа cat_ru:DocLinkType -->
	<xsl:template match="stz_asres:DocLink">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="cat_ru:ArchiveID"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="cat_ru:DocID"/>
					</td>
				</tr>
			</table>
		</div>
	</xsl:template>
	<!-- Шаблон для типа stz_asres:EADType -->
	<xsl:template match="stz_asres:EAD | stz_asres:AddEAD">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<xsl:variable name="bgc">
				<xsl:choose>
					<xsl:when test="local-name() = 'EAD'">#aaa</xsl:when>
					<xsl:otherwise>#ccc</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<div class="title marg-top" style="background-color: {$bgc}">Номер ЭУД</div>
			<xsl:apply-templates select="stz_asres:EAD_Number">
				<xsl:with-param name="wdth" select="$w"/>
			</xsl:apply-templates>
			
			<xsl:if test="stz_asres:AddEAD_Number">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Номер дополнения к ЭУД по порядку</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_asres:AddEAD_Number"/>
						</td>
					</tr>
				</table>
			</xsl:if>
			
			<div class="title marg-top">Идентификаторы, присвоенные документу при размещении в ЭАД</div>
			<xsl:apply-templates select="stz_asres:DocLink">
				<xsl:with-param name="wdth" select="$w"/>
			</xsl:apply-templates>
	
			<xsl:if test="stz_asres:AddEAD">
				<div class="title marg-top">Идентификаторы, присвоенные документу при размещении в ЭАД</div>
				<xsl:apply-templates select="stz_asres:AddEAD">
					<xsl:with-param name="wdth" select="$w"/>
				</xsl:apply-templates>
			</xsl:if>
		</div>
	</xsl:template>
	<!-- Шаблон для типа stz_asres:EAD_NumberType -->
	<xsl:template match="stz_asres:EAD_Number">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asres:INN"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asres:EAD_Date" mode="russian_date"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asres:Number"/>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:choose>
							<xsl:when test="stz_asres:GoodsKind='СТЗ'">
								<xsl:text>товары помещенные</xsl:text>
							</xsl:when>
							<xsl:when test="stz_asres:GoodsKind='ПП'">
								<xsl:text>товары</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="stz_asres:GoodsKind"/>
							</xsl:otherwise>
						</xsl:choose>
					</td>
				</tr>
			</table>
			<xsl:if test="stz_asres:NumPP">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Порядковый номер версии ЭУД</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_asres:NumPP"/>
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<xsl:template match="stz_asres:Person">
		<xsl:param name="wdth" select="0"/>
		<xsl:variable name="w" select="$wdth - 5"/>
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
	...
	<!-- Шаблон для типа stz_asres:ResidentType -->
	<xsl:template match="stz_asres:Resident">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			...
			<xsl:if test="stz_asres:ResidentCertificate">
				<div class="title marg-top">Cвидетельство о включении в реестр резидентов</div>
				<div>
					<xsl:apply-templates select="stz_asres:ResidentCertificate">
						<xsl:with-param name="wdth" select="$w" />
					</xsl:apply-templates>
				</div>
			</xsl:if>
	...
	<!-- Шаблон для типа stz_asres:ResidentCertificateType -->
	<xsl:template match="stz_asres:ResidentCertificate">
		<xsl:param name="wdth" select="0" />
		<xsl:variable name="w" select="$wdth - 5" />
		<div style="width:100%; min-width:{$w}mm; max-width:{$w}mm; padding-left:5mm;">
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asres:CertificateNumber" />
					</td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td class="value graphMain" style="width:50%">
						<xsl:apply-templates select="stz_asres:CertificateDate" mode="russian_date" />
					</td>
				</tr>
			</table>
			<xsl:if test="stz_asres:CertificateValidity">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Срок действия</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_asres:CertificateValidity" mode="russian_date" />
						</td>
					</tr>
				</table>
			</xsl:if>
			<xsl:if test="stz_asres:CertificateAgency">
				<table>
					<tr>
						<td class="annot graphMain" style="width:50%">Наименование органа, выдавшего свидетельство</td>
						<td class="value graphMain" style="width:50%">
							<xsl:apply-templates select="stz_asres:CertificateAgency" />
						</td>
					</tr>
				</table>
			</xsl:if>
	...
	<xsl:template match="//*[local-name()='STZ_AccSystemResponse']//*" priority="-1">
