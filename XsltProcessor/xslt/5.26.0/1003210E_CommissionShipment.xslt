...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
															...
															xmlns:catTrans_ru="urn:customs.ru:Information:TransportDocuments:TransportCommonAgregateTypesCust:5.24.0"
															xmlns:cltTrans_ru="urn:customs.ru:Information:TransportDocuments:TransportCommonLeafTypesCust:5.14.3"
											        xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.17.0"
                              xmlns:cshi="urn:customs.ru:Information:TransportDocuments:Sea:CommissionShipment:5.25.0">
          ...
          <table class="borderer w190">
            ...
            <tr>
              <td class="graphMain value_b" colspan="3" style="width: 41.1%">
                <xsl:apply-templates select="cshi:Shipowner/cat_ru:OrganizationName"/>
                <br/>
                <xsl:apply-templates select="cshi:Shipowner/cat_ru:Address"/>
              </td>
            ...
            <tr>
              ...
              <td class="graphMain value_b" colspan="3" style="width: 29.58%">
                <xsl:value-of select="count(cshi:AccompanyingDocument)"/>
                <br/>
            ...
            <tr>
              <td class="graphMain bordered" colspan="10" style="width: 100%">
                <br/>
              </td>
            </tr>
            ...
            <tr align="center">
              ...
              <td class="graphMain value_b" style="width: 6%; vertical-align: middle">
                <br/>
              </td>
              <td class="graphMain value_b" style="width: 6%; vertical-align: middle">
                <br/>
            ...
            <tr>
              <td class="graphMain value_b" colspan="10" style="width: 100%">
                ...
                <xsl:if test="cshi:TotalGrossWeightQuantity or cshi:TotalNetWeightQuantity or cshi:TotalVolumeQuantity">
                  <br/>
                </xsl:if>
            ...
            <tr align="center">
              <td class="graphMain value_b" style="width: 29.5%; vertical-align: middle">
                <br/>
              </td>
            ...
            <tr>
              <td class="graphMain" colspan="2" style="width: 35.3%; border-left: 1px solid gray">
                <b>
                  Manager/менеджер (конт. телефон):<br/><br/>ДАТА ПОДПИСАНИЯ
                </b>
              </td>
            ...
            <table class="bordered w190">
              <tbody>
                <tr class="title">
                  ...
                  <xsl:if test="cshi:CommissionShipmentGoods/cshi:CargoAcceptanceDoc">
					  <td class="graphMain bordered">Док-т, оформленный при приемке груза в порту</td>
                  </xsl:if>
                </tr>
  ...
  <xsl:template match="cat_ru:Address">
    ...
    <xsl:if test="cat_ru:City">
      <xsl:if test="cat_ru:Region or cat_ru:CounryName">
        <xsl:text>, </xsl:text>
      </xsl:if>
      <xsl:value-of select="cat_ru:City"/>
    </xsl:if>
    <xsl:if test="cat_ru:StreetHouse">
      <xsl:if test="cat_ru:Region or cat_ru:CounryName or cat_ru:City">
        <xsl:text>, </xsl:text>
      </xsl:if>
      <xsl:value-of select="cat_ru:StreetHouse"/>
    </xsl:if>
    <xsl:if test="cat_ru:TerritoryCode">
      <xsl:if test="cat_ru:Region or cat_ru:CounryName or cat_ru:City or cat_ru:StreetHouse">
        <xsl:text>, </xsl:text>
      </xsl:if>
  ...
  <xsl:template match="cshi:AccompanyingDocument">
    ...
    <xsl:if test="cshi:DocumentKind">
      ...
      <xsl:choose>
        <xsl:when test="cshi:DocumentKind='1'">
          <xsl:text>Коносамент</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='2'">
          <xsl:text>Копия коносамента</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='3'">
          <xsl:text>Спецификация для погрузки</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='4'">
          <xsl:text>Сертификат для погрузки</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='5'">
          <xsl:text>Разрешение капитана на погрузку на судно</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='6'">
          <xsl:text>Накладная для получателя</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='7'">
          <xsl:text>Копия накладной для получателя</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='8'">
          <xsl:text>Спецификация для получателя</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='9'">
          <xsl:text>Сертификат для получателя</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='10'">
          <xsl:text>Разрешение капитана на выгрузку с судна</xsl:text>
        </xsl:when>
        <xsl:when test="cshi:DocumentKind='99'">
          <xsl:text>иное</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="cshi:DocumentKind"/>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:text>)</xsl:text>
    </xsl:if>
  ...
  <xsl:template match="cshi:AdministrationInfo">
    <xsl:if test="cshi:SignatureAdministration">
      <u>
        <xsl:value-of select="cshi:SignatureAdministration"/>
      </u>
    </xsl:if>
  ...
  <xsl:template match="cshi:CommissionShipmentGoods">
    <tr>
      <td class="graphMain bordered">
        <xsl:value-of select="cshi:GoodsNumericDT"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="cshi:GoodsNumeric"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="cshi:GoodsTNVEDCode"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="cshi:CustomsProcedure"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="cshi:CustomsModeCode"/>
      </td>
      <td class="graphMain bordered">
        <xsl:for-each select="cshi:GoodsDescription">
          <xsl:value-of select="."/>
      ...
      <td class="graphMain bordered">
        <xsl:value-of select="cshi:GoodsMarking"/>
      </td>
      <!--td class="graphMain bordered"><xsl:value-of select="cshi:Dimensions"/></td-->
      <td class="graphMain bordered">
        <xsl:apply-templates select="cshi:GoodsQuantity"/>
      </td>
      <td class="graphMain bordered">
        <xsl:apply-templates select="cshi:Places"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="translate(cshi:VolumeQuantity, '.', ',')"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="translate(cshi:GrossTareWeightQuantity, '.', ',')"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="translate(cshi:NetWeightQuantity, '.', ',')"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="translate(cshi:GrossWeightQuantity, '.', ',')"/>
      </td>
      <td class="graphMain bordered">
        <xsl:value-of select="translate(cshi:WeightTareQuantity, '.', ',')"/>
      </td>
      ...
      <td class="graphMain bordered">
        <xsl:for-each select="cshi:ContainerInfo">
          <xsl:apply-templates select="."/>
          <br/>
        </xsl:for-each>
      </td>
      <td class="graphMain bordered">
        <xsl:apply-templates select="cshi:DTNumber"/>
      </td>
  ...
  <xsl:template match="cshi:Consignee| cshi:Consignor| cshi:Carrier| cshi:WhomNotify">
    ...
    <xsl:if test="cat_ru:ShortName or cat_ru:OrganizationName">
      <br/>
    </xsl:if>
