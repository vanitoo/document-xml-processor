...
<xsl:stylesheet exclude-result-prefixes="  cat_ru rslt" version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.24.0" xmlns:rslt="urn:customs.ru:Information:ExchangeDocuments:Result:5.24.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                     ...
                     <xsl:for-each select="rslt:Response">
                        <tr>
                           <td class="graph" style="border-bottom:solid 0pt #000000;">
                              <span class="graph" style="background-color:#aaa;">Идентификатор запроса / обработанного документа: </span>
                              <xsl:value-of select="rslt:RefDocumentID"/>
                              <br/>
                              <xsl:for-each select="rslt:ResultInformation">
								 <xsl:if test="position() != 1"><br/></xsl:if>
                                 <xsl:if test="rslt:ResultSource">
                                    <span class="graph" style="background-color: #ccc;">Подсистема, вернувшая результат: </span>
